...
c = c.replace('"daily_basic": (basic, ["turnover_rate", "turnover_rate_f", "volume_ratio",',
...
# Fix rename_map
c = c.replace('    "up_limit": "up_limit_raw", "down_limit": "down_limit_raw",\n    ', '')

# Add free_float_turnover_rate mapping
c = c.replace('# LHB merge',
...
c = c.replace('if "turn" in panel_cols and "turnover_rate" in df.columns:\n    df["turn"] = df["turnover_rate"]\n\n', '')
...
'''
c = c.replace('# --- Simple derived features', sw_block + '# --- Simple derived features')
...
# Fix audit
c = c.replace('print(f"Panel: {pf2.metadata.num_rows:,} rows")',
              'print(f"Panel: {pf2.metadata.num_rows:,} rows, {len(pf2.schema_arrow.names)} cols")')
c = c.replace('''if empty_cols:
...
# Fix pct_con comment
c = c.replace('# Formula from cyq_calculator.py: concentration = (hi-lo)/(hi+lo)',
              '# Formula from cyq_calculator.py: concentration = (hi-lo)/(hi+lo)\n# Result in [0, 1]: smaller = more concentrated (bullish)')

# Update docstring
c = c.replace('- pct_70_con, pct_90_con (cyq_calculator formula)',
              '- pct_70_con, pct_90_con (cyq formula: (hi-lo)/(hi+lo), [0,1] range)')
c = c.replace('- Forward-fill: financials, margin(T+1 gap), SW indices, shareholders',
