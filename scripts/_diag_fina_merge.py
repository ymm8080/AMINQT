...
logger.info(
    "ann_date dtype: %s, sample: %s",
    new_raw["ann_date"].dtype,
    new_raw["ann_date"].head(3).tolist(),
)
new_norm = normalize_fina(new_raw.copy())
logger.info(
    "After normalize: announce_date dtype=%s, sample=%s",
    new_norm["announce_date"].dtype,
    new_norm["announce_date"].head(3).tolist(),
)
logger.info(
    "New cache date range: %s ~ %s",
    new_norm["announce_date"].min(),
    new_norm["announce_date"].max(),
)
...
if per_files:
    ...
    logger.info(
        "Per-stock file %s: %d rows, cols=%s",
        per_files[0],
        len(sample),
    ...
    if "announce_date" in sample.columns:
        logger.info(
            "  has announce_date, dtype=%s, sample=%s",
            sample["announce_date"].dtype,
            sample["announce_date"].head(3).tolist(),
        )
...
logger.info(
    "announce_date range: %s ~ %s",
    fina["announce_date"].min(),
    fina["announce_date"].max(),
)
...
logger.info(
    "Pre-2023-07-01 rows: %d, stocks: %d",
    len(early_2023),
    early_2023["symbol"].nunique(),
)
if len(early_2023) > 0:
    logger.info(
        "Sample: %s",
        early_2023[["symbol", "date", "roe", "eps_yoy"]].head(5).to_string(),
    )
...
logger.info(
    "V3 data for 000620: %d rows, roe NaN=%d (%.1f%%)",
    len(v3_stock),
    v3_stock["roe"].isna().sum(),
    v3_stock["roe"].isna().mean() * 100,
)
