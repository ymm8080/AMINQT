...
def _groll(s: pd.Series, key: pd.Series, n: int) -> pd.Series:
    ...
    return (s.groupby(key, sort=False).rolling(n, min_periods=n).mean()
...
def load_probs(board: str, predictor, oos_start) -> tuple[pd.DataFrame, str]:
    ...
    k, kind = next(((k, kind) for k, kind in _PROB_PRIORITY_MODEL
                    if kind in models), (None, None))
    if kind is None:
        raise RuntimeError(f"[{board}] 模型包无 {_PROB_PRIORITY_MODEL}")
    df = pd.read_parquet(
        ckpt, columns=feature_cols + ["symbol", "date"],
    ...
    out = pd.DataFrame({"symbol": df["symbol"].values,
                        "date": df["date"].values,
                        "prob_up": prob})
    del df, X, raw, prob
    gc.collect()
...
def measure(sub: pd.DataFrame, score: pd.Series, spec, bcrit) -> dict:
    ...
    picks = picks.merge(sub[["date", "symbol"] + list(spec.labels)],
                        on=["date", "symbol"], how="left")
    per = {}
    for h, lab in zip(spec.horizons, spec.labels):
        m = measure_dual_head(picks, lab)
        per[h] = {"mag": m["mag"], "winrate": m["winrate"], "n": m["n"],
...
def _print_methods(methods: dict) -> None:
    print(f"    {'方法':<16}{'3d_wr':>8}{'3d_mag':>9}{'5d_wr':>8}{'5d_mag':>9}"
    ...
    for name, m in methods.items():
        ...
        for h in ("3d", "5d", "10d", "2d"):
            r = ph.get(h, {})
            row += (f"{_fmt(r.get('winrate'), True):>8}"
                    f"{_fmt(r.get('mag'), True):>9}")
        print(row)


def main() -> int:
    ap = argparse.ArgumentParser(description="池分 vs LEGACY prob 正交性 + 叠加 OOS 双头")
    ...
    out: dict = {"ts": "2026-08-05", "type": "overlay_orthogonality",
    ...
    for board in (["main", "dual"] if not args.board else [args.board]):
        ...
        print(f"OOS{args.window} {pd.Timestamp(oos_start).date()} → "
              f"{pd.Timestamp(sub['date'].max()).date()} ({d} 交易日) | "
              f"行 {len(sub):,} | 阈值 wr>={bcrit[0]} mag>{bcrit[1]}", flush=True)
        ...
        board_out: dict = {"prob_col": pc, "prob_coverage": cover,
                           "systems": {}}
        for name, spec in SYSTEMS.items():
            ...
            s["combined_nopv"] = (W_POOL * s["pool_score_nopv"]
                                  + W_PROB * prob_f)
            ...
            ok = all(
                ...
                and abs((mine["per_horizon"].get(h, {}).get("mag") or 0)
                        - (ref["per_horizon"].get(h, {}).get("mag") or 0)) < 1e-9
            ...
            print(f"  正交性 Spearman(池分, {pc}): avg={ortho['spearman_avg']} "
            ...
            print(f"  3d: 叠加 vs 纯池 Δwr {_fmt(_safe_delta(d3.get('winrate'), p3.get('winrate')), True)} "
        ...
        out["boards"][board] = {"latest": str(sub["date"].max()),
