...
N_DAYS = int(os.getenv("LT_N_DAYS", "300"))  # 采样每股最近的交易日数 (含 rolling warmup, 前 ~60 天弃用)
...
NEW_TRANSFORMS = {
    "skew": {"windows": (20, 60), "fn": lambda s, w: pd.Series(s).rolling(w, min_periods=5).skew()},
    "kurt": {"windows": (60,), "fn": lambda s, w: pd.Series(s).rolling(w, min_periods=20).kurt()},
    "zscore": {
        "windows": (20, 60),
        "fn": lambda s, w: (
    ...
    "mdd": {
        "windows": (20, 60),
        "fn": lambda s, w: pd.Series(s).rolling(w, min_periods=5).max() / pd.Series(s) - 1,
    },
    "cv": {
        "windows": (20, 60),
        "fn": lambda s, w: pd.Series(s).rolling(w, min_periods=5).std()
...
def load_sample() -> pd.DataFrame:
    ...
    pick = sorted(np.random.RandomState(RNG).choice(syms.unique(), size=N_SYMS, replace=False))
...
def daily_spearman_ic(feat_df: pd.DataFrame, label: pd.Series, date: pd.Series) -> pd.Series:
...
def dedup_ic_keep(feats, df, ic_series, threshold=0.7):
    ...
    for _base, cols in groups.items():
        ...
        if ic_series is not None:
            order = sorted(avail, key=lambda c: abs(ic_series.get(c, 0.0)), reverse=True)
        dropped = set()
        ...
        for col in CORE_COLS:
            ...
            for fam, spec in NEW_TRANSFORMS.items():
                for w in spec["windows"]:
                    ...
                    feats[f"{col}_{fam}{w}"] = np.asarray(vals, dtype=np.float32).reshape(-1)
...
def main() -> None:
    ...
    df = load_sample()
    logger.info("sample %d rows x %d cols in %.1fs", len(df), df.shape[1], time.time() - t0)
    labels = build_labels(df)
    label = labels[f"fwd{LABEL_FWD}"]
    # 丢弃无未来标签的最后 k 行 (每股)
    mask = label.notna()
    df, labels, label = df[mask].reset_index(drop=True), labels[mask].reset_index(drop=True), label[mask].reset_index(drop=True)
    ...
    t1 = time.time()
    fams = {}
    for fam in ["pct_change", "rolling_mean", "rolling_std", "rolling_max", "diff", "momentum", "EMA"]:
    ...
    logger.info("OOS split: train %d dates (<=%s) / test %d dates", int(train_mask.sum()), cut, int(test_mask.sum()))
    ...
    report = {
        ...
        "sample": {"n_syms": N_SYMS, "n_rows": int(len(df)), "n_eligible_cols": int(len(eligible))},
        "label_fwd": LABEL_FWD,
        "oos_split": {"train_dates": int(train_mask.sum()), "test_dates": int(test_mask.sum()), "cut_date": str(cut)},
        "directions": {},
    }
    ...
    ic_test_existing = daily_spearman_ic(
        F.iloc[test_mask].loc[:, core_mask], label.iloc[test_mask], df["date"].iloc[test_mask]
    )
    ...
    report["directions"]["D3_new_vs_existing"] = {
        ...
        "new_top_features": ic_test_new.abs().sort_values(ascending=False).head(10).to_dict(),
        "existing_top_features": ic_test_existing.abs().sort_values(ascending=False).head(10).to_dict(),
    }

    # ── 备选视界稳健性 (B vs D2, test 期 fwd2/fwd10) ──
    for k in (2, 10):
        ik_test = daily_spearman_ic(
            F.iloc[test_mask], labels.iloc[test_mask][f"fwd{k}"], df["date"].iloc[test_mask]
        )
        report["directions"]["B_baseline_var_dedup"]["ic"][f"mean_abs_ic_fwd{k}"] = float(
            ik_test.reindex(sel_b).abs().mean()
        )
    ...
    print(f"sample: {N_SYMS} syms x {len(df)} rows | label fwd{LABEL_FWD} | eligible cols {len(eligible)}")
    print("=" * 72)
    print(f"{'direction':<28}{'n_feat':>8}{'mean|IC|':>10}{'%|IC|>2%':>10}{'fwd2':>8}{'fwd10':>8}")
    ...
    print(f"D3 new-families : mean|IC| {d3['new_family_mean_abs_ic_test']:.4f} (n={d3['new_family_n']}) [test期]")
    print(f"   vs existing  : mean|IC| {d3['existing_family_mean_abs_ic_test']:.4f} (n={d3['existing_family_n']}) on same core cols [test期]")
    print("\nnew-family top:", {k: round(v, 4) for k, v in list(d3['new_top_features'].items())[:6]})
    print("existing  top:", {k: round(v, 4) for k, v in list(d3['existing_top_features'].items())[:6]})
    print(f"\nWORM: {out_path}")
