...
def main() -> int:
    ...
    print(
        f"[panel] sliced last {WARMUP_DAYS} trading days -> {len(panel):,}r", flush=True
    )
    ...
    n_B = len(keep_B)
    print(
        f"[B] eval 窗口 {eval_start:%Y-%m-%d}..{b_dates[-1]:%Y-%m-%d} "
    ...
    dfA = prepare_board_frame(
        dual_a, features, cross_sectional_rank=True, registry=registry
    )
    del dual_a
    gc.collect()
    print(f"[A] 特征帧 {len(dfA):,}r x {len(dfA.columns)}c", flush=True)

    eval_A = dfA[dfA["date"] >= eval_start].merge(
        keep_B, on=["date", "symbol"], how="inner"
    )
    del dfA
    gc.collect()
    ...
    for tag, cols in bundle_cols.items():
        ...
        keep_cols = [c for c in (cols + labels) if c in eval_A.columns] + [
            "date",
            "symbol",
        ]
        ...
        if missingA or missingB:
            print(
                f"[FATAL] {tag} 缺失特征列 A={missingA[:5]} B={missingB[:5]} → abort",
                flush=True,
            )
            return 3
        testA = eval_A[keep_cols].sort_values(["date", "symbol"]).reset_index(drop=True)
        testB = (
            dfB[dfB["date"] >= eval_start][keep_cols]
            .sort_values(["date", "symbol"])
            .reset_index(drop=True)
        )
        if len(testA) != len(testB):
            print(
                f"[FATAL] {tag} 评估行数 A={len(testA)} B={len(testB)} → abort",
                flush=True,
            )
            return 3
        ...
        stdA = testA[cols].std()
        shift_idx = float((delta_std / stdA).replace([np.inf, -np.inf], np.nan).mean())
        ...
        tA10, tB10 = (
            resA["topn"]["10d_n10"]["mean_ret"],
            resB["topn"]["10d_n10"]["mean_ret"],
        )
        print(
    ...
    # ---- 判定 ----
    verdict = {
        "note": "主张=训练一致(全板块)特征 ≥ top-N 特征; 每 bundle 看 wIC + top10 双指标"
    }
    for tag, r in results.items():
        a10, b10 = (
            r["mode_A_full_board"]["topn"]["10d_n10"]["mean_ret"],
            r["mode_B_topn"]["topn"]["10d_n10"]["mean_ret"],
        )
        verdict[tag] = {
            "A_wins_ic": r["mode_A_full_board"]["weighted_ic"]
            >= r["mode_B_topn"]["weighted_ic"],
    ...
    out_dir = (
        ...
        / f"feat_serve_a_vs_b_{time.strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    for tag, v in verdict.items():
        if tag == "note":
            continue
        print(
            f"[verdict {tag}] ic={'A' if v['A_wins_ic'] else 'B'} "
            f"top10={'A' if v['A_wins_top10'] else 'B'} "
            f"Δwic={v['delta_wic']:+.4f} Δtop10={v['delta_top10_ret']:+.3f}",
            flush=True,
        )
    return 0
