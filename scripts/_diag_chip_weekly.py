# -*- coding: utf-8 -*-
"""_diag_chip_weekly.py — 筹码形态列周/双周/月频变换 **个股 time-series IC** 测试.

用户修正: 筹码形态是**个股自身的时间序列状态变量**, 跨股票横截面比较混入
个股结构性差异, 应采用 **per-stock time-series IC**: 对每只股票, 将该股自己
历史上筹码特征序列与未来收益序列做 rank 相关, 再对全样本股票取平均.

对每个 chip 列 × 各变换(windows 5/10/20)计算个股时序 IC, 在 HS300×1年窗口.
...
# 对照: 纯价格水平列 — 检验成本线列(成本线列本身是价格)的 level TSIC
# 是否只是个股价格均值回归, 而非筹码结构信息.
...
def per_stock_ts_ic(work, feats, labels, min_obs=20):
    """个股时序 IC: 对每只股票, 用该股**自身**历史序列, 计算特征与 T+2/T+3/T+5
    涨幅(label_pm_{2,3,5}d_net)的 rank 相关(Spearman), 再对全样本股票取平均.
...
def run(df, label, out):
    ...
    for col in CHIP_COLS:
        if col not in df.columns:
            continue
        _emit(df, col, out)
    out.append("--- 对照: 纯价格水平列 (level TSIC 是否=价格均值回归) ---")
