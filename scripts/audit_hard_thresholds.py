...
def _classify(name: str, value: float, filepath: str) -> str:
    ...
    if any(kw in lower for kw in ("stop", "fuse", "loss_limit", "amplitude",
                                   "vol_surge", "daily_loss", "time_stop",
                                   "hard_stop", "drawdown", "halt")):
...
def audit_all(root: str = ".") -> dict:
    ...
    for dirpath, _, filenames in os.walk(root):
        # 跳过非代码目录
        for skip in ("__pycache__", "node_modules", ".git", ".claude", "venv",
                      "skills", ".tox", "site-packages"):
            if skip in dirpath:
                break
        else:
            for fn in filenames:
...
def ci_check(report: dict) -> bool:
    ...
    if violations:
        ...
        for v in violations:
            logger.error("  %s:%d %s=%.4f (%s)",
                         v["file"], v["line"], v["name"], v["value"], v["classification"])
        return False
...
def main():
    ...
    logger.info("审计完成: %d 文件, %d 阈值, 输出 → %s",
                report["scanned_files"], report["total_findings"], out_path)
