...
def get_all_sw_codes():
    ...
    for _, row in df.iterrows():
        for level, col in [("L1", "sw_l1_code"), ("L2", "sw_l2_code"), ("L3", "sw_l3_code")]:
            c = str(row.get(col, "")).strip()
            if c and c != "nan":
                codes[level].add(c)
    logger.info("SW codes: L1=%d, L2=%d, L3=%d", len(codes["L1"]), len(codes["L2"]), len(codes["L3"]))
    return codes
...
def main():
    ...
    parser.add_argument("--incremental", action="store_true", help="Only fetch dates after existing max")
    ...
    logger.info("Fetching %d indices from %s to %s ...", len(all_codes), args.start, args.end)
    ...
    for i, (level, code) in enumerate(all_codes):
        ...
        if (i + 1) % 50 == 0:
            logger.info("  Progress: %d/%d indices, %d rows so far", i + 1, len(all_codes), sum(len(f) for f in frames))
    ...
    logger.info("Fetched: %d rows, %d indices", len(new_df), new_df["ts_code"].nunique())
    ...
    if existing_df is not None and len(existing_df) > 0:
        ...
        logger.info("Combined: %d rows (existing %d + new %d)", len(combined), len(existing_df), len(new_df))
        out_df = combined
    else:
        out_df = new_df.sort_values(["level", "ts_code", "trade_date"]).reset_index(drop=True)

    # ── Save ──
    out_df.to_parquet(OUTPUT, index=False)
    logger.info("Saved to %s: %d rows, %d indices", OUTPUT, len(out_df), out_df["ts_code"].nunique())

    # ── Audit ──
    for level in ["L1", "L2", "L3"]:
        sub = out_df[out_df["level"] == level]
        logger.info("  %s: %d indices, %d dates (%s ~ %s)",
                     level, sub["ts_code"].nunique(), sub["trade_date"].nunique(),
