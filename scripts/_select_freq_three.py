...
def main() -> None:
    ...
    if board_df is None or len(board_df) == 0:
        ...
        return
    print(
        f"[{board}] 清洗后 rows={len(board_df):,} stocks={board_df['symbol'].nunique():,}",
        flush=True,
    )

    features = FeatureEngineV35()
    use_xrank = board != "main"
    df = prepare_board_frame(board_df, features, None, cross_sectional_rank=use_xrank)
    del board_df
    gc.collect()
    ...
    out.append("覆盖率: " + "  ".join(f"{k}={len(v):,}" for k, v in buckets.items()))
    ...
    for freq in FREQ_ORDER:
        ...
        confirmed_hit = [
            f
            for f in feats
            if f.split("_brute_")[0] in FREQ_ASSIGNMENT
