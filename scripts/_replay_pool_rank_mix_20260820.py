...
输出: BACKTEST_RESULT_DIR/pool_rank_mix_20260820_<ts>/result.json (WORM)
...
def project_eval(
    ...
    keep = (
        ...
        + ["date", "symbol"]
        + [c for c in ("liquidity_score", "amount") if c in eval_df.columns]
    )
...
def cut_top_n(df: pd.DataFrame, key: str, n: int) -> pd.DataFrame:
    """每 date 按 key 降序取前 n (池成员资格)."""
    rk = df.groupby("date")[key].rank(ascending=False, method="first")
    return df[rk <= n].copy()
...
def main() -> int:
    ...
    # (d) 控制: 全谱特征 + 流动性前 800 → E6 (隔离"特征截面变大"效应)
    pool_d = e6_cut(cut_top_n(eval_full, "liquidity_score", n_pool), e6)
    ...
    # (b) 混合选池: blend 前 800 → E6 → pred 排名
    r_pred_full = eval_full.groupby("date")["_pred_10d"].rank(pct=True)
    for w in _weights:
        ef = eval_full.assign(
            _blend=blend_score(
        ...
        pool_b = e6_cut(cut_top_n(ef, "_blend", n_pool), e6)
    ...
    out_dir = Path(BACKTEST_RESULT_DIR) / (
        f"pool_rank_mix_20260820_{_eval_days}d_{time.strftime('%Y%m%d_%H%M%S')}"
    )
