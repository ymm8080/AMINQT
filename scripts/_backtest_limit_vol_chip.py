...
COST_RT = (
    ...
    + (_bt_cfg.get("slippage_buy_bp", 10) + _bt_cfg.get("slippage_sell_moo_bp", 10))
    / 10000
)
...
def _median_stat(ser):
    ...
    return {
        "n": len(ser),
        "mean_pct": round(float(ser.mean()) * 100, 3),
    ...
    for horizon in ["label_pm_3d", "label_pm_5d"]:
        ...
        for name, g in rows:
            ...
            if s is None:
                ...
                continue
            print(
                f"    {name:<14s} {s['n']:>7d} {s['mean_pct']:>+7.2f} {s['hit_pct']:>7.1f}"
            )
    out[board]["h4_buckets"] = {
        h: {name: _median_stat(g[h]) for name, g in rows}
...
def cond_density(df, board, out):
    ...
    pb["d_gini_hi"] = pb["chip_gini"] >= pb.groupby("date")["chip_gini"].transform(
        "median"
    )
    pb["d_winner_hi"] = pb["winner_ratio"] >= pb.groupby("date")[
        "winner_ratio"
    ].transform("median")
    pb["d_price"] = pd.cut(
        pb["close"], [0, 10, 30, np.inf], labels=["低价<10", "中价10-30", "高价>30"]
    )
    conds = [
    ...
    for name, mask in conds:
        ...
        print(
            f"    {name:<22s} {s['n']:>7d} {s['mean_pct']:>+7.2f} {s['hit_pct']:>7.1f} {net:>+7.2f}"
        )
    out[board]["cond_density"] = [
        {"cond": n, **s, "net_pct": round(net, 3)} for n, s, net in rows
    ]
    ...
    for pl in ["低价<10", "中价10-30", "高价>30"]:
        ...
        print(
            f"    {pl:<14s} {s['n']:>7d} {s['mean_pct']:>+7.2f} {s['hit_pct']:>7.1f} {net:>+7.2f}"
        )
...
def model_density(df, board, out):
    ...
    for tag, feats in [("base", R.MODEL_FEATS), ("+vol_chip_price", LIMIT_MODEL_FEATS)]:
        ...
        out[board].setdefault("model_density", {})[f"cont_{tag}"] = {
            "auc": round(auc, 4),
            "n_te": len(te),
        }
    ...
    for tag, feats in [("base", R.PULL_FEATS), ("+vol_chip_price", PULL_MODEL_FEATS)]:
        ...
        print(
            f"    回调续涨 RankIC {tag:<16s} = {r_ic:+.4f} | top20% {top['label_pm_5d'].mean() * 100:+.2f}% (基线 {te['label_pm_5d'].mean() * 100:+.2f}%)"
        )
...
def backtest(df, board, out):
    ...
    def _run(mask, label):
        ...
        print(
            f"    {label:<24s} n={len(rows):>6d} 组合均值{float(daily.mean()) * 100:>+6.2f}% "
        ...
        return {
            ...
            "trade_mean_pct": per_trade["mean_pct"],
            "trade_hit_pct": per_trade["hit_pct"],
        }
    ...
    print(
        f"    {'全池基线(逐日等权)':<24s} 组合均值{float(bdaily.mean()) * 100:>+6.2f}% "
        f"Sharpe{bsharpe:>6.2f} 交易均值{b_per['mean_pct']:>+6.2f}% "
        f"交易上涨率{b_per['hit_pct']:>5.1f}%"
    )
    res = {
        "base": {
            "port_mean_pct": round(float(bdaily.mean()) * 100, 3),
            "port_sharpe": round(bsharpe, 3),
            "trade_mean_pct": b_per["mean_pct"],
            "trade_hit_pct": b_per["hit_pct"],
        },
    ...
    res["strategies"]["回调满月+筹码集中"] = _run(
        pull_win & gini_hi, "回调满月+筹码集中"
    )
    res["strategies"]["回调满月+缩量+筹码集中"] = _run(
        pull_win & shrink & gini_hi, "回调满月+缩量+筹码集中"
    )
    # 价格分档
    for pl, lo, hi in [
        ("低价<10", 0, 10),
        ...
        res["strategies"][f"回调满月+{pl}"] = _run(
            pull_win & df["close"].between(lo, hi, inclusive="left"), f"回调满月+{pl}"
        )
