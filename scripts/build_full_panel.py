...
def load_panel(path):
    ...
    logger.info("Loaded panel: %d rows, %d symbols, %d columns, dates %s to %s",
...
def fix_industry(panel, refresh=False):
    ...
    try:
        ind_map, name_map = load_or_fetch_meta(cache_dir="data/processed", refresh=refresh)
        if ind_map:
            before = (panel["industry"] == "UNKNOWN").sum()
            panel["industry"] = panel["symbol"].map(ind_map).fillna("UNKNOWN")
            after = (panel["industry"] == "UNKNOWN").sum()
            logger.info("fix_industry: %d UNKNOWN -> %d UNKNOWN (fixed %d rows, %d industries)",
                        before, after, before - after, panel["industry"].nunique())
        else:
...
def merge_northbound(panel, refresh=False):
    ...
    try:
        df = supply.fetch_northbound(
            ...
            end_date=panel["date"].max().strftime("%Y%m%d"), refresh=refresh)
        if len(df) == 0:
...
def merge_margin(panel, refresh=False):
    ...
    try:
        ...
        logger.info("margin: %d rows, %d symbols, +%d cols -> panel",
                    len(df), df["symbol"].nunique(), len(panel.columns) - before)
    except Exception as exc:
...
def _merge_fina_panel(panel, df):
    ...
    panel_p = pd.merge_asof(panel_p, f, left_on="date", right_on="announce_date",
...
def merge_lhb(panel, refresh=False):
    ...
    try:
        ...
        if len(df) > 0 and "date" in df.columns and "symbol" in df.columns:
            ...
            before = len(panel.columns)
            panel = panel.merge(df[merge_cols + avail], on=merge_cols, how="left")
            logger.info("lhb: %d rows, %d symbols, +%d cols -> panel",
                        len(df), df["symbol"].nunique(), len(panel.columns) - before)
            return panel
        else:
    ...
    try:
        ...
        logger.info("lhb: read cache %s: %d rows, %d cols",
                    os.path.basename(main_cache), len(raw), len(raw.columns))
                ...
                try:
                    ...
                    if sample is not None:
                        ...
                        if len(s) == 8 and s.isdigit():
                            raw["date"] = pd.to_datetime(raw[col], format="%Y%m%d", errors="coerce")
                            break
        ...
        if "date" in raw.columns and "symbol" in raw.columns:
            ...
            avail = [c for c in raw.columns if c not in exclude and not str(c).startswith("_")]
            before = len(panel.columns)
            panel = panel.merge(raw[merge_cols + avail], on=merge_cols, how="left")
            logger.info("lhb: direct merge: +%d cols -> panel", len(panel.columns) - before)
        else:
...
def merge_sector_index(panel, refresh=False):
    ...
    try:
        ...
        if panel["industry"].nunique() <= 1 and panel["industry"].iloc[0] == "UNKNOWN":
            ...
            return panel
        name_to_code = {name: code for code, name in df[["index_code", "index_name"]].drop_duplicates().itertuples(index=False)}
        ind_map = {}
        ...
        sw_data = df.rename(columns={"ret_pct": "sw_ret_1d", "close": "sw_index_close", "volume": "sw_index_vol"})
        avail = [c for c in sw_data.columns if c not in ("index_code", "date")]
        before = len(panel.columns)
        panel = panel.merge(sw_data[["index_name", "date"] + avail],
                            left_on=["_sw_name", "date"], right_on=["index_name", "date"], how="left")
        panel = panel.drop(columns=["_sw_name", "index_name"], errors="ignore")
        logger.info("sector_index: %d rows, +%d cols -> panel", len(df), len(panel.columns) - before)
    except Exception as exc:
...
def merge_holdernumber(panel, refresh=False):
    ...
    if "announce_date" in df.columns:
        hn_cols = [c for c in df.columns if c not in ("symbol", "date", "_ts_code", "announce_date")]
...
def report_coverage(panel, alt_cols):
    ...
    logger.info("=" * 70)
    logger.info("Panel: %d rows, %d symbols, %d columns", len(panel), panel["symbol"].nunique(), len(panel.columns))
    dims = {
        "northbound": [c for c in alt_cols if c.startswith("north_")],
        "margin": [c for c in alt_cols if c.startswith("margin_") or c.startswith("short_")],
        "fina_indicator": [c for c in alt_cols if c in (
            "roe", "roe_deducted", "roa", "net_margin", "gross_margin",
        ...
        "holdernumber": [c for c in alt_cols if c in ("holder_count", "avg_shares_per_holder")],
    ...
    for dim, cols in dims.items():
        ...
        logger.info("  %-20s: %2d cols, %5.1f%% NaN, %5.1f%% rows have some data",
                    dim, len(cols), cell_nan, row_pct)
...
def main():
    ...
    logger.info("\n--- Summary ---")
    logger.info("Final panel: %d rows, %d symbols, %d columns", len(panel), panel["symbol"].nunique(), len(panel.columns))
