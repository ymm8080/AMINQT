...
def main() -> int:
    ...
    print(
        f"[rows] {len(df):,} | dates {df['date'].nunique()} | boards {sorted(df['board'].unique())}",
        flush=True,
    )
    ...
    for board in ("main", "dual"):
        ...
        for seg, mask in (
            ("rk1_5", ranked["rk"] <= 5),
            ("rk6_15", (ranked["rk"] > 5) & (ranked["rk"] <= 15)),
        ):
        ...
        for kname, (kcol, asc) in RANK_KEYS.items():
            ...
            for depth in (5, 10):
                for wname, wdays in WINDOWS.items():
                    ...
                    topn = (
                        w.sort_values(["date", kcol], ascending=[True, asc])
                        .groupby("date", sort=False)
                        .head(depth)
                    )
                    s = _stats(topn)
                    rows.append(
                        {"board": board, "variant": f"{kname}_top{depth}_{wname}", **s}
                    )
        # 4) 首 20 评估日 vs 其余 (base_rate 预热旁证)
        ds = sorted(b["date"].unique())
        for seg, sub in (
            ("first20d", b[b["date"].isin(ds[:20])]),
            ("rest", b[b["date"].isin(ds[20:])]),
        ):
            top5 = (
    ...
    print(
        f"\n{'板':<4}{'变体':<18}{'日':>4}{'票':>5}{'票/日':>6} {'命中':>7}{'实得':>8}{'中位':>8}{'≥5%':>7}{'≥10%':>7}",
        flush=True,
    )
    for r in rows:
        ...
        print(
            f"{r['board']:<4}{r['variant']:<18}{nd:>4}{pk:>5}{ap:>6.1f} "
            f"{_sf(r['hit'], '{:.1%}')}{_sf(r['mean'], '{:+.2%}')}{_sf(r['med'], '{:+.2%}')}"
            f"{_sf(r['ge5'], '{:.1%}')}{_sf(r['ge10'], '{:.1%}')}",
            flush=True,
        )
