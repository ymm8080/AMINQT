...
TOP-5/TOP-10 质量。预测复用 data/_diag_replay_wf_pred_<board>.parquet 检查点 (免重训,
与 250d 生产口径复验 _diag_parallel_gbm_prod_replay.py 同源)。
排名键: mag(关闸)=生产闸关闭基线; mag=纯 pred_mag_10d (生产现状); prob=纯 pred_prob;
blend=pred_mag_10d × pred_prob。
指标: 250 已实现交易日 label_pm_10d_net 实得/命中/≥5%/≥10% + 4 子窗, TOP-5 与 TOP-10。
用法: python scripts/_diag_prob_rank_ab.py
...
def main() -> int:
    ...
    for board in ("main", "dual"):
        ...
        for df in (pool, gated):
            ...
            df["rank_prob"] = df["pred_prob"]
            df["rank_blend"] = df["pred_mag_10d"] * df["pred_prob"]

        combos = [
            ("mag(关闸)", pool, "rank_mag"),
            ("mag", gated, "rank_mag"),
            ("prob", gated, "rank_prob"),
            ("mag×prob", gated, "rank_blend"),
        ]
        ...
        json.dumps(
            {
                ...
                "t3_landed": T3_LANDED,
                "margin": MARGIN,
                "note": "排名键 A/B: mag=纯 pred_mag_10d (生产现状); prob=纯 pred_prob; "
                "blend=mag×prob; 生产顺序 t3 门 → 概率闸(margin) → 排名; "
                "预测复用 _diag_replay_wf_pred_<board>.parquet 检查点",
