"""_diag_icdedup_topn_ab.py — MAIN 特征选择 IC 排序 vs 方差排序 TopN 金标准验证.

来自 _diag_tiered_topn_ab.py (2026-08-08 tier 门测) 的同一协议, 唯一差别 = 特征列表:

  BASE = FeatureSelector 默认 (dedup_key="variance") → 组内按方差降序 greedy keep
  D2   = 同 selector, dedup_key="ic" → 组内按 |IC| 降序 greedy keep (选预测力最强的)

IC 排序只在 OOS 起点 (ic_cut_date=oos_start) 之前的行上评 IC —— 诚实, 防 OOS label
泄漏进选择 (tier 门测是方差排序, 无此问题; IC 排序必须防 look-ahead).

流水线 (与 tier 门测完全一致): CleaningPipeline.run_train(board="main") →
prepare_board_frame (V35 全窗 build → 标签 → 停牌/近端掩码) → 裁剪到末
TRAIN_WINDOW_DAYS 交易日 → select_features (选择 + event scope screens + brute
后注入) × 2 (BASE/D2) → risk_filter → OOS 边界切分 (训练严格早于 OOS) →
model_params("main","reg"/"cls") LGBM (训练尾部 es_days 早停) →
度量: Top5/10/20 命中率+均值 (金标准), dir_acc/AUC/校准, OOS 加权 IC, OOS 子窗口稳定性.

Gate (全部满足才允许落地 dedup_key="ic", 否则保持方差排序并交用户定夺):
  主判据: D2 Top10 hit/mean >= BASE (允许 -0.5pp 噪声), Top5/Top20 同向不恶化
  副判据: dir_acc / AUC / 校准误差不劣化 >1pp; OOS weighted_IC 不劣化 >0.005
  稳定性: OOS 按时间三等分, D2>=BASE (容差) 的方向须 >= 2/3 子窗稳定

产出 (WORM): data/_diag_icdedup_topn_ab_<ts>.json + 控制台对比表 + 明确 GATE 结论.

用法: python scripts/_diag_icdedup_topn_ab.py [--window-days 380] [--oos-days 120]
      [--topn-k 10] [--ic-label label_pm_10d_net]

注: 金标准 TopN 按 --topn-k 视界评估 (默认 10 = 生产资金窗口 pred_ret_10d).
"""
...
logging.disable(logging.ERROR)  # 压 INFO/WARNING, 保留 ERROR 级回退警告

ES_DAYS = 40  # 训练尾部早停验证段 (与生产 es 段一致)
PREP_DAYS = 750  # 特征构建窗 = 末 ~750 交易日 (3 年, 与生产 assemble_panel(years=3) 一致)
HORIZONS = (2, 3, 5, 10)  # 全视界, 含 10d 生产资金窗口
TOPKS = (5, 10, 20)
# 权重对齐生产 LABEL_WEIGHTS 去零项 (用户 2026-08-08: 弃 2d, 3d 最小化 vs 5d, 10d 最高)
W_IC = {3: 0.10, 5: 0.40, 10: 0.50}
HIT_TOL = 0.005  # -0.5pp 噪声容差 (命中率/均值)
WIC_TOL = 0.005  # OOS weighted_IC 劣化容差
PP_TOL = 0.01  # dir_acc/AUC/校准 1pp 劣化容差
TMP_REGISTRY = os.path.join(ROOT, "data", "_diag_icdedup_registry")
...
def _fit_model(rows, Xcols, kind, k, es_days=ES_DAYS):
    ...
    dts = sorted(data["date"].unique())
    n_dates = len(dts)
    # 小窗保护: 早停段最多占总日期, 保证 fit 与 es 均非空 (正常窗 es_n=es_days).
    es_n = min(es_days, max(1, n_dates - 2))
    es_cut = dts[-es_n]
...
def _topn_eval(te, Xcols, cache, tag, k=10):
    """每日按 pred_ret_{k}d 取 TOP5/10/20 → 命中率 + 均值 (金标准口径)."""
    y_col = f"label_pm_{k}d_net"
    reg = cache[(tag, k, "reg")]
    cls = cache[(tag, k, "cls")]
    data = te.dropna(subset=[y_col]).copy()
...
def _subwindow_topn(te, Xcols, cache, tag, k=10):
...
def _run_arm(df, tag, oos_start, arm_cfg):
    print(f"[3/5] 选择特征 {tag} (cfg={arm_cfg}) ...", flush=True)
    cfg = copy.deepcopy(FeatureSelector.DEFAULT_CONFIG)
    if arm_cfg:
        cfg["main"].update(arm_cfg)
...
def _print_table(b, t):
    print("\n" + "=" * 96)
    print("MAIN 选择 IC 排序 A/B — OOS 对比 (BASE=方差排序 vs D2=IC 排序)")
    print("=" * 96)
    print(f"{'metric':<14s} {'BASE':>12s} {'D2':>12s} {'Δ':>10s}  {'判据'}")
    print("-" * 96)
    ...
    for key, lab in (("top5_ret", "TOP5 hit"), ("top10_ret", "TOP10 hit"), ("top20_ret", "TOP20 hit")):
        bh, th = b["topn"][key]["hit"], t["topn"][key]["hit"]
    ...
    for seg in ("seg1", "seg2", "seg3"):
        ...
        if np.isnan(bv) or np.isnan(tv):
            ...
            continue
        print(
            f"  {seg:<6s} BASE={bv * 100:>7.2f}% D2={tv * 100:>7.2f}% "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MAIN 选择 IC 排序 TopN A/B 验证")
    parser.add_argument("--window-days", type=int, default=380)
    parser.add_argument("--oos-days", type=int, default=120)
    parser.add_argument("--topn-k", type=int, default=10, help="金标准 TopN 视界 (生产资金窗口=10)")
    parser.add_argument("--ic-label", default="label_pm_10d_net")
    parser.add_argument(
        "--board-cache",
        default=None,
        help="3y board frame 缓存 parquet (存在则跳过 V35 build, 否则 build 后保存). "
        "两次运行面板相同 → build 确定性, 可安全复用.",
    )
    ...
    assert args.topn_k in HORIZONS, f"--topn-k {args.topn_k} 必须在 HORIZONS={HORIZONS} 内"
    ...
    if args.board_cache and os.path.exists(args.board_cache):
        ...
        print(f"  [cache] 载入 board frame rows={len(df)} cols={len(df.columns)} from {args.board_cache}", flush=True)
        del main_df
        gc.collect()
    else:
        ...
        if args.board_cache:
            ...
            print(f"  [cache] 已保存 board frame ({len(df)} rows) 到 {args.board_cache}", flush=True)
        del main_df
        gc.collect()
    _downcast(df)
    ...
    d2_cfg = {
        ...
        "ic_cut_date": str(oos_start.date()),
    }
    for tag, arm_cfg in (("BASE", None), ("D2", d2_cfg)):
        picked, tr, te = _run_arm(df, tag, oos_start, arm_cfg)
        arms[tag] = {"picked": picked, "tr": tr, "te": te}

    print(f"[4/5] 训练 LGBM ({'/'.join(map(str, HORIZONS))}d × reg/cls) ...", flush=True)
    cache = {}
    for tag in ("BASE", "D2"):
        for k in HORIZONS:
            for kind in ("reg", "cls"):
    ...
    for tag in ("BASE", "D2"):
        ...
        topn = _topn_eval(te, picked, cache, tag, k=args.topn_k)
        subwin = _subwindow_topn(te, picked, cache, tag, k=args.topn_k)
        report[tag] = {
            "n_picked": len(picked),
            "oos_ics": ics,
            "oos_wic": wic,
    ...
    _print_table(report["BASE"], report["D2"])
    gate = _apply_gate(report["BASE"], report["D2"])

    print("=" * 96)
    print("GATE 判定 (全部满足才允许落地 dedup_key='ic'):")
    ...
    print(f"  副判据 OOS weighted_IC 不劣化>0.005:   {'PASS' if gate['wic_ok'] else 'FAIL'}  (Δ={report['D2']['oos_wic'] - report['BASE']['oos_wic']:+.4f})")
    print(f"  稳定性 子窗 D2>=BASE >= 2/3:            {'PASS' if gate['stable_ok'] else 'FAIL'}  ({gate['stable_windows']}/3)")
    verdict = "PASS → 可落地 dedup_key='ic'" if gate["pass"] else "FAIL → 保持方差排序, 交用户定夺"
    print(f"  ==> {'=' * 4} {verdict} {'=' * 4}")
    print("=" * 96)

    ts = datetime.now().strftime("%Y%m%dT%H%M%S")
    out_path = os.path.join(ROOT, "data", f"_diag_icdedup_topn_ab_{ts}.json")
    payload = {
        "created": ts,
