...
def _hit_stats(sub: pd.DataFrame, sl: pd.DataFrame) -> dict:
    out: dict = {}
    for h, lab in LABELS:
        v = sub[["symbol", "date", lab]].merge(sl[["symbol", "date"]], on=["symbol", "date"], how="inner")[lab].dropna()
        out[h] = {
            "n": int(len(v)),
...
def main() -> int:
    ...
    print(f"[data] merged {len(m):,} 票 / {m['date'].nunique()} 决策日 ({time.time() - t0:.0f}s)", flush=True)
    ...
    for vname in ("A_基准", "B_剔dual低价", "C_剔main高价", "D_B加C"):
        ...
        for board in ("main", "dual"):
            ...
            for wname, wdays in WINDOWS.items():
                ...
                sl_w = vmb[vmb["date"].isin(set(sub_w["date"]))][["date", "symbol"]]
                for cut, n in (("T-5", 5), ("T-10", 10)):
                    sl_c = vmb[
                        (vmb["rk"] <= n) & vmb["date"].isin(set(sub_w["date"]))
                    ][["date", "symbol"]]
                    ph = _merged_dual(sub_w, sl_c, crit[board])
                    hs = _hit_stats(sub_w, sl_c)
                    for h, r in ph.items():
                        rows.append({
                            "variant": vname, "board": board, "window": wname, "cut": cut,
                            "horizon": h,
                            "picks": int(len(sl_c)),
    ...
    print(f"\n{'变体':<12}{'板':<4}{'窗':<6}{'切':<5}{'h':>3} | "
          f"{'票':>5}{'胜率':>7}{'幅度':>8}{'ok':>4}{'Δ胜率':>7} | {'命中':>7}{'实得':>8}", flush=True)
    for _, r in df.iterrows():
        print(f"{r['variant']:<12}{r['board']:<4}{r['window']:<6}{r['cut']:<5}{r['horizon']:>3} | "
              f"{int(r['picks']):>5}{r['winrate']:>7.1%}{r['mag']:>+8.2%}{str(r['ok']):>4}"
              f"{r['delta_wr'] if r['delta_wr'] is None else r['delta_wr']:>7.2%} | "
              f"{r['hit']:>7.1%}{r['mean']:>+8.2%}", flush=True)
        ...
        for board in ("main", "dual"):
            for wname in ("126d", "63d"):
                for cut, h in (("T-5", "10d"), ("T-10", "10d")):
                    b = base[(base.board == board) & (base.window == wname) & (base.cut == cut) & (base.horizon == h)].iloc[0]
                    v = vd[(vd.board == board) & (vd.window == wname) & (vd.cut == cut) & (vd.horizon == h)].iloc[0]
                    print(
                        f"{vname} {board} {wname} {cut}: "
                        f"命中 {b['hit']:.1%}→{v['hit']:.1%} ({v['hit']-b['hit']:+.1%}) | "
                        f"实得 {b['mean']:+.2%}→{v['mean']:+.2%} ({v['mean']-b['mean']:+.2%}) | "
                        f"票 {int(b['picks'])}→{int(v['picks'])}", flush=True,
                    )
    print(f"\n[saved] {out_dir}/price_band_ab_{ts}.json ({time.time() - t0:.0f}s)", flush=True)
    return 0
