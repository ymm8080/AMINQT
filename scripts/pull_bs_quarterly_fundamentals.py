...
def pull_fundamentals_batch(symbols: list[str]) -> pd.DataFrame | None:
    ...
    val_cols = [
        c
        for c in FUNDAMENTAL_COLS["balance"] + FUNDAMENTAL_COLS["cashflow"]
...
def forward_fill_to_daily(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    logger.info("Forward-filling quarterly data to daily...")
    ...
    logger.info(
        f"Forward-filled: {len(result)} rows, {result['symbol'].nunique()} stocks"
    )
    return result
...
def main():
    ...
    if not args.skip_pull:
        ...
        for batch_start in range(0, len(todo), BATCH_SIZE):
            ...
            try:
                ...
                logger.warning(
                    f"Batch failed for {batch[0]}..{batch[-1]}: {e}, "
            ...
            logger.info(
                f"Fundamentals [{n_done}/{len(todo)}] {pct:.0f}% "
        ...
        logger.info(
            f"Fundamentals pull done: {n_ok}/{len(todo)} in {elapsed / 3600:.1f}hr"
        )

        if frames:
            raw = pd.concat(frames, ignore_index=True)
            # 排序并去重
            raw = raw.sort_values(["symbol", "statDate", "pubDate"]).reset_index(
                drop=True
            )
            raw = raw.drop_duplicates(subset=["symbol", "statDate"], keep="last")
            raw.to_parquet(OUTPUT_RAW, index=False)
            logger.info(
                f"Saved {OUTPUT_RAW}: {len(raw)} rows, {raw['symbol'].nunique()} stocks"
            )
        else:
        ...
        logger.info(
            f"Loaded raw fundamentals: {len(raw)} rows, "
    ...
    if len(ff) > 0:
        ...
        logger.info(
            f"Saved {OUTPUT_FF}: {len(ff)} rows, {ff['symbol'].nunique()} stocks"
        )
        # 打印覆盖率
        for col in [
            c
