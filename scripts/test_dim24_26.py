...
panel['is_suspended'] = False; panel['is_st'] = False
...
if os.path.exists(mg_path):
    ...
    mg_cols = [c for c in mg.columns if c not in ['symbol','date'] and not c.startswith('_')]
    panel = panel.merge(mg[['symbol','date']+mg_cols], on=['symbol','date'], how='left')
    n_dates = panel.dropna(subset=['margin_balance'])['date'].nunique()
    print(f"Margin merged: non-NaN={panel['margin_balance'].notna().sum()}/{len(panel)}, dates={n_dates}")

# ── Merge northbound ──
nb_path = os.path.join(CACHE, "northbound", "20240102_20260727.parquet")
if os.path.exists(nb_path):
    nb = pd.read_parquet(nb_path)
    nb_cols = [c for c in nb.columns if c not in ['symbol','date'] and not c.startswith('_')]
...
if os.path.exists(lhb_path):
    ...
    lhb_cols = [c for c in lhb.columns if c not in ['symbol','date'] and not c.startswith('_')]
    panel = panel.merge(lhb[['symbol','date']+lhb_cols], on=['symbol','date'], how='left')
...
print(f"{'='*70}")
for col in ['margin_balance','short_balance','margin_buy_amt',
            'north_net_buy_sh','north_net_buy_sz',
            'lhb_net_buy','lhb_institutional_net_buy','lhb_institutional_count']:
    if col in panel.columns:
        n_na = panel[col].isna().sum(); n_tot = len(panel)
        if n_na < n_tot:
            vmin, vmax = panel[col].min(), panel[col].max()
        else:
            vmin = vmax = 0
        print(f"  {col:35s}: NaN {n_na:>8d}/{n_tot} ({n_na/n_tot*100:5.1f}%),  range=[{vmin:>15.4f}, {vmax:>15.4f}]")

# Date range coverage
for label, col in [('Margin','margin_balance'),('Northbound','north_net_buy_sh'),('LHB','lhb_net_buy')]:
    if col in panel.columns:
        dts = panel.dropna(subset=[col])['date'].unique()
        if len(dts):
            print(f"  {label} coverage: {len(dts)} dates, {pd.Timestamp(min(dts)).strftime('%Y-%m-%d')} ~ {pd.Timestamp(max(dts)).strftime('%Y-%m-%d')}")
...
dims = {
    'DIM24_MARGIN': [
        'margin_balance_chg_1d', 'margin_balance_chg_5d', 'short_balance_ratio',
        'margin_buy_ratio', 'margin_balance_ma20_dev', 'margin_balance_yoy',
    ...
    'DIM26_LHB': [
        'lhb_inst_net_buy_5d', 'lhb_inst_net_buy_20d', 'lhb_inst_count_5d',
        'lhb_inst_buy_ratio', 'lhb_abnormal_score',
    ],
}
...
for dim, feats in dims.items():
    ...
    for f in feats:
        ...
        ic_20d, hit_20d, _ = rank_ic(panel, f, label_20d) if label_20d else (0.0, 0.0, 0)

        sig = '***STRONG' if ic_1d >= 0.03 else ('**OK' if ic_1d >= 0.02 else ('*weak' if ic_1d >= 0.01 else ' -'))
        print(f"  {f:30s}: |IC|_1d={ic_1d:.5f} hit={hit_1d:.1%}  "
