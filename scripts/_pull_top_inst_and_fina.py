...
def pull_top_inst():
    ...
    for i, ds in enumerate(dates):
        ...
        if not t.empty:
            keep = [x for x in ["symbol", "ts_code", "trade_date", "exalter", "buy", "sell"]
                    if x in t.columns]
            t2 = t[keep].copy()
...
def pull_fina():
    ...
    for i, sym in enumerate(todo_fina):
        ...
        f = _fetch(pro, pro.fina_indicator, f"fina {sym}",
                   ts_code=code, start_date="20220101", end_date="20260815",
