...
if os.path.exists(hn_path):
    ...
    logger.info(
        "holdernumber: %d rows from cache, cov=%.1f%%",
        len(hn),
        (1 - df["holder_count"].isna().mean()) * 100,
    )
...
logger.info(
    "margin: %d days, cov=%.1f%%, %.1fs",
    len(mg_frames),
    (1 - df["margin_balance"].isna().mean()) * 100
    ...
    try:
        ...
        if raw is not None and len(raw) > 0:
            ...
            if "net_mf_amount" in raw.columns:
                raw["net_mf_amount"] = pd.to_numeric(
...
with open(
    f"data/factor_registry/alt_eval_fast_{pd.Timestamp.now():%Y%m%d_%H%M}.json", "w"
) as fh:
