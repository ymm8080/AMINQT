...
def part_a() -> None:
    need = [
        "symbol", "date", "close_hfq", "high_hfq", "low_hfq", "volume", "turnover_rate",
        "chip_entropy", "chip_gini",
        "vol_ratio_ma5", "upper_shadow_pct", "lower_shadow_pct",
        "吸筹_density_10d", "洗盘_density_5d",
        "close_position", "label_pm_10d_net",
    ]
    t = pd.read_parquet(CKPT, columns=need)
    t = t[t["symbol"] == SYMBOL].copy()
    t = t.sort_values("date")
    t = prepare_adx(t)  # vol_ratio / rps_60 / pct_70_con / adx / ma_tightness / sharpe_20 / pv_corr_5
    t["dt"] = pd.to_datetime(t["date"]).dt.strftime("%m-%d")
    t["ent_chg20"] = t["chip_entropy"].pct_change(20)
    g = t.groupby("symbol")
    t["vol_ma20"] = t.groupby("symbol")["volume"].transform(
        lambda v: v.where(v > 0).rolling(20, min_periods=10).mean()
    ) if "volume" in t.columns else np.nan
    show = t.tail(12)[
        ["dt", "close_hfq", "chip_entropy", "ent_chg20", "chip_gini",
         "vol_ratio_ma5", "upper_shadow_pct", "洗盘_density_5d",
         "vol_ratio", "rps_60", "pct_70_con", "ma_tightness", "label_pm_10d_net"]
    ]
    print("\n=== A. 300911 链路特征 (dual 检查点 + parallel prepare_adx 列) ===")
    print(show.round(3).to_string(index=False))
    print("   列来源: chip_*/vol_ratio_ma5/shadow/吸筹/洗盘/close_position = LEGACY 特征;")
...
def part_b() -> None:
    p = pd.read_parquet(
        str(PANEL_V3_PATH), columns=["symbol", "date", "close_hfq", "low_hfq", "volume", "chip_entropy"]
    )
    ...
    wash = ok & (p["fut3_max_ret"] < 0.07) & (p["fut3_min_low"] >= p["low_hfq"])  # S 后洗盘
    ...
    def stats(sub: pd.DataFrame, label: str) -> None:
        if len(sub) < 30:
            ...
            return
        print(
            f"{label:<30} n={len(sub):>6} 10天突破≥7%={float((sub['fut10_max_ret'] >= .07).mean()):>6.2%}"
        )

    print(f"\n=== B. 三阶段链路 (879d dual 池, 截至 {pd.Timestamp(dates[-1]).date()}) ===")
    print("   时序: 收集(ent_chg20<-8% 于 S) → 试盘日 S(vr>1.5 & 0<ret<15%) → 洗盘(S+1..S+3)")
    ...
    out = {
        ...
        "probe_wash_rate": round(float(p.loc[probe & wash, "fut10_max_ret"].ge(0.07).mean()), 4),
        "chain_rate": round(float(p.loc[collect & probe & wash, "fut10_max_ret"].ge(0.07).mean()), 4),
    }
