...
ts_df["ts_con90_formula"] = (ts_df["ts_c95"] - ts_df["ts_c5"]) / (ts_df["ts_c95"] + ts_df["ts_c5"])

ak_df = ak.stock_cyq_em(symbol="600671", adjust="")
ak_df.columns = ["date", "bp", "avg", "p90lo", "p90hi", "p90con", "p70lo", "p70hi", "p70con"]
ak_df["date"] = pd.to_datetime(ak_df["date"])
# akshare: derive con90 from its own low/high using the SAME formula
ak_df["ak_con90_formula"] = (ak_df["p90hi"] - ak_df["p90lo"]) / (ak_df["p90hi"] + ak_df["p90lo"])

m = ak_df.merge(ts_df, on="date", how="inner")
m = m.sort_values("date", ascending=False).head(15)

print("=" * 95)
print("600671 — Is the concentration formula (hi-lo)/(hi+lo) the same for both sources?")
print("=" * 95)
print()
print(f"{'date':>12s}  {'ak_reported':>12s}  {'ak_formula':>12s}  {'ak_diff':>8s}"
...
for r in m.itertuples():
    ...
    print(f"{r.date.strftime('%Y-%m-%d'):>12s}  {r.p90con:>12.6f}  {r.ak_con90_formula:>12.6f}  {ak_diff:>8.6f}"
          f"  |  {r.ts_con90_formula:>12.6f}  |  {cross_diff:>14.6f}")

print()
print("Legend:")
print("  ak_reported    = akshare's pct_90_con (directly from API)")
print("  ak_formula     = (pct_90_high - pct_90_low) / (pct_90_high + pct_90_low)  [from akshare data]")
print("  ak_diff        = |ak_reported - ak_formula|  [should be ~0 if formula is correct]")
print("  ts_formula     = (cost_95pct - cost_5pct) / (cost_95pct + cost_5pct)      [from Tushare data]")
print("  cross_src_diff = |ak_formula - ts_formula|  [will be >0 because distributions differ]")
