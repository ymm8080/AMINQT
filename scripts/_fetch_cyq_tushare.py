...
def fetch_one(pro, ts_code, start_date, end_date):
    ...
    for attempt in range(2):
        try:
            raw = pro.cyq_perf(ts_code=ts_code, start_date=start_date, end_date=end_date, timeout=15)
    ...
    if not remaining:
        ...
        for i, sym in enumerate(remaining):
            ...
            if (i + 1) % 100 == 0:
                ...
                logger.info("  进度: %d/%d (%.0f%%), 失败%d", i + 1, len(remaining),
                           (i + 1) / len(remaining) * 100, fail_count)
        ...
        logger.info("拉取完成: %d 行, %d 股, 失败%d, 耗时 %.1f 分钟",
                    len(cyq), cyq["symbol"].nunique(), fail_count, (time.time() - t0) / 60)
