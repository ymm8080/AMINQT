...
for board, board_df in [("main", main_df), ("dual", dual_df)]:
    ...
    valid = [c for c in candidates if c in df.columns and df[c].isna().mean() < 0.95 and df[c].dtype != object]
    ...
    print(f"  {'Feature':25s} {'IC_1d':>8s} {'IC_3d':>8s} {'IC_5d':>8s} {'AUC':>6s} {'Grade':>8s} {'RollPos':>7s}")
    print(f"  {'-'*70}")
    ...
    for f in new_features:
        ...
        print(f"  {f:25s} {ic_1d:>+8.4f} {ic_3d:>+8.4f} {ic_5d:>+8.4f} {auc:>6.4f} {grade:>8s} {roll_pos:>7.1%}")
    ...
    new_only = {
        ...
        "factors": passed,
        "detail": {f: result["detail"][f] for f in new_features if f in result["detail"]},
    }
...
del main_df, dual_df; gc.collect()
print("\nDONE")
