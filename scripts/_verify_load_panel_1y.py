...
from app.pipeline_parallel.config import BOARD_THRESHOLDS, OOS_WINDOWS, SYSTEMS
...
tracemalloc.stop()
print(
    f"load_panel: rows={len(work):,} stocks={work['symbol'].nunique():,} "
...
for board in ("main", "dual"):
    ...
    for sname, spec in SYSTEMS.items():
        ...
        for lab, _d in OOS_WINDOWS.items():
            for kind, _tn in (("primary", spec.top_n), ("alt", spec.top_n_alt)):
                cell = f"{lab}/{kind}"
...
print(
    f"结论: load_panel 已按 window_days=242 过滤; 判定与消融 1y {'完全一致' if not mismatch else '存在差异!'}"
)
