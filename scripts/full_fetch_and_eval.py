...
logger.info('Panel: %d stocks, %d rows, %d dates', df['symbol'].nunique(), len(df), df['date'].nunique())
...
if os.path.exists(mg_path):
    ...
    for i, d in enumerate(margin_dates):
        ...
        try:
            ...
            if i < 3: logger.warning('margin %s: %s', dt, str(e)[:60])
        if (i+1) % 20 == 0: logger.info('  margin: %d/%d', i+1, len(margin_dates))
        time.sleep(0.15)
    if mg_frames:
        mg_panel = pd.concat(mg_frames, ignore_index=True)
        mg_panel = mg_panel.rename(columns={'rzye':'margin_balance','rqye':'short_balance','rzmre':'margin_buy_amt','rqmcl':'short_sell_vol'})
        os.makedirs(os.path.dirname(mg_path), exist_ok=True)
        mg_panel.to_parquet(mg_path, index=False)
    logger.info('Margin: %d days, %d rows', len(mg_frames), len(mg_panel) if mg_frames else 0)
...
for i, d in enumerate(all_dates[::10]):
    ...
    try:
        ...
        if raw is not None and len(raw) > 0:
            raw['symbol'] = raw['ts_code'].str.replace('.SZ','').str.replace('.SH','')
            raw['date'] = pd.to_datetime(d)
            for c in ['net_mf_amount','buy_lg_vol','sell_lg_vol','buy_elg_vol','sell_elg_vol']:
                if c in raw.columns: raw[c] = pd.to_numeric(raw[c], errors='coerce')
            mf_frames.append(raw[['symbol','date','net_mf_amount','buy_lg_vol','sell_lg_vol','buy_elg_vol','sell_elg_vol']])
    except: pass
    if (i+1) % 20 == 0: logger.info('  moneyflow: %d/%d', i+1, len(all_dates[::10]))
    time.sleep(0.15)
if mf_frames:
    mf = pd.concat(mf_frames, ignore_index=True)
    mf['main_money_flow'] = mf['net_mf_amount']
    mf['super_large_order_net'] = mf['buy_elg_vol'].fillna(0) - mf['sell_elg_vol'].fillna(0)
    df = df.merge(mf[['symbol','date','main_money_flow','super_large_order_net','buy_lg_vol','sell_lg_vol','buy_elg_vol','sell_elg_vol']], on=['symbol','date'], how='left')
logger.info('Moneyflow: %d days, cov=%.1f%%', len(mf_frames), (1-df['main_money_flow'].isna().mean())*100 if 'main_money_flow' in df.columns else 0)
...
if os.path.exists(hn_path):
    ...
    for i, sym in enumerate(all_symbols):
        try:
            ...
            if raw is not None and len(raw) > 0:
                raw['symbol'] = sym
                raw['announce_date'] = pd.to_datetime(raw.get('ann_date', raw.get('end_date')), format='%Y%m%d', errors='coerce')
                raw['holder_count'] = pd.to_numeric(raw.get('holder_num', 0), errors='coerce')
                hn_frames.append(raw[['symbol','announce_date','holder_count']])
        except: pass
        if (i+1) % 500 == 0: logger.info('  holdernumber: %d/%d', i+1, len(all_symbols))
        time.sleep(0.05)
    if hn_frames:
    ...
    logger.info('Holdernumber: %d/%d stocks, %d rows', len(hn_frames), len(all_symbols), len(hn_all) if hn_frames else 0)

# Merge holdernumber (PIT via announce_date)
if 'hn_all' in dir() and len(hn_all) > 0:
    hn_all = hn_all.dropna(subset=['holder_count','announce_date']).sort_values('announce_date')
    df = df.sort_values('date')
    df = pd.merge_asof(df, hn_all[['symbol','announce_date','holder_count']],
...
for i, d in enumerate(all_dates[::5][:100]):  # Sample ~100 dates
    ...
    if (i+1) % 50 == 0: logger.info('  northbound: %d/%d', i+1, min(100, len(all_dates[::5])))
    time.sleep(0.2)
if nb_frames:
    ...
    df['north_net_buy'] = df['north_hold_vol'] - df.groupby('symbol')['north_hold_vol'].shift(1)
    ...
    try:
        ...
        if raw is not None and len(raw) > 0:
            ...
            for c in ['net_buy','buy_amount','sell_amount']:
                if c in raw.columns: raw[c] = pd.to_numeric(raw[c], errors='coerce')
            lhb_frames.append(raw[['symbol','date','net_buy','buy_amount','sell_amount']])
    except: pass
    time.sleep(0.2)
if lhb_frames:
    lhb_full = pd.concat(lhb_frames, ignore_index=True)
    lhb_full = lhb_full.rename(columns={'net_buy':'lhb_net_buy','buy_amount':'lhb_buy_amt','sell_amount':'lhb_sell_amt'})
    df = df.merge(lhb_full, on=['symbol','date'], how='left')
logger.info('LHB: %d days', len(lhb_frames))

# ====== Coverage report ======
print('\n=== Data Coverage ===')
for c in ['margin_balance','main_money_flow','holder_count','north_hold_vol','lhb_net_buy','roe']:
...
dims_features = {
    ...
    'dim27_indflow': ['ind_margin_chg_5d','ind_margin_accel','ind_holder_trend_20d','ind_north_chg_5d','ind_lhb_net_flow_5d','ind_capital_flow'],
    'dim22_finPIT': ['roe_qoq','roa_qoq','margin_chg','growth_accel','profit_accel','debt_leveraging','efficiency_chg','ocf_stability','roe_trend_4q','margin_trend_4q','rev_yoy_trend','quality_momentum'],
}
...
for dim, r in results.items():
    ...
    print(f"  {dim:<20s}: {r['best_factor']:<30s} IC={r['best_ic']:.5f}  NaN={r['nan_pct']:.1f}%  --> {r['verdict']} {tag}")
...
with open(out_path, 'w') as fh:
    json.dump({'timestamp': pd.Timestamp.now().isoformat(), 'include_dims': include_dims, 'results': results}, fh, ensure_ascii=False, indent=2)
