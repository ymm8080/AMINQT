...
def predict_legacy(symbols):
    ...
    del panel
    print(
        f"[legacy] clean main={len(main_df):,} dual={len(dual_df):,} valve={valve}",
        flush=True,
    )
...
def predict_parallel(symbols):
    ...
    sb_rows = w.sort_values("date").groupby("symbol", as_index=False).tail(1)
    print(
        f"[parallel] slow_bull 池含目标 {sorted(pool_members & want)} "
        f"({time.time() - t0:.0f}s)",
        flush=True,
    )
...
def main() -> int:
    ...
    want = [_z6(s) for s in symbols]
    print(
        f"预测目标: {want}  (多模块: legacy main/dual + parallel sniper/fusion/slow_bull)",
        flush=True,
    )
    ...
    show_legacy = (
        [c for c in LEGACY_SHOW if c in leg.columns] + ["module"]
        if not leg.empty
        else []
    )
    if not leg.empty:
        print("\n" + "=" * 130)
        print(
            "【legacy 模块】逐股预测 (pred_ret=预期涨幅, prob_up=上涨概率, pain_prob=回撤风险)"
        )
        print("=" * 130)
        ...
        if missing_leg:
            print(
                f"[legacy][miss] {missing_leg} 未被 legacy 预测 (清洗剔除或缺模型)",
                flush=True,
            )
    ...
    if not merged.empty:
        print("\n" + "=" * 130)
        print(
            f"【并行 merged 短名单】sniper/fusion 池分→mag_10d 校准 (T+10 预期 close-to-close 幅度), "
        ...
        if not_top:
            print(
                f"[parallel] 未进 TOP-10: {not_top} (仍给出 score/mag 供参考)",
                flush=True,
            )
        missing_m = sorted(set(want) - set(merged["symbol"]))
        if missing_m:
            print(
                f"[parallel][miss] {missing_m} 不在 merged 行集 (无 label_pm_10d_net 或缺特征)",
                flush=True,
            )
    ...
    if not sb_rows.empty:
        ...
        if miss_sb:
            print(
                f"[parallel][miss] {miss_sb} 不在 slow_bull 行集 (可交易性门剔除/停牌?)",
                flush=True,
            )
    return 0
