...
def main() -> int:
    ...
    out: dict = {"meta": {"panel": f"{dates[0]} → {dates[-1]}", "n_days": int(D),
                          "max_hold": M, "hard_stop": HARD_STOP, "n_min": N_MIN,
    ...
    for b, pk in picks_all.items():
        for wname, (d0, d1) in windows.items():
            ...
            out.setdefault(wname, {})[b] = (
                {"n_picks": int(len(sub))} if sub.empty
                else {"n_picks": int(len(sub)),
                      **{m: sim(sub, m, A) for m in MODES}})
        del pk
        gc.collect()
    ...
    out["summary"] = {}
    for b in ("main", "dual"):
        judge = [w for w in ("h1", "h2", "h3", "h4", "h5", "h6", "oos6m")
        ...
        pwin_delta = [out[w][b]["trail8"]["p_win"] - out[w][b]["cur"]["p_win"] for w in judge]
        out["summary"][b] = {
            "judge_windows": judge,
            "n_windows": len(judge),
            "trail8_beats_cur": sum(1 for m in margins if m > 0),
            "pwin_not_worse": sum(1 for d in pwin_delta if d >= 0),
            "margin_mean_pp": round(float(np.mean(margins)) * 100, 2) if margins else None,
            "margin_min_pp": round(float(np.min(margins)) * 100, 2) if margins else None,
            "margin_max_pp": round(float(np.max(margins)) * 100, 2) if margins else None,
        }
    ...
    for b, s in out["summary"].items():
        print(f"[{b}] 判定窗口 {s['n_windows']} 个: trail8 跑赢 cur "
