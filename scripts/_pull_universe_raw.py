...
def _finalize(base: str, subdir: str, canonical: str) -> None:
    ...
    print(
        f"[finalize] {canonical}: {len(df):,} rows (merged {len(batches)} batches)",
        flush=True,
    )
    for f in batches:
        os.remove(f)
...
def main() -> None:
    ...
    print(
        "[plan] per-date full market: daily/daily_basic/adj_factor/cyq_perf/"
    ...
    print(
        f"[perdate] todo={len(todo)} (resume after {done_date or 'none'})", flush=True
    )
    ...
    for i, d in enumerate(todo):
        ...
        if len(a):
            _save_alt(
                a[["ts_code", "trade_date", "adj_factor", "symbol"]],
                f"adj_{ds}.parquet",
                "adj_factor",
            )
        ...
        if (i + 1) % FLUSH_EVERY == 0:
            for name, parts, sub in [
                ("daily", acc_daily, "daily"),
                ("daily_basic", acc_basic, "daily_basic"),
            ...
            print(
                f"[perdate] {i + 1}/{len(todo)} ({rate:.0f}/hr) flushed @ {ds}",
                flush=True,
            )
    # 尾部 flush
    for name, parts, sub in [
    ...
    _finalize(
        cyq_base,
        "cyq_tushare",
        f"cyq_full_{today}_{datetime.now().strftime('%H%M%S')}.parquet",
    )
    _finalize(
        marg_base,
        "",
        f"margin_panel_{today}_{datetime.now().strftime('%H%M%S')}.parquet",
    )
    _finalize(lhb_base, "lhb", f"all_{d0}_{d1}_{today}.parquet")
    _finalize(
        bt_base,
        "block_trade",
        f"block_trade_full_{today}_{datetime.now().strftime('%H%M%S')}.parquet",
    )
    ...
    if prog.get("top_inst") != "done":
        ...
        lhb = pd.concat(
            [pd.read_parquet(f, columns=["symbol", "date"]) for f in lhb_files]
        )
    ...
    if prog.get("fina") != "done":
        ...
        for i, sym in enumerate(todo_fina):
            code = ts_map.get(
                sym, sym + (".SH" if sym.startswith(("6", "5")) else ".SZ")
            )
            f = _fetch_retry(
                pro.fina_indicator,
                f"fina {sym}",
            ...
            if (i + 1) % FLUSH_EVERY == 0:
                if acc:
                    ...
                    acc = []
                print(
                    f"[fina] {i + 1}/{len(todo_fina)} "
                    f"({(i + 1) / (time.time() - t0) * 3600:.0f}/hr)",
                    flush=True,
                )
        if acc:
            _save("fina", pd.concat(acc, ignore_index=True), "fina")
