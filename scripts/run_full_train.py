...
"""
import json, logging, os, sys, pickle
import numpy as np
import pandas as pd
...
from app.pipeline1.panel_builder import enrich_panel
...
def main():
    ...
    cyq_cov = panel['pct_90_con'].notna().mean() * 100 if 'pct_90_con' in panel.columns else 0
    logger.info(f"Panel: {len(panel)} rows, {n_stocks} stocks, CYQ coverage={cyq_cov:.1f}%")

    # ---- 2. Train ----
    cfg = CleaningConfig(min_list_days=180)  # 降低阈值适配200天面板
    ...
    for board, df in [("main", main_df), ("dual", dual_df)]:
        ...
        for kind in ('1d_reg', '3d_reg', '5d_reg', '1d_cls'):
            if kind in bundle.get("models", {}):
                ...
                names = model.feature_name_ if hasattr(model, 'feature_name_') and model.feature_name_ else []
                idx = np.argsort(fi)[::-1]
                board_out["model_info"][kind] = {
                    "best_iter": int(getattr(model, 'best_iteration_', -1)),
                    "nonzero": int((fi > 0).sum()),
                    "top5_share": round(float(np.sort(fi)[::-1][:5].sum() / fi.sum()), 4) if fi.sum() > 0 else 0,
                }
                # Top 10 features
                if names:
                    board_out["top_features"] = [
                        {"rank": i+1, "name": names[idx[i]], "imp": round(float(fi[idx[i]]), 4)}
        ...
        logger.info(f"[预测] {board}: pred_ret_1d n={board_out['pred_stats'].get('pred_ret_1d',{}).get('n',0)}")
    ...
    for board, brd in output["boards"].items():
        ...
        if oos_1d is not None:
            quality = "✅ GOOD" if oos_1d >= 0.03 else "⚠️ MARGINAL" if oos_1d > 0 else "❌ BAD"
