...
def measure_topn_h(work: pd.DataFrame, col: str, top_n: int) -> dict:
    ...
    if col not in work.columns:
        return {"missing": True}
    base = work[
        ["symbol", "date", col] + [lab for _, lab in HORIZONS if lab in work.columns]
    ]
...
def main():
    ...
    ap.add_argument(
        "--limit", type=int, default=0, help="只测前 N 列 (调试用, 默认全量)"
    )
    args = ap.parse_args()
    try:
    ...
    for name, lab in HORIZONS:
        ...
        baselines[name] = {
            ...
            "per_date": {
                "mag": float(g.mean().mean()),
                "winrate": float((g.mean() > 0).mean()),
            },
        }
        ...
        summary.append(
            {
                ...
                "ok_top5": ok5,
                "ok_top3": ok3,
                "passed_horizons_top5": passed5,
                "passed_horizons_top3": passed3,
    ...
    prog("-" * 100)
    prog(
        f"TOP-{TOP_N} 通过 {n_pass5} / TOP-{TOP_N_ALT} 通过 {n_pass3} / 合计 {len(cols)}"
    )
    ...
    out = {
        ...
        "meta": {
            ...
            "per": "date",
            "end": "high_only",
            "labels": [lab for _, lab in HORIZONS],
