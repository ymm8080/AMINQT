...
for code in test_codes:
    ...
    try:
        ...
        ak_df.columns = ["date", "benefit_part_ak", "avg_cost_ak", 
                         "pct_90_low_ak", "pct_90_high_ak", "pct_90_con_ak",
                         "pct_70_low_ak", "pct_70_high_ak", "pct_70_con_ak"]
        ak_df["date"] = pd.to_datetime(ak_df["date"])
        ak_df["benefit_part_ak"] = ak_df["benefit_part_ak"] * 100  # 转百分比
        print(f"akshare: {len(ak_df)} 行, {ak_df['date'].min().date()} ~ {ak_df['date'].max().date()}")
    ...
    if "float_share" in df.columns and df["float_share"].notna().any():
        ...
        if len(valid):
            float_shares = float(valid["volume"].iloc[0] / (valid["turn"].iloc[0] / 100))
        else:
            float_shares = 1e8
    ...
    print(f"本地计算: {len(calc_df)} 行, {calc_df['date'].min().date()} ~ {calc_df['date'].max().date()}")
    ...
    for name, ak_col, calc_col in metrics:
        ...
        mape = ((ak_vals - calc_vals).abs() / ak_vals.abs().replace(0, np.nan)).mean() * 100
        corr = ak_vals.corr(calc_vals) if len(ak_vals) > 1 else 0
        print(f"{name:<16} {ak_vals.mean():>12.4f} {calc_vals.mean():>12.4f} {mae:>10.4f} {mape:>7.1f}% {corr:>8.4f}")
    
    # 5. 打印最近 5 天对比
    print(f"\n最近 5 天对比:")
    cols = ["date", "benefit_part_ak", "benefit_part_calc", 
            "avg_cost_ak", "avg_cost_calc",
            "pct_70_con_ak", "pct_70_con_calc"]
