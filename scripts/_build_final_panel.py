...
def main():
    ...
    if not limit.empty:
        panel = pd.merge(panel, limit[["symbol", "trade_date", "up_limit_raw", "down_limit_raw"]], on=["symbol", "trade_date"], how="left")
    if not susp.empty:
        susp["is_suspend"] = 1
        panel = pd.merge(panel, susp[["symbol", "trade_date", "is_suspend"]], on=["symbol", "trade_date"], how="left")
        panel["is_suspend"] = panel["is_suspend"].fillna(0)
    if not margin.empty:
        m = margin[["symbol", "date", "margin_balance", "short_balance", "margin_buy_amt", "short_sell_vol"]].copy()
    ...
    if not bt.empty:
        bt_ = bt[["symbol", "trade_date", "price", "vol", "amount", "buyer", "seller"]].copy()
        panel = pd.merge(panel, bt_, on=["symbol", "trade_date"], how="left", suffixes=("", "_bt"))
    if not cyq.empty:
        c = cyq[["symbol", "trade_date", "his_low", "his_high", "cost_5pct", "cost_15pct",
                "cost_50pct", "cost_85pct", "cost_95pct", "weight_avg", "winner_rate"]].copy()
        panel = pd.merge(panel, c, on=["symbol", "trade_date"], how="left")
    if not topinst.empty:
        ti = topinst.groupby(["symbol", "trade_date"]).agg(top_inst_buy=("buy", "sum"), top_inst_sell=("sell", "sum")).reset_index()
    ...
    cols = ["symbol", "trade_date"] + [c for c in panel.columns if c not in ("symbol", "trade_date")]
    panel = panel[cols]
