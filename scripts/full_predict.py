...
import pickle, json, pandas as pd, numpy as np, pyarrow.parquet as pq, os, sys, time, warnings
...
CR = dict(n_estimators=200, max_depth=6, num_leaves=31, learning_rate=0.05, min_child_samples=100,
    subsample=0.8, colsample_bytree=0.6, reg_alpha=0.1, reg_lambda=1.0, random_state=42, n_jobs=-1, verbosity=-1)
...
q10 = LGBMRegressor(objective='quantile', alpha=0.10,
    **{k:v for k,v in CR.items() if k != 'reg_alpha'}).fit(X_tr, y1)
...
# --- Train pain (P(loss > 2%)) ---
t0 = time.time()
pain = LGBMClassifier(n_estimators=150, max_depth=5, num_leaves=31, learning_rate=0.05,
    min_child_samples=200, subsample=0.8, colsample_bytree=0.6, reg_alpha=0.1, reg_lambda=1.0,
    random_state=42, n_jobs=-1, verbosity=-1).fit(X_tr, (y1 < -0.02).astype(int))
...
r = pd.DataFrame({
    ...
    'pred_1d': P['pred_1d'], 'prob_up': P['prob_up'],
    'pred_3d': P['pred_3d'], 'pred_5d': P['pred_5d'],
    'q10': P['q10'], 'q90': P['q90'], 'pain': P['pain'],
    'adj_score': P['adj'],
...
# --- REPORT ---
print(f'\n{"="*90}')
print(f'  FULL PREDICTION REPORT — Top 10 Risk-Adjusted')
print(f'{"="*90}')
print(f'{"Rk":<3} {"Symbol":<10} {"1d":>8} {"3d":>8} {"5d":>8} {"Prob":>6} {"Pain":>6} {"Q10":>8} {"Q90":>8}')
print(f'{"-"*85}')
for i, (_, row) in enumerate(r.head(10).iterrows(), 1):
    print(f'{i:<3} {row.symbol:<10} {row.pred_1d:>+.4f} {row.pred_3d:>+.4f} {row.pred_5d:>+.4f} '
