...
def joint_gate(df: pd.DataFrame) -> pd.Series:
    return (df["pred_ret_3d"] > 0.0) | ((df["pred_ret_2d"] > T2_MIN) & (df["pred_ret_3d"] > T3_FLOOR))
...
def subset_metrics(sub: pd.DataFrame, tag: str, day) -> dict:
    ...
    if len(sub) < 3:
        row.update({"ret3d": np.nan, "ret2d": np.nan, "win3d": np.nan,
                    "win3d5": np.nan, "ic3d": np.nan, "prob_ric3d": np.nan})
        return row
    ...
    if sub["ret_3d"].nunique() > 1:
        ...
        row["prob_ric3d"] = float(r2.statistic) if r2.statistic == r2.statistic else np.nan
    else:
...
def main() -> None:
    ...
    for N in (10, 14, 20):
        ...
        for period, tag_n in (("月", None), ("周", 5)):
            ...
            for grp in ("old", "joint", "rescued"):
                ...
                if not len(g):
                    continue
                print(f"  {grp:8s} n={g['n'].mean():4.1f}/日 | "
    ...
    for N, A in partA:
        ...
        for period, tag_n in (("month", None), ("week", 5)):
            ...
            agg = sub.groupby("group").agg(
                n=("n", "mean"), ret3d=("ret3d", "mean"), ret2d=("ret2d", "mean"),
                win3d=("win3d", "mean"), win3d5=("win3d5", "mean"),
                ic3d=("ic3d", "mean"), prob_ric3d=("prob_ric3d", "mean"))
    ...
    for _d, g in raw.groupby("date"):
        ...
        rescued = g2[(g2["pred_ret_3d"] > T3_FLOOR) & (g2["pred_ret_3d"] <= 0) & (g2["pred_ret_2d"] > T2_MIN)]
        for df2, tag in ((old, "old_keep"), (rescued, "rescue_band")):
            if len(df2) < 3:
                continue
            rb_rows.append({"day": _d, "group": tag, "n": len(df2),
    ...
    for period, tag_n in (("月", None), ("周", 5)):
        ...
        for grp in ("old_keep", "rescue_band"):
            ...
            if not len(g):
                print(f"  {grp:12s} (无数据)")
                continue
            print(f"  {grp:12s} n={g['n'].mean():5.1f}/日 | "
                  f"T+3 {g['ret3d'].mean():+.4f} | T+2 {g['ret2d'].mean():+.4f} | T+5 {g['ret5d'].mean():+.4f} | "
                  f"上涨率 {g['win3d'].mean():.1%} ({g['win3d5'].mean():.1%} >+0.5%)")
        agg = sub.groupby("group").agg(n=("n", "mean"), ret3d=("ret3d", "mean"),
                                       ret2d=("ret2d", "mean"), ret5d=("ret5d", "mean"),
                                       win3d=("win3d", "mean"), win3d5=("win3d5", "mean"))
    ...
    fs = sorted(glob.glob(r"D:/AMINQT/DAILY OPERATION/STOCK LIST/parallel_shortlist_2026*.csv"))
    partB = []
    for f in fs:
        ...
        added = df[(df["pred_mag_3d"] > T3_FLOOR) & (df["pred_mag_3d"] <= 0) & (df["pred_mag_2d"] > T2_MIN)]
        for df2, tag in ((old, "old_keep"), (added, "joint_added")):
            if len(df2) == 0:
                continue
            r3 = df2["ret_3d"].dropna()
            r2 = df2["ret_2d"].dropna()
            print(f"  {f.split('/')[-1]:46s} {tag:12s} n={len(df2):2d} "
                  f"T+3 {r3.mean():+.2%}({len(r3)}有数据) T+2 {r2.mean():+.2%}({len(r2)}有数据) "
                  f"标的: {','.join(df2['symbol'])}")
            partB.append({"file": f.split('/')[-1], "tag": tag, "n": len(df2),
