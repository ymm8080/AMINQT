...
def eval_all(board_name: str, board_df: pd.DataFrame) -> dict:
    ...
    logger.info("[%s] %s -> %s, %d rows, %d stocks",
    ...
    logger.info("[%s] Feature engine: %d cols in %.0fs", board_name, len(df.columns), (datetime.now()-t0).total_seconds())
    ...
    for i, col in enumerate(feature_cols):
        ...
        if (i + 1) % 500 == 0:
            ...
            logger.info("[%s] %d/%d (%.0fs, %d results)",
                        board_name, i + 1, len(feature_cols), elapsed, len(results))
    ...
    print(f"  {len(board_df['symbol'].unique())} stocks, {len(board_df)} rows, {len(feature_cols)} features evaluated")
    print(f"{'=' * 100}")
    ...
    print(f"  IC range: [{results_df['ic_mean'].min():+.4f}, {results_df['ic_mean'].max():+.4f}]")
    ...
    return {
        ...
        "ic_range": [float(results_df["ic_mean"].min()), float(results_df["ic_mean"].max())],
    }
...
def main():
    ...
    for board_val, label, n in [("main", "main", N_STOCKS), ("GEM", "dual", N_STOCKS//2), ("STAR", "dual", N_STOCKS//2)]:
    ...
    logger.info("Main: %d stocks, %d rows | Dual: %d stocks, %d rows",
                main_df["symbol"].nunique(), len(main_df),
                dual_df["symbol"].nunique(), len(dual_df))
