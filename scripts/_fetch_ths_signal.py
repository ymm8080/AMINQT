...
def _extract_table(d: dict):
    ...
    tb = next(
        (
            c
            for c in comps
            if c.get("show_type", "").startswith("xuangu_table")
            and c.get("data", {}).get("datas")
        ),
        None,
    )
    if tb is None:
...
def _paginate_pool(
    question: str, dstr: str, ck: dict, perpage: int, max_pages: int
) -> pd.DataFrame:
    ...
    if d.get("status_code") != 0:
        raise RuntimeError(
            f"API status={d.get('status_code')} "
    ...
    comps = d["data"]["answer"][0]["txt"][0]["content"]["components"]
    tb = next(
        (c for c in comps if str(c.get("show_type", "")).startswith("xuangu_table")),
        None,
    )
    furl = ""
    if tb is not None:
    ...
    if not furl:
        raise RuntimeError(
            f"robot 无 footer_info.url (found_n={total}, date={dstr}) — 不落盘"
        )
    u = urlparse(
        furl
        if "://" in furl
        else "http://x.com" + (furl if furl.startswith("/") else "/" + furl)
    )
    up = {
    ...
    for page in range(1, max_pages + 1):
        ...
        list_headers = {
            k: v for k, v in _headers_with_token()[0].items() if k != "Content-Type"
        }
        lr = None
        last_err = None
        ...
        if not lr.text.lstrip().startswith("{"):
            raise RuntimeError(
                f"WAF 非JSON HTTP{lr.status_code} (date={dstr} list p{page})"
            )
        ld = lr.json()
        if str(ld.get("status_code")) != "0":
            raise RuntimeError(
                f"list API status={ld.get('status_code')} "
    ...
    if total is not None and len(seen) < total:
        raise RuntimeError(
            f"分页不完整: 找到{total} 抓到{len(seen)} (date={dstr}) — 不落盘"
        )
    return df
...
def prev_trading_day(dstr: str) -> str:
    """T-1交易日: 从本地面板日历回退"""
    pf = pd.read_parquet(
        "D:/AMINQT/PARQUET/panel_full_enriched_v3.parquet", columns=["date"]
    )
