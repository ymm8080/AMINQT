...
test_dates = dates[-60:]
test_mask = np.isin(d, np.array(test_dates).astype("datetime64[ns]")) & df[LBL].notna().to_numpy()
...
print(f"test rows={len(X_test)} ({test_dates[0]:%Y-%m-%d}..{test_dates[-1]:%Y-%m-%d})", flush=True)
...
def train_fix_a():
    ...
    train_d = dates[:-80]
    es_d = dates[-80:-60]
    keep = np.isin(d, np.array(train_d).astype("datetime64[ns]")) & df[LBL].notna().to_numpy()
    idx = np.flatnonzero(keep)
    X, y = df.iloc[idx].loc[:, cols].to_numpy(np.float32), df.iloc[idx][LBL].to_numpy(float)
    es_keep = np.isin(d, np.array(es_d).astype("datetime64[ns]")) & df[LBL].notna().to_numpy()
    eidx = np.flatnonzero(es_keep)
    Xe, ye = df.iloc[eidx].loc[:, cols].to_numpy(np.float32), df.iloc[eidx][LBL].to_numpy(float)
    # time_weights (半衰期 250) 复刻生产
    tr_dates_sub = sorted(set(train_d))
    w_map = {dp: 0.5 ** ((len(tr_dates_sub) - 1 - i) / 250) for i, dp in enumerate(tr_dates_sub)}
    w = np.array([w_map[dp] for dp in d[idx]])
    ds = lgb.Dataset(X, y)
    ds.set_weight(w)
    bst = lgb.train(
        dict(objective="huber", learning_rate=0.05, num_leaves=31, seed=42, verbosity=-1),
        ds,
...
print(f"  TOP-15: hit={md['hit_rate']:.3f} top_ret={md['top_ret']:+.4f} pool={md['pool_ret']:+.4f} excess={md['excess']:+.4f} ({md['n_days']}d)")
del m10, pred_deployed; gc.collect()
...
print(f"  TOP-15: hit={mf['hit_rate']:.3f} top_ret={mf['top_ret']:+.4f} pool={mf['pool_ret']:+.4f} excess={mf['excess']:+.4f} ({mf['n_days']}d)")

# rank IC 对比
from scipy.stats import spearmanr
def rank_ic(pred, labels, dtarr):
    f = pd.DataFrame({"d": dtarr, "p": pred, "y": labels})
    return f.groupby("d").apply(lambda g: spearmanr(g["p"], g["y"])[0], include_groups=False).mean()
