...
def gap_pocket_picks(close: pd.DataFrame, pctchg: pd.DataFrame, amount: pd.DataFrame,
                     turnover: pd.DataFrame, day_ts: pd.Timestamp,
                     top_n: int = GAP_TOP_N) -> pd.DataFrame:
    ...
    if len(idx) < BIAS_WIN + 1:
        return pd.DataFrame(columns=["rank", "symbol", "d", "dry", "pull", "r10", "tov"])
    c = close.loc[idx]
    p = pctchg.loc[idx]
    a = amount.loc[idx]
    tv = turnover.loc[idx]
    ...
    for i0 in range(max(0, t - D_HI), t):
        ...
        for j in np.where(first.values[i0])[0]:
            ...
            tov = tvv[t, j]
            if not (np.nan_to_num(r10, nan=0.0) <= R10_MAX
                    and np.nan_to_num(tov, nan=0.0) <= TOV_MAX):
                continue                            # 安全闸
            rows.append((syms[j], d, dry, pull, r10,
                         0.0 if np.isnan(tov) else tov, bias.get(syms[j], np.nan)))
    if not rows:
        return pd.DataFrame(columns=["rank", "symbol", "d", "dry", "pull", "r10", "tov"])
    out = pd.DataFrame(rows, columns=["symbol", "d", "dry", "pull", "r10", "tov", "bias"])
...
def main() -> int:
    ...
    panel = pd.read_parquet(
        ...
        filters=[("date", ">=", day_ts - pd.Timedelta(days=LOOKBACK_DAYS)),
                 ("date", "<=", day_ts)],
    )
    ...
    tov = panel.pivot(index="date", columns="symbol", values="turnover_rate").sort_index()
