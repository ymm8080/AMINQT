...
def measure_topn(
    ...
    if per == "date":
        ...
        top = (
            base.sort_values([col], ascending=ascending)
        ...
        top = (
            base.sort_values(["symbol", col], ascending=ascending)
...
def measure_topdecile(work: pd.DataFrame, col: str, per: str = "date") -> dict:
    ...
    if per == "date":
        top = (
            ...
            .groupby("date", group_keys=False)
            .apply(lambda g: g.head(max(1, int(len(g) * 0.10))))
        )
    else:
        top = (
            base.sort_values(["symbol", col], ascending=False)
            .groupby("symbol", group_keys=False)
            .apply(lambda g: g.head(max(1, int(len(g) * 0.10))))
        )
    out = {}
...
def report_topn(
    work: pd.DataFrame, cols, top_n=10, per="date", out=None, prog=None
) -> list:
    """对多列批量测 TOP-N, 输出 幅度/胜率 表, 返回裁决清单."""
    lines = out if out is not None else []
    lines.append("=" * 92)
    lines.append(
        f"  TOP{top_n} 绝对上涨裁决 ({per}截面 | 净收益标签 label_pm_*d_net | "
    ...
    for c in cols:
        ...
        lines.append(row)
        summary.append(
            {"col": c, "verdict": verdict, "detail": {k: r[k] for k in HORIZONS}}
        )
        if prog:
            prog(f"    done {c}")
    return summary
