...
from app.pipeline_parallel.config import FUSION, MAG10D_CAL, SNIPER
...
def main() -> int:
    print(
        f"calibrate_mag10d 键 vs score 键 | 末 {EVAL_DAYS} 已实现日, TOP-{TOP} | "
    ...
    for board in ("main", "dual"):
        ...
        sco_st = (
            rr.sort_values(["date", "score"], ascending=[True, False])
            .groupby("date", sort=False)
            .head(TOP)
        )
        ...
        if len(dv):
            print(
                f"  反序日({len(dv)}天): mag 实得 {div_m:+.2%} vs score 实得 {div_s:+.2%}"
                f" (Δ {div_m - div_s:+.2%})"
            )
        print(
            f"  反序日 mag-top5 均分 {float(df.loc[df.overlap < TOP, 'mag5_mean_score'].mean()):.3f}"
