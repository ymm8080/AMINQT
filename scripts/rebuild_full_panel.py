...
import pandas as pd, numpy as np, logging, os, time, sys
...
logger.info('Base panel: %d stocks, %d rows, %d cols',
...
if pro is not None:
    try:
        ...
        if basic is not None and len(basic):
            basic['symbol'] = basic['ts_code'].str.replace('.SZ','',regex=False).str.replace('.SH','',regex=False)
            ind_map = dict(zip(basic['symbol'], basic['industry'].fillna('综合')))
            panel['industry'] = panel['symbol'].map(ind_map).fillna('综合')
            industry_fixed = True
            logger.info('Industry fixed via Tushare: %d unique', panel['industry'].nunique())
...
if not industry_fixed:
    ...
    try:
        ...
        if ind_map:
            panel['industry'] = panel['symbol'].map(ind_map).fillna('综合')
            logger.info('Industry fixed via AKShare: %d unique', panel['industry'].nunique())
    except Exception as e:
...
CYQ_COLS = ['benefit_part','avg_cost','pct_70_low','pct_70_high','pct_70_con',
            'pct_90_low','pct_90_high','pct_90_con','cost_5pct','cost_15pct',
...
if os.path.exists(margin_path):
    ...
    margin_cols = ['symbol', 'date'] + [c for c in margin.columns
                   if c not in ('symbol', 'date') and not c.startswith('_')]
    panel = panel.merge(margin[margin_cols], on=['symbol', 'date'], how='left')
    mc = 'margin_balance' if 'margin_balance' in panel.columns else (margin_cols[2] if len(margin_cols) > 2 else None)
    logger.info('  margin: merged %d rows, %d cols -> NaN=%.1f%%',
                len(margin), len(margin_cols) - 2,
...
for cand in [nb_path, os.path.join(CACHE, 'northbound', 'all_20240102_20260727.parquet')]:
    if os.path.exists(cand):
        nb_path = cand
        break
if os.path.exists(nb_path):
    nb = safe_date(pd.read_parquet(nb_path))
    date_cols = [c for c in nb.columns if c not in ('symbol', 'date') and not c.startswith('_')]
    ...
    for c in date_cols:
        ...
        logger.info('  northbound source %s: NaN=%.1f%% (%d/%d dates have data)',
                    c, src_nan * 100,
                    nb_by_date[c].notna().sum(), len(nb_by_date))
...
if os.path.exists(lhb_path):
    ...
    logger.info('  lhb: merged %d rows, %d cols -> NaN=%.1f%%',
                len(lhb_sub), len(avail) - 2,
                panel['lhb_net_buy'].isna().mean() * 100 if 'lhb_net_buy' in panel.columns else 0)
else:
...
if not os.path.exists(fina_path):
    fina_path = os.path.join(CACHE, 'fina_indicator', f'all_{date_min}_{date_max}.parquet')
if os.path.exists(fina_path):
    ...
    if 'announce_date' in fina.columns:
        ...
        if dropped:
            logger.warning('  fina_indicator: dropped %d rows with null announce_date', dropped)
        fina_cols = [c for c in fina.columns
        ...
        roe_col = 'roe' if 'roe' in panel.columns else (fina_cols[0] if fina_cols else None)
        if roe_col:
            logger.info('  fina_indicator: merged %d rows, %d cols -> NaN=%.1f%%',
                        len(f), len(fina_cols), panel[roe_col].isna().mean() * 100)
    else:
...
if sw_path and 'industry' in panel.columns:
    ...
    if ind_map:
        ...
        sw_data = sw.rename(columns={
            'ret_pct': 'sw_ret_1d', 'close': 'sw_index_close', 'volume': 'sw_index_vol',
        })
        sw_avail = [c for c in ['sw_ret_1d', 'sw_index_close', 'sw_index_vol'] if c in sw_data.columns]
        panel = panel.merge(
            sw_data[['index_name', 'date'] + sw_avail],
            left_on=['_sw_name', 'date'], right_on=['index_name', 'date'],
            how='left',
        )
        # Deduplicate columns (merge may add duplicate index_name)
        panel = panel.loc[:, ~panel.columns.duplicated()]
        panel = panel.drop(columns=['_sw_name'], errors='ignore')
        logger.info('  sector_index: %d/%d industries mapped, %d cols -> sw_ret_1d NaN=%.1f%%',
                    len(ind_map), panel['industry'].nunique(),
                    len(sw_avail), panel['sw_ret_1d'].isna().mean() * 100 if 'sw_ret_1d' in panel.columns else 0)
    else:
...
fina_dim_cols = ['roe', 'roe_deducted', 'roa', 'gross_margin', 'rev_yoy',
                 'debt_ratio', 'current_ratio', 'asset_turnover', 'ocf_to_or']
...
if present:
    ...
    logger.info('  DIM22 基本面细项: %d/%d cols present, avg NaN=%.1f%%',
                len(present), len(fina_dim_cols), avg_nan)
