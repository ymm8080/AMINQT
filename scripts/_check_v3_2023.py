...
v3_old = pd.read_parquet(
    "data/panel_full_enriched_v3_backup_20260729.parquet",
    columns=["symbol", "date", "roe"],
)
...
for stock in ["000001", "000002", "000620", "600519"]:
    ...
    print(
        f"  Current v3 roe: NaN={cur['roe'].isna().sum()}, non-NaN={cur['roe'].notna().sum()}"
    )
    print(
        f"  Backup v3 roe:  NaN={old['roe'].isna().sum()}, non-NaN={old['roe'].notna().sum()}"
    )
    if cur["roe"].notna().sum() > 0:
        print(
            f"  Current v3 first non-NaN roe: {cur[cur['roe'].notna()].head(3)[['date', 'roe']].to_string()}"
        )
    if old["roe"].notna().sum() > 0:
        print(
            f"  Backup v3 first non-NaN roe: {old[old['roe'].notna()].head(3)[['date', 'roe']].to_string()}"
        )
...
print(
    f"2023 backup NaN:  {v3_old_2023['roe'].isna().sum()} / {len(v3_old_2023)} = {v3_old_2023['roe'].isna().mean() * 100:.1f}%"
)

# Are the roe values actually the same?
merged = v3_2023[["symbol", "date", "roe"]].merge(
    v3_old_2023[["symbol", "date", "roe"]],
    on=["symbol", "date"],
    suffixes=("_new", "_old"),
)
