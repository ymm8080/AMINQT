...
need = ["date", "symbol", "close_hfq", "cost_50pct", "cost_95pct", "pct_90_con",
        "pct_90_high", "winner_ratio", "cost_bias"]
...
if len(miss):
    for sym in syms_missing:
        suffix = ".SZ" if sym.startswith(("0", "3")) else ".SH" if sym.startswith("6") else None
        if suffix is None:
            print(f"    {sym}: 非 SZ/SH, 跳过")
            continue
        d = base_t[(base_t["symbol"] == sym) & base_t["cost_50pct"].isna()]["date"]
        d0 = d.min().strftime("%Y%m%d"); d1 = d.max().strftime("%Y%m%d")
        # 限速: cyq 逐只拉, 温和节流
        time.sleep(0.12)
        try:
        ...
        r["pct_90_con"] = (r["cost_95pct"] - r["cost_5pct"]) / (r["cost_95pct"] + r["cost_5pct"])
        r["winner_ratio"] = r["winner_rate"] / 100.0
        fetched[sym] = r.set_index("date")[["cost_50pct", "cost_95pct", "pct_90_con", "pct_90_high", "winner_ratio"]].dropna(how="all")
        print(f"    {sym}: fetched {len(r)} rows in [{d0}..{d1}]")

# ── 3. 构造写入映射 (idx → dict-of-col-vals) ──
t_all = t_all.set_index(["date", "symbol"], drop=False)
changes_A = 0; changes_B = 0
...
cb_mask = t_all["cost_bias"].isna() & t_all["cost_50pct"].notna() & t_all["close_hfq"].notna()
...
for rg in range(pf.metadata.num_row_groups):
    ...
    df_rg_idx = pd.MultiIndex.from_arrays([pd.to_datetime(df_rg["date"]), df_rg["symbol"]])
    ...
    for i in a_here:
        ...
        if pd.notna(c50) and c50 != 0 and pd.notna(df_rg.at[df_rg.index[i], "close_hfq"]):
        ...
        if examples_printed < 3:
            print(f"    A-fill sample {key}: cost_50pct={c50}, cost_bias={df_rg.at[df_rg.index[i],'cost_bias']}")
            examples_printed += 1

    # B: cost_bias 重算 (含 2026 两日大洞)
    b_here_mask = df_rg["cost_bias"].isna() & df_rg["cost_50pct"].notna() & df_rg["close_hfq"].notna()
    if b_here_mask.any():
...
# ── 7. 审计 ──
v = pq.read_table(PANEL, columns=["date", "symbol", "cost_50pct", "cost_bias"]).to_pandas()
v["date"] = pd.to_datetime(v["date"])
w = v[(v["date"] >= "2023-09-18") & (v["date"] <= "2026-09-16")]
g = w.groupby("date").agg(rows=("symbol", "count"), c50_miss=("cost_50pct", lambda s: s.isna().sum()),
