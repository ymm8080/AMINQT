...
def _gate_counts(df: pd.DataFrame) -> dict:
    s = ADX_SPEC
    g1 = (
        (df["close_cont"] > df["ma5"]) & (df["ma5"] > df["ma10"])
        & (df["ma10"] > df["ma20"]) & (df["ma20"] > df["ma60"])
        & (df["ma_slope5"] > 0) & (df["ma_slope10"] > 0)
        & (df["ma_slope20"] > 0)
        & ((df["ma5"] - df["ma10"]).abs() / df["ma10"] < s["ma_bias_max"])
    )
    g2 = ((df["adx"] > s["adx_min"]) & (df["pdi"] > df["mdi"])
          & (df["adx_rise5"] > 0))
    g3 = ((df["amplitude_20"] < s["amplitude_20_max"])
          & (df["max_drop_20"] > -s["max_drop_20_max"])
          & (df["limit_down_20"] == 0))
    g4 = ((df["ma_vol_5"] > df["ma_vol_10"]) & (df["ma_vol_10"] > df["ma_vol_20"])
    ...
    for name, g in (("g1_ma", g1), ("g2_adx", g2), ("g3_lowvol", g3),
                    ("g4_volume", g4)):
        out[name] = {"rows": int(g.sum()),
                     "pct": round(float(g.mean()), 5),
                     "stocks": int(df.loc[g, "symbol"].nunique())}
    out["all_4"] = {"rows": int((g1 & g2 & g3 & g4).sum()),
    ...
    for name, g in (("g4a_vol_mono", g4a), ("g4b_volratio_yest", g4b),
                    ("g4c_turnover_3_15", g4c)):
        out[name] = {"rows": int(g.sum()),
                     "pct": round(float(g.mean()), 5),
                     "stocks": int(df.loc[g, "symbol"].nunique())}
    out["g4a_g4b_g4c"] = {"rows": int((g4a & g4b & g4c).sum()),
                          "stocks": int(df.loc[g4a & g4b & g4c, "symbol"].nunique())}
    # NaN 中毒检查: 量比/换手 缺失率
    for col in ("vol_ratio", "turnover_rate"):
        out[f"nan_{col}"] = {"rows": int(df[col].isna().sum()),
                             "pct": round(float(df[col].isna().mean()), 5)}
    vr = df["vol_ratio"].dropna()
    out["vol_ratio_desc"] = {"min": float(vr.min()), "max": float(vr.max()),
