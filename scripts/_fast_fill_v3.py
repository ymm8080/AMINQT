...
SOURCES = [
    ...
    ("margin",        "symbol+date", ["margin_balance", "margin_buy_amt", "short_balance", "short_sell_vol"]),
    ("daily_basic",   "symbol+date", ["turnover_rate_f", "volume_ratio_y", "pe_ttm", "pb", "ps_ttm",
    ...
    ("northbound",    "date",        ["north_net_buy_sh", "north_net_buy_sz", "north_buy_amt_sh",
...
def load_cache(src):
    ...
    logger.info("  %s: %d 行 %d 列, %s ~ %s", src, len(out), len(out.columns),
...
def merge_source(v3, src_name, merge_type, known_cols, cache_df):
    ...
    # 排除非数据列
    skip_cols = {"symbol", "date", "_ts_code", "announce_date", "report_period",
                 "index_code", "index_name", "sh_holder_name", "sh_change_type"}
...
def main():
    ...
    logger.info("v3: %d 行 %d 列 %d 股, %s ~ %s",
                len(v3), len(v3.columns), v3["symbol"].nunique(),
                v3["date"].min(), v3["date"].max())
