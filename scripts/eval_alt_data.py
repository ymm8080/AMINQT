...
def fetch_and_merge_alt_data(
    ...
    logger.info(
        "Fetching alt data: %s, %d stocks, %s - %s", sources, len(symbols), start, end
    )
            ...
            if src == "fina_indicator":
                ...
                if len(df) and "announce_date" in df.columns:
                    fin_cols = [
                        ...
                        for c in df.columns
                        if c
                        not in ("symbol", "report_period", "_ts_code", "announce_date")
                    ]
                    cols_avail = ["symbol", "announce_date"] + [
                        c for c in fin_cols if c in df.columns
                    ]
                    ...
                    logger.info(
                        "  fina_indicator: %d rows, %d cols, %.1fs",
                        len(df),
                        len(fin_cols),
                        time.time() - t0,
                    )
                ...
                if frames:
                    ...
                    if "announce_date" in df.columns:
                        ...
                        f = df[["symbol", "announce_date"] + hn_cols].sort_values(
                            "announce_date"
                        )
                        panel = panel.sort_values("date")
                        panel = pd.merge_asof(
                ...
                if len(df):
                    ...
                    logger.info(
                        "  northbound: %d rows, %.1fs", len(df), time.time() - t0
                    )
                ...
                if len(df) and "industry" in panel.columns:
                    ...
                    if ind_map:
                        ...
                        si = si.rename(
                            columns={"ret_pct": "sw_ret_1d", "close": "sw_close"}
                        )
                        panel = panel.merge(
                            si,
                            left_on=["_sw", "date"],
                        ...
                        panel = panel.drop(
                            columns=["_sw", "index_name"], errors="ignore"
                        )
                        logger.info(
                            "  sector_index: %d indices, %.1fs",
                            len(sw_names),
                            time.time() - t0,
                        )
...
def evaluate_and_report(panel: pd.DataFrame, output_dir: str = "data/factor_registry"):
    ...
    print("\n" + "=" * 110)
    print(
        f"{'数据源':<20s} {'因子数':<6s} {'IC_1d':<8s} {'IC_3d':<8s} {'IC_5d':<8s} {'Roll60':<8s} {'Pos%':<7s} {'NaN%':<7s} {'判决':<10s} {'最优因子'}"
    )
    print("-" * 110)
    ...
    for src_id, cfg in DATA_SOURCE_GROUPS.items():
        ...
        if not feats:
            ...
            print(
                f"{cfg['name']:<20s} {'0':<6s} {'-':<8s} {'-':<8s} {'-':<8s} {'-':<8s} {'-':<7s} {'100%':<7s} {'无数据':<10s} -"
            )
            continue
        ...
        results[src_id] = {
            ...
            "ic_1d": round(best_ic1, 5),
            "ic_3d": round(best_ic3, 5),
            "ic_5d": round(best_ic5, 5),
            "rolling_mean": round(best_rm, 5),
            "rolling_pos": round(best_rp, 3),
            "nan_rate": round(best_nan, 3),
            "verdict": verdict,
            "best_factor": best_f,
            "coverage": cfg["coverage"],
            "freq": cfg["freq"],
        ...
        print(
            f"{cfg['name']:<20s} {len(feats):<6d} {best_ic1:<8.4f} {best_ic3:<8.4f} "
    ...
    out_path = os.path.join(
        output_dir, f"alt_data_eval_{datetime.now():%Y%m%d_%H%M}.json"
    )
    ...
    print(f"\nSaved: {out_path}")
    print(
        f"Panel: {df['symbol'].nunique()} stocks x {df['date'].nunique()} dates = {len(df)} rows"
    )
...
def main():
    ...
    p.add_argument(
        "--sources",
        default="fina_indicator,holdernumber,margin,northbound,lhb,sector_index",
    )
    p.add_argument("--output-dir", default="data/factor_registry")
    p.add_argument(
        "--skip-fetch", action="store_true", help="跳过拉取, 面板已有 alt 列"
    )
    args = p.parse_args()
