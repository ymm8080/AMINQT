...
for L in ["l1","l2","l3"]:
    for s in ["close","vol","amount","pe","pb","ret_1d","ret_5d","ret_20d","vol_20d","momentum_accel","turnover_anomaly","rotation_position","relative_strength"]:
        c = f"sw_{L}_{s}"
        if c not in DROPS:
            SC.append(c)
...
def load_df(n=200, d0="2022-01-01"):
    ...
    cl = ["date","symbol","close","high","low","open","volume","amount","pre_close","industry","sw_l1_name","sw_l2_name","sw_l3_name"]
    ...
    if n > 0:
        ...
        if len(st) > n:
            df = df[df["symbol"].isin(np.random.RandomState(42).choice(st,n,False))].copy()
...
def ic_eval(df, cols, lc):
    ...
    for c in cols:
        if c not in df.columns:
            out.append({"f":c,"ic":np.nan,"icir":np.nan,"t":np.nan,"n":0,"cov":np.nan}); continue
        cv = df[c].notna().mean()
        ics = []
        ...
        nn = len(ics)
        if nn<20:
            out.append({"f":c,"ic":0.0,"icir":0.0,"t":0.0,"n":nn,"cov":cv}); continue
        a = np.array(ics); m=a.mean(); s=a.std(ddof=1)
        out.append({"f":c,"ic":round(m,5),"icir":round(m/s if s>0 else 0,4),"t":round(m/(s/np.sqrt(nn)) if s>0 else 0,2),"n":nn,"cov":round(cv,4)})
...
def lgb_eval(df, cols, lc):
    ...
    m.fit(tr[av],tr[lc])
    imp = pd.DataFrame({"feature":av,"g":m.booster_.feature_importance(importance_type="gain")}).sort_values("g",ascending=False)
...
def cclust(df, cols, th=0.85):
    ...
    asg={}; cid=0
    for c in av:
        if c in asg: continue
        asg[c]=cid
        for o in av:
            if o not in asg and cr.loc[c,o]>th: asg[o]=cid
        cid+=1
    return asg
...
def main():
    ...
    for h in H:
        ...
        r = ic.copy()
        if len(imp)>0: r = r.merge(imp, on="feature", how="left")
    ...
    print(f"  {'Feature':<32s} {'Cov':>5s} | {'IC':>8s} {'ICIR':>8s} {'t':>6s} {'Gain':>5s} | {'IC':>8s} {'ICIR':>8s} {'t':>6s} {'Gain':>5s} | {'IC':>8s} {'ICIR':>8s} {'t':>6s} {'Gain':>5s} | Cl")
    print(f"  {'-'*32} {'-'*5} | {'-'*33} | {'-'*33} | {'-'*33} | --")
    print(f"  {'':32s} {'':5s} | {'T+1':^33s} | {'T+3':^33s} | {'T+5':^33s} |")
    print(f"  {'-'*32} {'-'*5} | {'-'*33} | {'-'*33} | {'-'*33} | --")

    for _, row in b.iterrows():
        def f(v, fmt="+.4f"): return f"{v:{fmt}}" if pd.notna(v) else "  N/A"
        def g(v): return f"{v:4.1f}%" if pd.notna(v) else " N/A"
        print(f"  {row['feature']:<32s} {row['cov']:.3f} | "
    ...
    for h in H:
        ...
        print(f"    T+{h}: STRONG={len(s):2d}  MOD={len(m):2d}  WEAK={len(w):2d}  NOISE={len(ns):2d}  PF_ICIR={R[h][1]:+.4f}")
    ...
    for h in H:
        ...
        for _, row in R[h][0].head(5).iterrows():
            gp = f"{row.get('gain_pct',0):.1f}%" if pd.notna(row.get("gain_pct",np.nan)) else "N/A"
    ...
    for _, r in na.iterrows():
        print(f"    {r['feature']:<32s} T1={r['ir1']:+.4f}  T3={r['ir3']:+.4f}  T5={r['ir5']:+.4f}")
    print()
