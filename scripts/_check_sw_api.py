...
with open("scripts/_sw_api_check.txt", "w", encoding="utf-8") as f:
    ...
    f.write("\n")
    try:
        sb = pro.stock_basic(list_status="L", fields="ts_code,symbol,name,area,industry,market,list_date")
        f.write(f"stock_basic: {len(sb)} stocks, {sb['industry'].nunique()} unique industries\n")
    ...
    f.write("\n")
    try:
        sc = pro.stock_company(fields="ts_code,exchange,province,city,introduction,main_business,employee_count")
