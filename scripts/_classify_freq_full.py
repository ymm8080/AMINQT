...
def report_tsxs(work, out, prog):
    out.append("=" * 82)
    out.append(
        "  PART A. 非事件组 6格判定 (TS=个股时序IC / XS=日截面rankIC; 日/周/月=1/5/20变化窗口)"
    )
    out.append("=" * 82)
    g_sym = work["symbol"]
    g_date = work["date"]
...
def report_evt(work, out, prog):
    ...
    for gname, spec in GROUPS_EVT.items():
        ...
        if spec["mask"] == "lhb_net_buy":
            ...
            ev_mask = (
                work[["sh_net_ratio", "sh_g_ratio", "sh_p_ratio", "sh_c_ratio"]]
                .notna()
                .any(axis=1)
            )
        else:
        ...
        out.append(
            f"  {'feature':<24}{'T+2':>9}{'T+3':>9}{'T+5':>9}{'T+10':>9}{'T+20':>9}"
        )
        for c in spec["cols"]:
            if c not in work.columns:
                continue
            xf = work.loc[ev_mask, ["symbol", "date", c]].set_index(["symbol", "date"])[
                c
            ]
            row = [f"{c:<24}"]
...
def main() -> None:
    ...
    with open(
        os.path.join("data", f"_classify_freq_full_summary_{ts_run}.json"),
        "w",
        encoding="utf-8",
    ) as fh:
        ...
        _json.dump(
            {"ts": ts_run, "rows": len(work), "features": summary},
            fh,
            indent=2,
            ensure_ascii=False,
        )
    ...
    out.append("=" * 82)
    out.append(
        f"{'功能组':<26}{'判定频率':>8}{'判定类型':>8}{'最强|IC|':>10}{'样本数':>6}"
    )
    out.append("-" * 60)
    grp_freq = {}
    for s in summary:
    ...
    for gname, rows in grp_freq.items():
        ...
        out.append(f"{gname:<26}{best_freq:>8}{best_type:>8}{ic:>10.4f}{len(rows):>6}")
    out.append("")
    out.append(
        "事件组: 走事件池 (跨事件对齐研究), 独立事件模块, 不进个股TS/日截面常规模."
    )
    out.append(
        "事件组衰减视界决定其归属模型: LHB/HOLDER 均为短视界事件信号, 归 日/事件 模型 (LHB T+2 rankIC +0.156 → T+20 +0.045 衰减)."
    )
    out.append(
        "注: rankIC 正 ≠ 绝对上涨 — LHB 净买>0 绝对净收益口径 T+2 -0.56%/胜率41.5%, 不达'买进能赚'裁决."
    )
