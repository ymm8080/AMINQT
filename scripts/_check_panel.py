...
df = pd.read_parquet(panel_path, columns=["symbol", "date", "close", "pre_close", "pctChg",
    "margin_buy_amt", "short_sell_vol", "north_net_buy_sh", "north_net_buy_sz",
    "holder_count", "lhb_net_buy", "sh_net_change_sign"])

print(f"Panel: {len(df)} rows, {df['symbol'].nunique()} symbols, {df['date'].nunique()} dates")
print(f"Date range: {df['date'].min()} ~ {df['date'].max()}")

# Before coverage
print("\n=== BEFORE Coverage ===")
for c in ["pre_close", "pctChg", "margin_buy_amt", "short_sell_vol",
          "north_net_buy_sh", "north_net_buy_sz", "holder_count", "lhb_net_buy", "sh_net_change_sign"]:
