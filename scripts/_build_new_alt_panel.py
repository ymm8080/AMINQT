...
_INST_KW = ("机构专用", "QFII", "合格境外", "社保", "养老", "资产管理",
            "资管", "保险", "信托")
...
def main() -> None:
    ...
    print(f"[alt] input={f} rows={len(df):,} syms={df['symbol'].nunique()} "
          f"({d0}..{d1})", flush=True)
    ...
    if fina_files:
        ...
        fin_cols = [c for c in fina.columns
                    if c not in ("symbol", "announce_date", "report_period",
                                 "_ts_code", "ts_code", "end_date")]
        if len(fina) and fin_cols:
            df = pd.merge_asof(
                df, fina[["symbol", "announce_date"] + fin_cols],
                left_on="date", right_on="announce_date",
                by="symbol", direction="backward",
            )
            print(f"[fina] merged {len(fina):,} rows, {len(fin_cols)} cols",
                  flush=True)
            ...
            fina.to_parquet(os.path.join(cache_d, f"new_symbols_{ts_}.parquet"),
                            index=False)
    else:
    ...
    try:
        ...
        if len(ht):
            ...
            for c in ["sh_change_vol", "sh_change_amt_total",
    ...
    # ── 4. bt 4 列 (恢复缓存逐日聚合) ──
    bt_files = sorted(glob.glob(os.path.join(ALT_DIR, "block_trade",
                                             "block_trade_full_*.parquet")))
    if bt_files:
        ...
        bt = bt.drop_duplicates(subset=["symbol", "trade_date", "buyer", "seller",
                                        "price", "vol", "amount"])
        ...
        if parts:
            ...
            print(f"[bt] merged {len(bt_agg):,} event rows "
                  f"({len(bt_dates)} dates)", flush=True)
    else:
        print("[bt] WARN: 无 block_trade 恢复缓存", flush=True)

    # ── 5. lhb 席位 8 列 ──
    ti_files = sorted(glob.glob(os.path.join(OUT_DIR, "top_inst", "top_inst_*.parquet")))
    if ti_files:
        ti = pd.concat([pd.read_parquet(x) for x in ti_files], ignore_index=True)
        if "ts_code" not in ti.columns:
            print("[lhb seats] WARN: top_inst 缺 ts_code 列 (旧批次), 跳过该源",
                  flush=True)
        else:
            ...
            if parts:
                ...
                print(f"[lhb seats] merged {len(seats):,} rows, "
                      f"{seats['date'].nunique()} dates", flush=True)
    else:
    ...
    try:
        ...
        print(f"[sw cls] mapped: {df['sw_l1_name'].notna().mean():.1%} l1 coverage",
              flush=True)
    except Exception as exc:  # noqa: BLE001
            ...
            if ind in _ind2code:
                ...
                for sw_name, code in _ind2code.items():
                    if ind in sw_name or sw_name in ind:
                        ind_map[ind] = code
                        break
        print(f"[sw idx] industries={len(industries)}, mapped={len(ind_map)}",
              flush=True)
        idx_parts = []
        for code in sorted(set(ind_map.values())):
            try:
                idx = pro.index_daily(ts_code=f"{code}.SI", start_date=d0,
                                      end_date=d1)
            except Exception as exc:  # noqa: BLE001
            ...
            idx = idx.rename(columns={
                "pct_chg": "_pct", "close": "_close", "vol": "_vol",
            })
            idx["sw_ret_1d"] = idx["_pct"] / 100.0
            idx["sw_index_close"] = idx["_close"]
            idx["sw_index_vol"] = idx["_vol"] / 1e6
            idx["_sw_code"] = code
            idx_parts.append(idx[["date", "sw_ret_1d", "sw_index_close",
                                  "sw_index_vol", "_sw_code"]])
            time.sleep(0.12)
        if idx_parts:
            ...
            print(f"[sw idx] merged: sw_ret_1d NaN="
                  f"{df['sw_ret_1d'].isna().mean():.1%}", flush=True)
    except Exception as exc:  # noqa: BLE001
    ...
    cols = ["roe", "sh_net_change_sign", "margin_balance", "bt_count",
            "lhb_inst_buy", "sw_l1_name", "sw_ret_1d"]
