...
np.random.seed(42)
syms_300 = sorted(
    np.random.choice(main_syms, size=min(300, len(main_syms)), replace=False)
)
panel = panel[panel["symbol"].isin(syms_300)]

cleaner = CleaningPipeline()
m, d = cleaner.run_train(panel)
df = m
...
header = "{:<10} {:<6} {:>9} {:>9} {:>9} {:>7}".format(
    "Thresh", "Feats", "Mean IC", "IC_std", "IR", "IC>0%"
)
print("\n" + header)
print("-" * 55)
