...
for k in seg_dates:
    print(
        f"  {k}: {seg_dates[k][0]:%Y-%m-%d}..{seg_dates[k][-1]:%Y-%m-%d} ({len(seg_dates[k])})",
        flush=True,
    )
...
train_keep = (
    np.isin(d, seg_nd_train)
    & ~df["is_suspended"].to_numpy(dtype=bool)
    & df["label_pm_10d_net"].notna().to_numpy()
)
train_dates_sub = sorted(set(seg_dates["train"]))
w_map = {
    dp: 0.5 ** ((len(train_dates_sub) - 1 - i) / 250)
    for i, dp in enumerate(train_dates_sub)
}
...
print(
    f"\n10d_reg EXACT-PROD: trees={m.booster_.num_trees()} best_iteration={m.best_iteration_}",
    flush=True,
)
p = m.predict(X[:2000])
print(
    f"pred sample: std={np.std(p):.6f} min={p.min():.6f} max={p.max():.6f}", flush=True
)
...
m2.fit(
    X, y, eval_set=[(X_es, y_es)], callbacks=[lgb.early_stopping(100, verbose=False)]
)
print(
    f"10d_reg EXACT-PROD noweight: trees={m2.booster_.num_trees()} best_iteration={m2.best_iteration_}",
    flush=True,
)
