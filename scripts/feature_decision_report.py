...
def load_all_groups():
    ...
    for f in files:
        ...
        for t in d.get("top_20", []):
            ...
            if fac not in all_factors or abs(t["ic_5d"]) > abs(
                all_factors[fac]["ic_5d"]
            ):
...
def main():
    ...
    print(
        f"{'Rank':<5s} {'Factor':<42s} {'IC_1d':>8s} {'IC_3d':>8s} {'IC_5d':>8s} {'Grade':>8s} {'Source Dims'}"
    )
    print("-" * 100)
    for i, t in enumerate(ranked[:60], 1):
        dim_str = ",".join(t.get("source_dims", [])[:3])
        print(
            f"{i:<5d} {t['factor']:<42s} {t['ic_1d']:>+8.4f} {t['ic_3d']:>+8.4f} {t['ic_5d']:>+8.4f} {t['grade']:>8s} {dim_str}"
        )
    ...
    print(f"{'=' * 100}")
    print(
        f"{'Dim':<8s} {'Status':>8s} {'Best Feature':<38s} {'IC_5d':>8s} {'Grade':>8s} {'#Factors'}"
    )
    print("-" * 100)
    ...
    for d in dim_results:
        ...
        status = (
            "INCLUDE"
            if d["n_strong"] + d["n_weak"] > 0 and abs(best_ic) >= 0.15
            else "WEAK"
            if d["n_weak"] > 0
            else "DEAD"
        )
        print(
            f"{dims_str:<8s} {status:>8s} {best_fac:<38s} {best_ic:>+8.4f} {best_grade:>8s} {d['n_candidates']}"
        )
    ...
    output = {
        ...
        "dead_count": len(dead),
        "strong": [
            {"factor": t["factor"], "ic_5d": t["ic_5d"], "grade": t["grade"]}
            for t in strong
        ],
        "weak": [
            {"factor": t["factor"], "ic_5d": t["ic_5d"], "grade": t["grade"]}
