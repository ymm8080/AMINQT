...
def run_selection(df, board, label, dry_run=False):
    ...
    if board == "main":
        ...
        logger.info(f"  MAIN: {len(all_feats)} brute-force cols, {len(df):,} sample rows")
    ...
    result = {
        ...
        "pool_size": pool_size, "selected_count": len(selected),
        "params": (
...
def cmd_keep(board):
    ...
    try:
        ...
        print(f"Keeping current: {s.get('active_version', '?')} ({s.get('selected_count', '?')} features)")
    except Exception:
