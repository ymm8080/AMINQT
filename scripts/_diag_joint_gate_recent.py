...
def main() -> None:
    ...
    print(
        f"\n===== T+2/T+3 联合门 vs 纯 T+3 门 (最近 {args.trailing_days} 交易日) =====",
        flush=True,
    )
    for board in BOARDS:
        ...
        print(f"\n[{board}]", flush=True)
        for rank, rlabel in (
            ("by_mag", "按 mag_3d 排序 (并行生产)"),
            ("by_score", "按混合分排序 (legacy 类比)"),
            ("rescue", "救回带 (联合门多留的股)"),
        ):
            ...
            print(
                f"    {'组':<8}{'日':>4}{'均n':>6}{'MFE3d':>9}{'MFE2d':>9}{'涨率3d':>9}{'涨率>0.5%':>10}",
                flush=True,
            )
            ...
            if len(common):
                ...
                print(
                    f"    联合门逐日 MFE3d 赢纯T+3门: {win_days}/{len(common)} 天",
                    flush=True,
                )
