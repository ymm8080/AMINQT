...
ts_df["pct_90_con_derived"] = (ts_df["cost_95pct"] - ts_df["cost_5pct"]) / (ts_df["cost_95pct"] + ts_df["cost_5pct"])
ts_df["pct_70_low"] = ts_df["cost_15pct"]
ts_df["pct_70_high"] = ts_df["cost_85pct"]
ts_df["pct_70_con_derived"] = (ts_df["cost_85pct"] - ts_df["cost_15pct"]) / (ts_df["cost_85pct"] + ts_df["cost_15pct"])
...
print("=" * 80)
print(f"600671 CYQ Comparison: Tushare-derived vs akshare-reported")
print("=" * 80)
...
for d in TARGET_DATES:
    ...
    print(f"  {'90% concentration':<30s}  {ts_con90:>12.6f}  {ak_con90:>12.6f}  {abs(ts_con90-ak_con90):>10.6f}")

    # 90% low/high
    print(f"  {'90% low (price)':<30s}  {tr['cost_5pct']:>12.2f}  {ar['pct_90_low']:>12.2f}  {abs(tr['cost_5pct']-ar['pct_90_low']):>10.2f}")
    print(f"  {'90% high (price)':<30s}  {tr['cost_95pct']:>12.2f}  {ar['pct_90_high']:>12.2f}  {abs(tr['cost_95pct']-ar['pct_90_high']):>10.2f}")

    # 70% concentration
    ts_con70 = tr["pct_70_con_derived"]
    ak_con70 = ar["pct_70_con"]
    print(f"  {'70% concentration':<30s}  {ts_con70:>12.6f}  {ak_con70:>12.6f}  {abs(ts_con70-ak_con70):>10.6f}")

    # 70% low/high
    print(f"  {'70% low (price)':<30s}  {tr['cost_15pct']:>12.2f}  {ar['pct_70_low']:>12.2f}  {abs(tr['cost_15pct']-ar['pct_70_low']):>10.2f}")
    print(f"  {'70% high (price)':<30s}  {tr['cost_85pct']:>12.2f}  {ar['pct_70_high']:>12.2f}  {abs(tr['cost_85pct']-ar['pct_70_high']):>10.2f}")

    # benefit_part
    ts_bp = tr["winner_rate"] / 100
    ak_bp = ar["benefit_part"]
    print(f"  {'benefit_part (winner/100)':<30s}  {ts_bp:>12.6f}  {ak_bp:>12.6f}  {abs(ts_bp-ak_bp):>10.6f}")

    # weight_avg / avg_cost
    print(f"  {'weight_avg vs avg_cost':<30s}  {tr['weight_avg']:>12.2f}  {ar['avg_cost']:>12.2f}  {abs(tr['weight_avg']-ar['avg_cost']):>10.2f}")
