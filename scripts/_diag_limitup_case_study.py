...
COLS = [
    "symbol", "date", "close_hfq", "low_hfq", "volume",
...
def main() -> int:
    ...
    p["probe5"] = p.groupby("symbol")["vr"].transform(
        lambda v: (v > 1.5).rolling(5, min_periods=5).sum()
    ).shift(1)  # 过去 5 日放量天数 (排除 T 日自身)
    ...
    feats = [
        "ret_1d", "ret_5d", "ret_10d", "ret_20d", "vr", "vol20_xr",
        "ent_rank", "gini_rank", "ent_chg40", "shrink10", "probe5", "dd250",
    ]
    n_lim = int(p["label"].sum())
    print(f"== 涨停股 case study (879d dual 池, 涨停样本 n={n_lim}, 基准率 {n_lim/len(p):.3%}) ==")
    ...
    for f in feats:
        ...
        profile[f] = {"limitup_median": round(float(a), 4), "pool_median": round(float(b), 4)}
        arrow = "↑↑" if abs(a - b) > 0.5 * abs(b) else ("↑" if a > b else ("↓" if a < b else "="))
    ...
    for f in top:
        ...
        try:
            bins = pd.qcut(sub[f], 5, labels=["0-20%", "20-40%", "40-60%", "60-80%", "80-100%"])
        except ValueError:
            continue
    ...
    out_path = os.path.join(data_others_path("diag"), "limitup_case_study_20260819.json")
