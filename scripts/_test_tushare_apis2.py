...
try:
    ...
    print(
        f"  OK: {len(df)} rows, {df['ts_code'].nunique() if 'ts_code' in df.columns else 0} symbols"
    )
    if len(df) > 0:
        print(f"  Cols: {list(df.columns)}")
except Exception as e:
...
try:
    df = pro.daily(
        trade_date=TD,
        fields="ts_code,trade_date,pre_close,close,open,high,low,vol,amount",
    )
...
try:
    ...
    print(
        f"  Range query: {len(df)} rows, {df['trade_date'].nunique() if 'trade_date' in df.columns else 0} dates"
    )
    if len(df) > 0:
...
try:
    ...
    while True:
        df = pro.stk_holdertrade(
            start_date="20260701", end_date="20260724", limit=3000, offset=offset
        )
    ...
    print(
        f"  Total: {len(total)} rows, {total['ts_code'].nunique() if 'ts_code' in total.columns else 0} symbols"
    )
    if len(total) > 0:
