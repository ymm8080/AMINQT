...
for sym in test_syms:
    try:
        ...
        raw = pro.fina_indicator(ts_code=ts_code, fields='ts_code,ann_date,end_date,roe,roe_dt,roa,np_margin,gross_margin,eps_yoy,or_yoy,profit_yoy,cf_sales,debt_to_assets,current_ratio,assets_turn,inv_turn,ocf_to_or')
...
if fin_frames:
    ...
    fin = fin.rename(columns={'ann_date':'announce_date','end_date':'report_period','np_margin':'net_margin','or_yoy':'rev_yoy','cf_sales':'op_cf_ratio','debt_to_assets':'debt_ratio','assets_turn':'asset_turnover','inv_turn':'inventory_turnover'})
    fin = fin.loc[:, ~fin.columns.duplicated()]  # dedupe
    for c in ['roe','roe_dt','roa','gross_margin','net_margin','eps_yoy','rev_yoy','profit_yoy','op_cf_ratio','debt_ratio','current_ratio','asset_turnover','inventory_turnover','ocf_to_or']:
        if c in fin.columns: fin[c] = pd.to_numeric(fin[c], errors='coerce')
    if 'announce_date' in fin.columns:
        fin['announce_date'] = pd.to_datetime(fin['announce_date'], format='%Y%m%d', errors='coerce')
        fin_cols = [c for c in fin.columns if c not in ('symbol','ts_code','report_period','ann_date','announce_date')]
        f = fin[['symbol','announce_date']+fin_cols].dropna(subset=['announce_date']).sort_values('announce_date')
        df = df.sort_values('date')
        df = pd.merge_asof(df, f, left_on='date', right_on='announce_date', by='symbol', direction='backward')
logger.info('fina_indicator: %d/%d stocks, %.1fs', len(fin_frames), len(test_syms), time.time()-t0)
...
for sym in df['symbol'].unique():
    ...
    time.sleep(0.03)
if hn_all:
    hn = pd.concat(hn_all, ignore_index=True).dropna(subset=['announce_date']).sort_values('announce_date')
    df = df.sort_values('date')
    df = pd.merge_asof(df, hn, left_on='date', right_on='announce_date', by='symbol', direction='backward')
...
for d in sample_dates:
    ...
    time.sleep(0.3)
if mg_frames:
    mg = pd.concat(mg_frames, ignore_index=True).rename(columns={'rzye':'margin_balance','rqye':'short_balance','rzmre':'margin_buy_amt','rqmcl':'short_sell_vol'})
...
if mf_frames:
    ...
    df = df.merge(mf[['symbol','date','main_money_flow','super_large_order_net']], on=['symbol','date'], how='left')
...
dims_features = {
    ...
    'dim27_indflow': ['ind_north_flow_5d','ind_north_flow_rank','ind_margin_balance_chg','ind_margin_rank','ind_holder_qoq_mean','ind_holder_rank','ind_lhb_net_buy_5d','ind_lhb_activity','ind_flow_composite'],
...
for dim, feats in dims_features.items():
    ...
    for f in feats:
        ...
        dim_results.append({'factor': f, 'ic_1d': round(ic1,5), 'ic_3d': round(ic3,5), 'nan_pct': round(nan_r*100,1)})
    verdict = 'INCLUDE' if best_ic>=0.02 else ('WATCH' if best_ic>=0.01 else 'SKIP')
    results[dim] = {'best_factor': best_f, 'best_ic': round(best_ic,5), 'nan_pct': round(best_nan*100,1), 'verdict': verdict, 'factors': dim_results}
...
with open(f'data/factor_registry/alt_full_eval_{pd.Timestamp.now():%Y%m%d_%H%M}.json', 'w') as fh:
