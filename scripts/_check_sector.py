...
ind_cols = [c for c in df.columns if any(k in c.lower() for k in ["industr", "sector", "sw_", "board"])]
print(f"列: {ind_cols}")
print()
...
if "industry" in df.columns:
    print("=== industry 列分布 ===")
    ind_counts = df.groupby("date")["industry"].apply(lambda x: x.notna().sum() / len(x) * 100)
    print(f"  非空率 (最新5日):")
...
for f in sorted(os.listdir(sec_dir)):
    ...
    print(f"  日期范围: {sdf['date'].min() if 'date' in sdf.columns else '?'} ~ {sdf['date'].max() if 'date' in sdf.columns else '?'}")
