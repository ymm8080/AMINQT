...
def test_akshare_availability():
    ...
    try:
        ...
        try:
            ...
            results["fundamental_pit"] = f"✓ ({len(df)} 行, cols={list(df.columns)[:5]})"
        except Exception as e:
        ...
        try:
            ...
            results["inst_position_ts"] = f"✓ ({len(df)} 行, cols={list(df.columns)[:5]})"
        except Exception as e:
        ...
        try:
            ...
            results["sh_holdings_change"] = f"✓ ({len(df)} 行, cols={list(df.columns)[:5]})"
        except Exception as e:
        ...
        try:
            df = ak.stock_zh_a_gdhs_detail_em(symbol="000001")
            results["sh_count_change"] = f"✓ ({len(df)} 行, cols={list(df.columns)[:5]})"
        except Exception as e:
...
def test_tushare_availability():
    ...
    try:
        ...
        tests = {
            ...
            "fina_indicator": lambda: pro.fina_indicator(ts_code="000001.SZ", period="20251231"),
        ...
        for name, fn in tests.items():
            try:
                df = fn()
                results[name] = f"✓ ({len(df)} 行)" if df is not None and len(df) > 0 else "✓ (空)"
            except Exception as e:
                results[name] = f"✗ {e}"
    except ImportError:
...
def print_decision_matrix():
    ...
    print(f"评估日期: 2026-07-27 | 当前特征维度: 21 (dim01-dim21)")
    print("=" * 140)
    ...
    decisions = [
        ("fundamental_pit",  "基本面PIT",     "T+A", "98%",  ">10y", "季频", "是", "P0-立即",
         "纳入",  "daily_basic→fina_indicator升级,ann_date+20+财务因子"),
        ("main_money_flow",  "主力资金",       "已集成", "99%", ">5y", "日频", "是", "P1-升级",
         "升级",  "东财快照→Tushare moneyflow历史回填,四层资金滚动特征"),
        ("sh_count_change",  "股东户数变化",    "T+A", "95%", ">10y", "季频", "否*", "P0-立即",
         "纳入",  "最强筹码集中度信号,dim20仅OHLCV代理,真实数据IC预期+0.01-0.03"),
        ("avg_shares",       "股东平均股数",    "衍生", "95%", ">10y", "季频", "否*", "P0-自动",
         "纳入",  "从stk_holdernumber自动衍生,零额外API成本"),
        ("northbound",       "北向资金",       "T+A", "15%", ">8y", "日频", "是", "P1-板块限定",
         "纳入",  "仅沪深港通标的,独立训练或条件特征(board过滤)"),
        ("margin",           "融资融券",       "T+A+B","35%", ">14y", "日频", "是", "P1-板块限定",
         "纳入",  "仅两融标的,融资余额增速=高质量情绪信号,独立训练"),
        ("lhb_inst",         "机构龙虎榜",     "T+A", "5%",  ">19y", "日频", "是", "P2-条件",
         "条件纳入","dim18已占位,数据稀疏但信号质量高(机构专用席位买入)"),
        ("sh_holdings_change","股东增减持",     "T+A+B","30%",">10y", "不定期","否*","P2-条件",
         "条件纳入","内部人信号高质量,但稀疏+公告滞后,PIT对齐复杂"),
        ("inst_position_ts", "机构持仓时序",    "T+A", "60%", ">10y", "季频", "否*", "P2-条件",
         "待定",  "季频+45天延迟实时价值有限,先观察IC"),
        ("hot_money_seat",   "游资席位净值",    "需衍生","2%", ">19y", "日频", "是", "P3-观察",
         "暂缓",  "无现成API,需LHB营业部识别游资+净值时序,工程量大"),
    ]

    for (i, name, avail, cov, hist, freq, pit, pri, dec, reason) in decisions:
        print(f"{i:<20s} {name:<12s} {avail:<10s} {cov:<7s} "
              f"{hist:<6s} {freq:<6s} {pit:<5s} {pri:<10s} {dec:<10s} {reason}")

    print("-" * 140)
    print("* PIT 标注: 季频/不定期数据公告滞后, 需严格按 announce_date/公告日 merge_asof, 不可直接用报告期.")
    print()
    print("优先级说明:")
...
def print_feature_preview():
    ...
    dims = {
        "dim22_fundamental_pit (基本面PIT)": [
            "roe", "roa", "gross_margin", "net_margin", "eps_yoy", "rev_yoy",
            "profit_yoy", "op_cf_ratio", "debt_ratio", "current_ratio",
            "asset_turnover", "inventory_turnover", "pe_ttm_pit", "pb_lf_pit",
            "roe_5y_cv", "rev_3y_cagr", "accrual_ratio",
            "→ 17 因子, 季频 + merge_asof PIT 对齐到日频",
        ],
        "dim23_shareholder_structure (股东结构)": [
            "holder_count", "holder_count_qoq", "holder_count_yoy",
            "holder_count_qoq_accel",  # 二阶导: 减少速度加快=筹码加速集中
            "avg_shares_per_holder", "avg_shares_qoq", "avg_shares_yoy",
            "holder_concentration_zscore",  # 截面标准化后的集中度
            "→ 8 因子, 季频 → 日频 merge_asof",
        ],
        "dim24_margin_trading (融资融券)": [
            "margin_balance", "margin_balance_chg_1d", "margin_balance_chg_5d",
        ...
        "dim25_northbound (北向资金)": [
            "north_net_buy", "north_net_buy_5d", "north_net_buy_20d",
            "north_consecutive_buy_days",  # 连续净买入天数
            "north_buy_ratio",  # 北向买入额/成交额
            "north_hold_pct", "north_hold_pct_chg_5d",  # 北向持股占比及变化
            "→ 7 因子, 日频 (仅沪深港通标的)",
        ],
        ...
        "dim10_upgraded (资金流升级)": [
            "super_large_net_5d", "large_net_5d", "medium_net_5d", "small_net_5d",
...
if __name__ == "__main__":
    ...
    p.add_argument("--train", action="store_true", help="独立训练各数据源评估 IC (需完整面板)")
    ...
    if not any([args.fetch, args.train, args.all]):
        ...
        print()
        print("用法:")
        print("  python scripts/research_alternative_data.py              # 打印决策矩阵")
        print("  python scripts/research_alternative_data.py --fetch      # 测试数据源可用性")
        print("  python scripts/research_alternative_data.py --train      # 独立训练评估 IC")
