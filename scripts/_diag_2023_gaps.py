...
for half in ["H1", "H2"]:
    ...
    print(f"  2023 {half}: {len(sub):>8,} rows, roe NaN={sub['roe'].isna().sum():>7,} ({na:.1f}%)")
...
print(f"  H1 rows per date (top 5):")
...
for s in zero_cov:
    ...
    print(f"  {s}: {len(sub)} rows in 2023, dates {sub['date'].min()} ~ {sub['date'].max()}")

# Check the new cache for these stocks
cache = pd.read_parquet("data/supply_cache/alt_data/fina_indicator/all_20230101_20260728.parquet")
...
for s in zero_cov:
    ...
    if len(sub) > 0:
        ...
        print(f"  {s}: {len(sub)} rows, roe non-NaN={roe_non_nan}, earliest ann={sub['ann_dt'].min()}, earliest roe={sub.sort_values('ann_dt')['roe'].iloc[0]}")
    else:
