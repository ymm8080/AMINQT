...
cols = [
    "chip_concentration", "profit_ratio", "free_float_turnover_rate",
    "pct_70_con_x", "pct_90_con_x", "pct_70_con_y", "pct_90_con_y",
    "winner_ratio_x", "winner_ratio_y", "turnover_rate",
]
...
if "chip_concentration" in df.columns and "pct_90_con_x" in df.columns:
    ...
    print(f"  chip_concentration: NaN={df['chip_concentration'].isna().mean()*100:.2f}%, range=[{df['chip_concentration'].min():.6f}, {df['chip_concentration'].max():.6f}]")
    print(f"  pct_90_con_x:       NaN={df['pct_90_con_x'].isna().mean()*100:.2f}%, range=[{df['pct_90_con_x'].min():.6f}, {df['pct_90_con_x'].max():.6f}]")
...
if "profit_ratio" in df.columns and "winner_ratio_x" in df.columns:
    ...
    print(f"  profit_ratio:    NaN={df['profit_ratio'].isna().mean()*100:.2f}%, range=[{df['profit_ratio'].min():.6f}, {df['profit_ratio'].max():.6f}]")
    print(f"  winner_ratio_x:  NaN={df['winner_ratio_x'].isna().mean()*100:.2f}%, range=[{df['winner_ratio_x'].min():.6f}, {df['winner_ratio_x'].max():.6f}]")
    print(f"  Both non-null: {len(both):,}")
    if len(both) > 0:
        ...
        print(f"  profit_ratio vs winner_ratio*100: mean_diff={diff100.mean():.6f}, max_diff={diff100.max():.6f}")
...
if "free_float_turnover_rate" in df.columns and "turnover_rate" in df.columns:
    ...
    print(f"  free_float_turnover_rate: NaN={df['free_float_turnover_rate'].isna().mean()*100:.2f}%")
    print(f"  turnover_rate:           NaN={df['turnover_rate'].isna().mean()*100:.2f}%")
    print(f"  Both non-null: {len(both):,}")
    if len(both) > 0:
