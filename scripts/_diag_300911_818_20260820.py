...
def main() -> int:
    ...
    df = d.to_table(
        filter=(ds.field("board").isin(["GEM", "STAR"]))
    ...
    if len(r):
        ...
        for i, row in ranked.head(10).iterrows():
            log(
                f"    {int(i) + 1:>2} {row['symbol']}  ret={row['pred_ret_10d']:+.4f}  prob={row['compound_prob']:.3f}"
            )
        report["top10"] = [
            {"rank": int(i) + 1, "symbol": s, "pred_ret_10d": float(p)}
            for i, (s, p) in enumerate(
                zip(ranked.head(10)["symbol"], ranked.head(10)["pred_ret_10d"])
            )
        ]
    else:
    ...
    if len(r):
        pb = load_prob_bundle_live_on(
            Path(DATA_DIR / "prob_head_legacy"), "dual", pd.Timestamp("2026-08-20")
        )
        if pb is None:
            log("[7] 找不到 ≤8/20 legacy 概率头 bundle → 跳过")
        else:
            log(
                f"[7] legacy 概率头: {pb[0].name} (trained_through={pb[1]['trained_through']})"
            )
            b = pb[1]
            ...
            ok = p911 > gate
            log(
                f"[7] 300911 概率头 prob={p911:.3f} vs 闸={gate:.3f} → {'PASS' if ok else 'FAIL'}"
            )
    ...
    if actual_path.exists():
        ...
        report["actual_818_list"] = {
            "n": int(len(act)),
            "symbols": act["symbol"].tolist(),
        }
        ov = set(act["symbol"]) & set(ranked["symbol"]) if len(ranked) else set()
        log(
            f"[8] 8/18 实际清单 {len(act)} 只 (module 20260818); 与 20260820 模块重放交集 {len(ov)} 只"
        )
    ...
    out_dir = Path(BACKTEST_RESULT_DIR) / (
        "_diag_300911_818_20260820_" + time.strftime("%Y%m%d_%H%M%S")
    )
