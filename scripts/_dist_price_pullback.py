...
def _dist(ser, label):
    ...
    print(
        f"    {label:<20s} n={n:>6d} 上涨率={hit:5.1f}% 均值={ser.mean() * 100:+6.2f}% "
        f"中位={p[2] * 100:+6.2f}% p10={p[0] * 100:+6.2f}% p90={p[4] * 100:+6.2f}%"
    )
    print(
        f"                        max赚={ser.max() * 100:+6.1f}% max亏={ser.min() * 100:+6.1f}% "
        f"top10%均值={top10_mean * 100:+6.2f}% (其余90%均值={rest_mean * 100:+6.2f}%)"
    )
    return {
        "n": n,
        "hit": round(hit, 2),
        "mean": round(ser.mean() * 100, 3),
        "median": round(p[2] * 100, 3),
        "p10": round(p[0] * 100, 3),
        "p90": round(p[4] * 100, 3),
        "top10_mean": round(top10_mean * 100, 3),
        "rest90_mean": round(rest_mean * 100, 3),
    }
...
def main():
    ...
    print("=" * 78)
    print("  [主板] 交易分布")
    print("  --- 条件: 涨停后回调满月(22~28日,<-2%), 每周期首次入场 ---")
