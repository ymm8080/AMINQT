...
SKIP = {'symbol','date','board','industry','name','tradestatus','time',
        'open','high','low','close','pre_close','volume','amount',
        'open_hfq','high_hfq','low_hfq','close_hfq',
        'is_suspended','is_st','list_days','announce_date',
        'limit_pct','PE_TTM','touched_limit_up','score_rank',
        'rank_amount','rank_ff_turnover','liquidity_score',
        'churn_suspect','is_virtual','price_1455','adv20',
        'schema_version','market_state','free_float_turnover_rate'}

rng = np.random.RandomState(42)

for bv, bn in [('main','main'), ('GEM','dual')]:
    mask = recent['board']==bv if bn=='main' else recent['board'].isin(['GEM','STAR'])
    ...
    raw_cols = [c for c in df.columns if c not in SKIP and df[c].dtype in ('float64','float32','int64','int32')]
    ...
    for col in raw_cols:
        ...
        try:
            ...
            results.append({'factor':col, 'IC':round(ic,4), 'IR':round(ir,2), 'IC_std':round(ic_std,4), 'pos':round(pos,3), 'nan':round(nan,3)})
        except: pass
    ...
    print(f'\n{"=" * 100}')
    print(f'  {bn.upper()} — ALL {len(ranked)} FEATURES: IC + IR (Information Ratio) per feature')
    print(f'{"=" * 100}')
    print(f'  {"Rank":<4s} {"Column":<30s} {"IC":>8s} {"IR":>8s} {"IC_std":>8s} {"Pos%":>7s} {"NaN%":>7s}')
    print(f'  {"-" * 80}')
    for i, t in enumerate(ranked, 1):
        if i > 50: break
        print(f'  {i:<4d} {t["factor"]:<30s} {t["IC"]:>+8.4f} {t["IR"]:>+8.2f} {t["IC_std"]:>8.4f} {t["pos"]:>7.1%} {t["nan"]:>7.1%}')
