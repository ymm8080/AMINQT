...
def arr(seg_d, lbl):
    m = np.isin(d, np.array(seg_d).astype("datetime64[ns]")) & df[lbl].notna().to_numpy()
    ix = np.flatnonzero(m)
    X = np.nan_to_num(df.iloc[ix].loc[:, cols].to_numpy(np.float32), nan=0.0)
    return X, df.iloc[ix][lbl].to_numpy(), df.iloc[ix]["date"].to_numpy(), df.iloc[ix]["symbol"].to_numpy()

Xtr, ytr, dtr, _ = arr(train_d, lbl_cls)
Xes, yes, _, _ = arr(es_d, lbl_cls)
# test 用单一行掩码 (cls 标签非空), 同时取回归标签做 rank IC 参照
mte = np.isin(d, np.array(test_d).astype("datetime64[ns]")) & df[lbl_cls].notna().to_numpy() & df[lbl_reg].notna().to_numpy()
...
def fit_cls(X, y, Xes, yes):
    ds = lgb.Dataset(X, y)
    bst = lgb.train(
        dict(objective="binary", learning_rate=0.05, num_leaves=15, seed=42, verbosity=-1),
        ds, valid_sets=[lgb.Dataset(Xes, yes)], num_boost_round=1000,
...
def metrics(name, prob, te_dates, te_syms, te_ret):
    ...
    daily = f.groupby("d").apply(lambda g: spearmanr(g["p"], g["r"])[0], include_groups=False)
    p50 = f.groupby("d")["p"].apply(lambda s: np.percentile(s, 50))
    ...
    print(f"  [{name}] trees={bst.num_trees() if name.startswith('修复') else '部署'} "
          f"prob_std={prob.std():.5f} | 分位90-10={spread:.4f} "
          f"| rankIC={daily.mean():.4f} | TOP20命中={top_hit.mean():.3f} vs 池={pool_hit.mean():.3f}", flush=True)
