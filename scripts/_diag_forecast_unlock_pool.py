...
FLOAT_PATH = (
    DATA_DIR / "supply_cache" / "alt_data" / "share_float" / "share_float_full.parquet"
)
...
def print_trajectory(out: list, piv: pd.DataFrame) -> None:
    ...
    for a, b in [
        ...
        s = (
            piv.loc[:, a : b if a != b else a].mean(axis=1).dropna()
            if a != b
            else piv[a].dropna()
        )
        if not len(s):
        ...
        wr = (s > 0).mean()
        out.append(
            f"  [{a:+d},{b:+d}]{'':<6}{_pct(s.mean()):>10}{_pct(wr):>8}{len(s):>9,}"
        )
...
def print_strata(
    ...
    out.append(
        f"  {'分组':<24}{'n':>7}{'T+2':>9}{'T+3':>9}{'T+5':>9}{'T+10':>9}{'T+20':>9}"
    )
    for tag, sel in strata:
        ...
        for h in HORIZONS:
            s = (
                piv.loc[piv.index.isin(eids), h].dropna()
                if h in piv
...
def main() -> int:
    ...
    out.append(
        "类型分布: "
        + ", ".join(f"{t}={n:,}" for t, n in fc["type"].value_counts().items())
    )
    ...
    if len(ev_fc) >= 200:
        ...
        print_strata(
            ...
            [
                ...
                (
                    "正向|main",
                    (ev_fc["dir_group"] == "positive") & (ev_fc["board"] == "main"),
                ),
                (
                    "正向|dual",
                    (ev_fc["dir_group"] == "positive") & (ev_fc["board"] == "dual"),
                ),
                (
                    "负向|main",
                    (ev_fc["dir_group"] == "negative") & (ev_fc["board"] == "main"),
                ),
                (
                    "负向|dual",
                    (ev_fc["dir_group"] == "negative") & (ev_fc["board"] == "dual"),
                ),
            ],
        )
