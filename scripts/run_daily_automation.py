...
_STEPS = {
    ...
    # THS?????? (2026-09-10): ????, WORM?bull_{date}.parquet + bear_counts.csv
    "ths_signal": ["scripts/_fetch_ths_signal.py"],
...
def plan_steps(
    ...
    *,
    ...
    # ??????? pct_70_con (?? 0.05 ?????), ??????; ??? (??)
    steps.append("cyq")
    # THS?????? (2026-09-10): ??????/???WORM??, ??enrich ths_signal?;
    # ??? (WAF/cookie?? exit 2) ????????, ????
    steps.append("ths_signal")
