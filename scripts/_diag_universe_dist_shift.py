...
frames = []
for f in files:
    d = pd.read_parquet(f, columns=["symbol", "trade_date", "close", "high", "low", "pre_close", "amount"])
    frames.append(d)
df = pd.concat(frames, ignore_index=True)
df["amount"] = df["amount"] * 1000.0  # 千元→元
df["date"] = pd.to_datetime(df["trade_date"], format="%Y%m%d")
print(f"daily 载入 {df.shape[0]} 行, 日期 {df['date'].min().date()} ~ {df['date'].max().date()}")
...
stats = pd.DataFrame({
    ...
    "atr_pct": g.apply(lambda s: ((s["high"] - s["low"]) / s["pre_close"]).mean(), include_groups=False),
...
for b in ["main", "dual"]:
    for g in ["old", "new"]:
        ...
        print(f"  {b:>4} {g:>3}: {len(sub):>4}只 中 {(sub['med_amount'] < 5e7).mean()*100:.1f}% 低于5000万")
...
print(f"  old dual: {len(old_dual)}只 top200门槛={old_dual['med_amount'].iloc[199]/1e8:.2f}亿")
print(f"  new dual: {len(new_dual)}只 全进池需要的门槛={new_dual['med_amount'].iloc[199]/1e8:.2f}亿")
print(f"  合并池: {len(all_dual)}只 top200门槛={all_dual['med_amount'].iloc[199]/1e8:.2f}亿")
...
print(f"(新股在合并dual池的最低名次={list(all_dual.index).index(top200[top200['grp']=='new'].index.min())+1 if len(top200[top200['grp']=='new']) else '无'})")

print("\n=== 4.3 dual 20% 流动性过滤 (分位闸) ===")
cut = all_dual["med_amount"].quantile(0.20)
new_frac_in_bottom20 = (new_dual["med_amount"] <= cut).mean()
print(f"  合并池 20% 分位={cut/1e8:.2f}亿 | 新dualk 落入底部20%占比: {new_frac_in_bottom20*100:.1f}%")

print("\n=== 4.4 T+10 池级收益分布 (选股/验收闸基准) ===")
t10_all = df[df["board"] != "bse"].groupby(["grp", "board"])["_t10"].apply(lambda s: s.dropna())
for (g, b), s in t10_all.groupby(level=[0, 1]):
    print(f"  {b:>4} {g:>3}: n={len(s):>7} mean={s.mean():+.4f} median={s.median():+.4f} P10={s.quantile(.1):+.4f} P25={s.quantile(.25):+.4f} P75={s.quantile(.75):+.4f} P90={s.quantile(.9):+.4f} pos={(s>0).mean()*100:.1f}%")

# ── 5. 次新 gate (全量876天日历) ──
all_files = sorted(glob.glob(rf"{DAILY_DIR}\daily_*.parquet"))
cal_all = pd.DatetimeIndex(sorted(pd.to_datetime(
    pd.read_parquet(all_files[0], columns=["trade_date"])["trade_date"], format="%Y%m%d").unique()))
...
new["traded_since"] = new["list_dt"].map(lambda d: len(cal) - cal.searchsorted(d) if pd.notna(d) else np.nan)
