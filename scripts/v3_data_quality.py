...
valid = df.dropna(subset=["cost_5pct","cost_15pct","cost_50pct","cost_85pct","cost_95pct"])
n_valid = len(valid)
mono = (
...
violations = (~mono).sum()
print(f"  cost_5 ≤ 15 ≤ 50 ≤ 85 ≤ 95:  {'PASS' if violations==0 else f'FAIL ({violations:,}/{n_valid:,})'}")
...
if bp_oob_01 == 0:
    ...
    print(f"    ⚠ benefit_part is [0,100] not [0,1] — fetch_cyq_remaining stored winner_rate directly")
else:
...
print(f"  pct_70_con in [0,1]:         {'PASS' if p70_oob==0 else f'FAIL ({p70_oob:,})'}")

# 5d. pct_90_con in [0, 1]
p90 = df["pct_90_con"].dropna()
p90_oob = ((p90 < 0) | (p90 > 1)).sum()
print(f"  pct_90_con in [0,1]:         {'PASS' if p90_oob==0 else f'FAIL ({p90_oob:,})'}")
...
print(f"  pct_70_low ≤ pct_70_high:    {'PASS' if v70_fail==0 else f'FAIL ({v70_fail:,})'}")
...
print(f"  pct_90_low ≤ pct_90_high:    {'PASS' if v90_fail==0 else f'FAIL ({v90_fail:,})'}")
...
for cat, cols in categories.items():
    ...
    if present:
        ...
        print(f"  {cat:<20s}  avg_nan={avg_nan:5.2f}%  worst={worst:.2f}% ({worst_col})")
print()
...
show = ["date","close","benefit_part","cost_5pct","cost_15pct","cost_50pct","cost_85pct","cost_95pct",
        "pct_70_con","pct_90_con","weight_avg"]
