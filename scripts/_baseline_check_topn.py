...
PASSED = ["VAR51", "amihud_illiq", "amihud_illiquidity", "down_gap_pct",
...
def main():
    ...
    for k in HORIZONS:
        ...
        print(f"  全截面:   幅度={overall_mag:+.4f} 胜率={overall_wr:.1%} n={len(v):,}", flush=True)
        print(f"  日期等权: 幅度={date_mag:+.4f} 胜率={date_wr:.1%} (平均每日期截面均值为正的天数占比)", flush=True)
    ...
    for col in PASSED:
        ...
        for k in HORIZONS:
            ...
            mag = float(v.mean())
            wr = float((v > 0).mean())
            base_mag, base_wr = float(work[LABEL_COLS[k]].dropna().mean()), \
                float((work[LABEL_COLS[k]].dropna() > 0).mean())
            d_mag = mag - base_mag
            d_wr = wr - base_wr
            row += f"  T+{k} mag={mag:+.2%}(Δ{base_mag:+.2%}) wr={wr:.1%}(Δ{base_wr:+.1%})"
