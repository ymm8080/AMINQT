...
def make_panel(symbols=("600519", "601318", "600000"), days=120, seed=9) -> pd.DataFrame:
...
class TestBacktestProtocol:
    def test_run_and_metrics_keys(self):
        ...
        m = result["metrics"]
        for key in ("total_return", "annual_return", "net_excess_annual",
                    "max_drawdown", "sharpe"):
            assert key in m
    ...
    def test_limit_up_no_buy(self):
        ...
        lists = {panel["date"].unique()[0]: pd.DataFrame(
            {"symbol": ["600519"], "score": [1.0], "prob_up": [0.6], "industry": ["白酒"]})}
...
class TestParamTuner:
    def test_grid_search_and_report(self, tmp_path):
        ...
        report = tuner.grid_search(["max_hold_days", "prob_exit"], oos_ratio=0.3, top_k=3)
    ...
    def test_apply_to_config(self):
        cfg = Config()
        ParamTuner.apply_to_config({"max_hold_days": 4, "prob_exit": 0.55,
                                    "not_a_param": 999}, cfg)
