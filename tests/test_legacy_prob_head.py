"""Tests for app/pipeline1/prob_head legacy 并行式概率闸 (2026-08-15 代码先行).

配方镜像 app/pipeline_parallel/prob_head (250d OOS 定案), 差异=数据流: legacy 无
stage 特征检查点 → 特征/面板尾由调用方传入 (gate_probabilities 收 feat_day/tail/
panel_dates). 铁律覆盖:
...
def test_gate_probabilities_stale_bundle_failopen(monkeypatch):
    ...
    fake_bundle = {"trained_through": "2026-01-02"}
    monkeypatch.setattr(prob_head, "load_latest", lambda board: fake_bundle)
    panel_dates = pd.to_datetime(
        pd.date_range("2026-06-01", periods=80, freq="B")
    ).to_numpy("datetime64[ns]")
    feat_day = pd.DataFrame({"symbol": ["A"]})
    tail = pd.DataFrame()
    # 年龄 ≈ 80-20 > 42 → 闸失效
    assert prob_head.gate_probabilities("main", feat_day, tail, panel_dates) is None
...
def test_apply_prob_gate_drop_and_failopen(monkeypatch):
    ...
    def fake(board, feat_day, tail, panel_dates):
...
def test_apply_prob_gate_missing_feats_keeps_symbol(monkeypatch):
    """缺当日特征截面 (feats 无该板) → fail-open 保留, 不杀清单."""
    res = pd.DataFrame({"board": ["main", "dual"], "symbol": ["A", "B"]})
    monkeypatch.setattr(
        prob_head,
        "gate_probabilities",
        lambda board, feat_day, tail, panel_dates: (
            pd.Series({"A": 0.60}),
            0.20,
        ),
    )
    feats = {"main": pd.DataFrame()}  # dual 无截面
    panel_dates = pd.to_datetime(
        pd.date_range("2026-06-01", periods=80, freq="B")
    ).to_numpy("datetime64[ns]")
    out = prob_head.apply_prob_gate(res, feats, pd.DataFrame(), panel_dates)
...
def test_apply_prob_gate_disabled_returns_unchanged(monkeypatch):
    ...
    monkeypatch.setitem(prob_head.LEGACY_PROB_GATE, "enable", False)
