...
def _payload(overrides=None, trained_through="2026-09-12"):
    return {
        ...
        "overrides": overrides if overrides is not None else {"5d_cls": {"num_leaves": 63}},
        "evidence": {"sweep": "test", "judge": "TOP10 实净"},
    }


def test_save_worm_same_ts_suffix(tmp_path):
    p1 = param_source.save_param_source("main", _payload(), directory=tmp_path, ts="20260913_0200")
    p2 = param_source.save_param_source("main", _payload(), directory=tmp_path, ts="20260913_0200")
    assert p1 != p2
    assert json.loads(p1.read_text(encoding="utf-8"))["overrides"]["5d_cls"]["num_leaves"] == 63


def test_load_latest_and_board_isolation(tmp_path):
    param_source.save_param_source("main", _payload(), directory=tmp_path, ts="20260913_0200")
    param_source.save_param_source(
        "main", _payload({"10d_cls": {"num_leaves": 7}}), directory=tmp_path, ts="20260913_0300"
    )
...
def test_resolve_no_json_returns_empty(tmp_path):
    assert param_source.resolve_param_override("main", "5d_cls", directory=tmp_path) == {}
...
def test_resolve_stale_json_falls_back_code_table(tmp_path):
    param_source.save_param_source(
        "main", _payload(trained_through="2026-06-01"), directory=tmp_path, ts="20260601_0200"
    )
    assert (
        param_source.resolve_param_override(
            "main", "5d_cls", as_of="2026-09-13", directory=tmp_path, max_stale_days=45
...
def test_resolve_missing_trained_through_falls_back(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("config.settings.LEGACY_PARAM_SOURCE", {"main": "auto", "dual": "auto"})
    param_source.save_param_source(
        "main", {"board": "main", "overrides": {"5d_cls": {"num_leaves": 63}}},
        directory=tmp_path,
        ts="20260913_0200",
    )
    assert (
        param_source.resolve_param_override("main", "5d_cls", as_of="2026-09-13", directory=tmp_path)
        == {}
    )
...
def test_model_params_knob_code_restores_code_table(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("config.settings.LEGACY_PARAM_SOURCE", {"main": "code", "dual": "auto"})
    param_source.save_param_source("main", _payload(), directory=tmp_path, ts="20260913_0200")
