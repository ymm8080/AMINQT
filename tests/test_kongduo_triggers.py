...
def test_no_lookahead_tail_mutation_does_not_change_history():
    ...
    cols = [
        c
        for c in ("duo", "kong", "gap", "pct", "r20", "r60", "r120", "wr_rise60", "pb5",
                  "band20", "kong10", "d_since_kong", "ma10up", "ext10", "ext10p", "vr",
                  "sl_flip_age", "sl_wash_days", "T1", "T2", "T3")
        if c in fb.columns
    ]
    pd.testing.assert_frame_equal(fb.loc[:cut, cols], fd.loc[:cut, cols], check_dtype=True)
...
def _trigger_frame(rows: list[dict]) -> pd.DataFrame:
    ...
    base = {
        "kong": False, "kong10": False, "duo": False,
        "r20": np.nan, "winner_ratio": np.nan, "wr_rise60": np.nan, "ma10up": False,
        "ext10": 1.0, "ext10p": 1.0, "pct": 0.0, "gap": 0.0, "pb5": 0.0,
    }
    return pd.DataFrame([{**base, **r} for r in rows])


_T1_OK = {
    "kong": True, "r20": 0.10, "winner_ratio": 0.70, "wr_rise60": 0.10,
    "ma10up": True, "ext10": 1.0,
}


def test_t1_thresholds_and_edges():
    df = kt.compute_triggers(_trigger_frame([
        _T1_OK,                                              # 0 fire
        {**_T1_OK, "winner_ratio": GENIOUS["wr_min"]},        # 1 == wr_min → 不触发 (strict >)
        {**_T1_OK, "winner_ratio": 0.61},                    # 2 fire
        {**_T1_OK, "wr_rise60": GENIOUS["wr_rise_min"]},      # 3 == 阈值 → 不触发
        {**_T1_OK, "ext10": GENIOUS["t1_ma10_dev_max"]},      # 4 == 1.05 → fire (<=)
    ...
    assert df["T1"].tolist() == [True, False, True, False, True, False, False, False, False]


def test_t2_thresholds_and_edges():
    df = kt.compute_triggers(_trigger_frame([
        {"kong10": True, "duo": True, "pct": 0.03},          # 0 fire
        {"kong10": True, "duo": True, "pct": GENIOUS["t2_gain_min"]},  # 1 == 2% → 不触发
        {"kong10": True, "duo": True, "pct": 0.021},         # 2 fire
        {"kong10": False, "duo": True, "pct": 0.03},         # 3 前10日无空图标 → 不触发
...
def test_t3_thresholds_and_edges():
    df = kt.compute_triggers(_trigger_frame([
        ...
        {"gap": 1.0, "pb5": GENIOUS["t3_pb5_max"], "pct": 0.06},             # 1 == -3% → fire (<=)
        {"gap": 1.0, "pb5": -0.029, "pct": 0.06},                           # 2 回撤不足 → 不触发
        {"gap": -1.0, "pb5": -0.05, "pct": 0.06},                           # 3 空头态 → 不触发
        {"gap": 1.0, "pb5": -0.05, "pct": GENIOUS["t3_gain_min"]},           # 4 == 5% → 不触发
        {"gap": 1.0, "pb5": -0.05, "pct": 0.06, "ext10p": 1.06},            # 5 昨日乖离超标 → 不触发
    ]))
...
def test_trigger_name_is_mutually_exclusive_for_t1():
    ...
    df = kt.compute_triggers(_trigger_frame([
        ...
        {"kong10": True, "duo": True, "pct": 0.06, "gap": 1.0, "pb5": -0.05},  # T2+T3
    ]))
...
def test_assign_layers_each_segment():
    df = _layer_frame([
        ...
        {"T3": True, "r60": -0.40, "vr": GENIOUS["vr_quiet"] + 0.1},           # vr 超缩量闸 → 非 CH3
...
def test_t1_rest_uses_wide_gate():
    ...
    df = _layer_frame([
        {"T1": True, "r120": GENIOUS["t1_wide_r120_max"] + 0.01, "r20": 0.0},  # r120 超宽闸 → 弃
        {"T1": True, "r120": 0.0, "r60": GENIOUS["t1_wide_r60_min"] - 0.01},   # r60 超宽闸 → 弃
        {"T1": True, "r120": 0.0, "r60": GENIOUS["t1_wide_r60_min"]},          # 边界含 → T1余
    ])
...
def test_assign_layers_mutual_exclusivity_and_precedence():
    ...
    df = _layer_frame([
        {"T2": True, "T3": True, "r60": -0.40, "vr": 1.0, "pct": 0.03},  # 双触发 → CH3
        {"T2": True, "T3": True, "r60": 0.00, "pct": 0.10, "vr": 1.0},   # 双触发非深跌 → T2 臂先占
        {"T2": True, "T3": True, "r60": -0.10, "ext10p": 0.90, "pct": 0.03},  # → CH2B
    ])
...
def test_band_requires_trigger_and_drops_untuned_gap():
    ...
    df = _layer_frame([
        ...
        {"T2": True, "pct": 0.06, "vr": 3.0, "r120": -0.5},        # 涨幅 5-9% 且放量 → 无层
...
def test_build_delivery_splits_sheets_and_ranks():
    ...
    df = _layer_frame([
        ...
        {"T1": True, "r120": 0.10, "r20": 0.00, "date": "20260911", "symbol": "000003"},
        {"T3": True, "r60": -0.05, "pct": 0.10, "vr": 1.0, "date": "20260911", "symbol": "000004"},
        {"T1": True, "r120": -0.20, "r20": 0.10, "date": "20260910", "symbol": "000005"},
    ])
...
def test_sheet2_ranks_unrun_first_and_truncates():
    ...
    rows = [
        ...
        {"T3": True, "r60": 0.15, "r120": 0.0, "pct": 0.10,
         "band20": 0.90, "ext10": 1.20, "winner_ratio": 0.95,
         "date": "20260911", "symbol": "000001"},
        # 未启动: 带宽窄 / 未偏离 / 获利盘低 / 跌得比填充行深 (但不到 CH3 的 r60<=-30)
        {"T3": True, "r60": -0.20, "r120": -0.20, "pct": 0.10,
         "band20": 0.05, "ext10": 0.95, "winner_ratio": 0.10,
         "date": "20260911", "symbol": "000002"},
    ]
    # 填充到超过 top_n (T1余: r120<=0.40 且 r60>=-0.05)
    for i in range(top_n + 3 - len(rows)):
        rows.append({
            "T1": True, "r60": 0.0, "r120": 0.0, "r20": 0.0, "pct": 0.0,
            "band20": 0.10 + 0.001 * i, "ext10": 1.0, "winner_ratio": 0.5,
            "date": "20260911", "symbol": f"9{i:05d}",
        })
...
def test_sheet2_sort_key_switch_orders_by_sl_flip(monkeypatch):
    ...
    df = _layer_frame([
        {"T1": True, "r120": 0.0, "r20": 0.0, "sl_flip_age": 0.0, "sl_wash_days": 30.0,
         "date": "20260911", "symbol": "000001"},
        {"T1": True, "r120": 0.0, "r20": 0.0, "sl_flip_age": 9.0, "sl_wash_days": 5.0,
         "date": "20260911", "symbol": "000002"},
        {"T1": True, "r120": 0.0, "r20": 0.0, "sl_flip_age": np.nan, "sl_wash_days": np.nan,
         "date": "20260911", "symbol": "000003"},
    ])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("603421", "20260908", kt.BAND_T2_WARM, "T2"),      # 翻转 + 温火 (r60 仅 -9% 不进冠军段)
    ],
)
...
def test_real_panel_no_ohlcv_corruption(real_day):
    ...
    d = real_day
    bad = d[
        (d["high"] < d["low"]) | (d["high"] < d["close"]) | (d["low"] > d["close"])
        | (d["volume"] < 0)
    ]
...
def test_sl_flip_state_machine_matches_independent_derivation(real_day):
    ...
    for sym in d["symbol"].unique()[:12]:
        ...
        assert s["sl_flip_age"].to_numpy() == pytest.approx(np.array(want_age), nan_ok=True)
        assert s["sl_wash_days"].to_numpy() == pytest.approx(np.array(want_wash), nan_ok=True)
        checked += 1
    ...
    assert (flips["sl_wash_days"] >= 1).all(), "翻正当日洗盘天数必须 >=1 (至少洗过 1 天)"
