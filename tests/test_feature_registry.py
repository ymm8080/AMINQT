...
class TestFeatureRegistryUpdateFromScreen:
    ...
    def test_update_from_screen_basic(self):
        ...
        reg = FeatureRegistry(path="/nonexistent/test.json")
        reg.register_new(
            "MACD", {"dim_group": "dim01", "active": True, "grade": "unknown"}
        )
        reg.register_new(
            "RSI", {"dim_group": "dim01", "active": True, "grade": "unknown"}
        )
        reg.register_new(
            "dead_factor", {"dim_group": "dim02", "active": True, "grade": "unknown"}
        )
