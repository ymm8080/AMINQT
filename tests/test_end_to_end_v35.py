...
from app.rules.rule_engine import (Action, Candidate, PortfolioState, Position,
                                   RuleEngine, Tick)
...
def chain(tmp_path_factory):
    ...
    feed = CompositeFeed(
        ...
        chip=ChipFeed({"600519": ChipDistribution().build(df.copy(), float_shares=1e8)}),
        capital=None,
...
class TestEndToEndV35:
    def test_full_workflow(self, chain):
        ...
        cands = [Candidate(str(r["symbol"]), "白酒", float(r["pred_ret_1d"]),
                           float(r["prob_up"]), turnover_today=12,
        ...
        for t, px in zip(times, seq):
            out = eng.on_tick(2, {sym: Tick(t, px, volume=1.6, turnover=8,
                                            big_order_net=5e6)},
                              cands, pf, {sym: 100.0}, 0.62, [])
            orders += out
        assert any(o.action == Action.BUY for o in orders)     # 形态买入成交
        assert sym in pf.bought_today

        # ── 4. 持仓 + 卖出状态机 (P1 硬止损) ──
        pf.positions[sym] = Position(sym, "白酒", cost=99.4, weight=0.10,
                                     buy_day=1, sellable=True)
        out = eng.on_tick(3, {sym: Tick("10:00", 95.0, volume=1.0)},
                          [], pf, {sym: 99.4}, 0.5, [])
