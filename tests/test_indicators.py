...
from app.rules.rule_engine import Candidate, PortfolioState, RuleEngine, Tick, Action
...
class TestZhuliLasheng:
    def test_outputs_and_no_nan_tail(self):
        ...
        for col in ("主力轨迹", "MAZL", "吸筹", "洗盘", "拉高", "出货", "吸筹峰", "上方死叉出货"):
            assert col in df.columns, col
        assert df["主力轨迹"].iloc[-1] == pytest.approx(df["主力轨迹"].iloc[-1])  # 非 NaN
...
class TestChipDistribution:
    ...
    def test_rally_profit_ratio_high(self):
        ...
        df = pd.DataFrame({
            "time": dates.strftime("%Y%m%d"), "date": dates,
...
class TestCompositeFeedIntegration:
    def _feed(self):
        ...
        return CompositeFeed(
            ...
            prob_provider=lambda code: 0.60), zdf

    def test_all_protocol_methods(self):
        feed, _ = self._feed()
        for fn in ("control_ratio", "red_above_blue_since_peak", "red_blue_distance_min",
                   "control_weekly_up", "bottom_breakout_volume", "recent_shadow_lines",
    ...
    def test_rule_engine_with_real_feed(self):
        ...
        c = Candidate("600519", "白酒", 0.03, 0.60,
                      daily_closes=[100, 101, 99, 102, 103], turnover_today=12)
...
class TestDualTrackTrainer:
    def test_split_window_segments(self):
        ...
        assert {k: len(v) for k, v in segs.items()} == \
            {"train": 690, "es": 10, "calib": 10, "test": 10}
    ...
    def test_train_smoke(self):
        ...
        df = pd.DataFrame({
            "date": dates, "symbol": "600519",
            "f1": f, "f2": rng.normal(size=720),
