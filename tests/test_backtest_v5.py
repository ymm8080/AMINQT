...
def sample_market_df():
    dates = pd.date_range("2024-01-01", periods=6, freq="B")
    return pd.DataFrame(
        {"date": dates, "index_close": [3000, 3010, 3005, 2990, 3015, 3020]}
    )
...
class TestReportGenerator:
    def test_generate_text_report(self, sample_config, tmp_path):
        ...
        basepath = reporter.generate(
            "squad", result_df, trades_df, metrics, data_version_hash="sha256:test"
        )
...
class TestBacktestEngine:
    ...
    def test_market_drop_no_data(self, sample_config, sample_pred_df, sample_price_df):
        ...
        assert (
            engine._check_market_drop(
                pd.Timestamp("2024-01-02"), pd.Timestamp("2024-01-01")
            )
            is True
        )

    def test_market_drop_with_data(
        self, sample_config, sample_pred_df, sample_price_df, sample_market_df
    ):
        from app.backtest.engine import BacktestEngine
