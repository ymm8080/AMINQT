...
def _cache_row(symbol, ann, period, **fina):
    row = {"symbol": symbol, "announce_date": pd.Timestamp(ann),
...
def test_select_latest_drops_rows_without_period_and_filters_cols():
    ...
    cache = _cache_frame([
        _cache_row("000001", "2026-08-28", "2026-06-30", roe=2.0),
        {"symbol": "000009", "announce_date": pd.Timestamp("2026-08-28"),
         "report_period": pd.NaT, "roe": 9.0},
    ])
...
def test_replay_respects_each_stock_announce_date():
    ...
    rows = pd.DataFrame({
        ...
        "date": pd.to_datetime([
            "2026-08-15", "2026-08-21", "2026-08-25", "2026-08-29", "2026-08-21",
        ]),
    })
...
def test_fina_cache_cols_exclude_protected_ffill_entries():
    ...
    banned = {
        "announce_date", "sh_change_vol", "sh_change_amt_total",
        "sh_net_change_sign", "sh_net_sign", "sw_l1_name", "sw_l2_name",
        "sw_l3_name", "margin_balance", "short_balance", "margin_buy_amt",
        "short_sell_vol",
    }
...
@pytest.mark.parametrize("sym,ann,period", [
...
def test_replay_single_row_smoke(sym, ann, period):
    ...
    rows = pd.DataFrame({
        "symbol": [sym], "date": pd.to_datetime(["2026-08-21"]),
    })
    out = replay_fina_asof(rows, cache, fina_cols=["roe"])
