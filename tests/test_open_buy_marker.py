...
def test_big_rise_flagged(tmp_path):
    ...
    rows = _many(20) + [
        {"symbol": "300001", "pctChg": 9.9, "amplitude_5d": 1.0, "winner_ratio": 0.2}
    ]
    out = open_buy_marker(
        _picks(["300001", "600001"]), TRADE_DATE, panel_path=_panel(tmp_path, rows)
    )
...
def test_hot_deep_chips_flagged(tmp_path):
    ...
    rows = _many(20) + [
        {"symbol": "300002", "pctChg": 2.0, "amplitude_5d": 9.9, "winner_ratio": 0.85}
    ]
    out = open_buy_marker(
        _picks(["300002"]), TRADE_DATE, panel_path=_panel(tmp_path, rows)
    )
    assert out.loc[0, "明日开盘"] == "勿买·防高开低走"
...
def test_hot_shallow_chips_not_flagged(tmp_path):
    ...
    rows = _many(20) + [
        {"symbol": "300003", "pctChg": 2.0, "amplitude_5d": 9.9, "winner_ratio": 0.5}
    ]
    out = open_buy_marker(
        _picks(["300003"]), TRADE_DATE, panel_path=_panel(tmp_path, rows)
    )
...
def test_calm_not_flagged(tmp_path):
    rows = _many(20) + [
        {"symbol": "300004", "pctChg": 1.5, "amplitude_5d": 1.2, "winner_ratio": 0.3}
    ]
    out = open_buy_marker(
        _picks(["300004"]), TRADE_DATE, panel_path=_panel(tmp_path, rows)
    )
    assert out.loc[0, "明日开盘"] == ""
...
def test_missing_panel_fail_open(tmp_path):
    out = open_buy_marker(
        _picks(["300001"]), TRADE_DATE, panel_path=tmp_path / "nope.parquet"
    )
...
def test_symbol_absent_from_panel(tmp_path):
    rows = _many(20)
    out = open_buy_marker(
        _picks(["999999"]), TRADE_DATE, panel_path=_panel(tmp_path, rows)
    )
    assert out.loc[0, "明日开盘"] == ""
...
def test_boundary_pct_rise(tmp_path):
    # 恰好 7.0 = 命中 (≥ 闭区间)
    rows = _many(20) + [
        {"symbol": "300005", "pctChg": 7.0, "amplitude_5d": 1.0, "winner_ratio": 0.2}
    ]
    out = open_buy_marker(
        _picks(["300005"]), TRADE_DATE, panel_path=_panel(tmp_path, rows)
    )
    assert out.loc[0, "明日开盘"] == "勿买·防高开低走"
...
def test_kill_line_disabled_noop(monkeypatch):
    ...
    monkeypatch.setattr(
        "scripts._open_buy_marker._gate_exempt",
        lambda *a, **k: (_ for _ in ()).throw(AssertionError("不应调涨闸")),
    )
    df = _flagged_df()
...
def test_kill_removes_flagged_not_passed_gate():
    ...
    df = (
        _flagged_df(with_gate_col=True)
        .drop(columns=["flag"])
        .rename(columns={"gate": "涨闸"})
    )
...
def test_kill_gate_recompute_exempts(monkeypatch):
    ...
    df = _flagged_df()
    monkeypatch.setattr(
        "scripts._open_buy_marker._gate_exempt", lambda syms, td, pp=None: {"600002"}
    )
...
def test_kill_no_flagged_noop(monkeypatch):
    monkeypatch.setattr(
        "scripts._open_buy_marker._gate_exempt",
        lambda *a, **k: (_ for _ in ()).throw(AssertionError("无 flagged 不该调涨闸")),
    )
