...
1d/2d 视界 2026-08-09 删除, 最近端触发统一用 pred_ret_3d.
"""
...
def test_watch_when_3d_turns_negative():
    # -0.3% 在黄色区间 (-0.5% < r3 < 0), 未到橙/红
    out = evaluate_sell_signal(pd.DataFrame([_row(pred_ret_3d=-0.003, prob_up=0.60)]))
...
def test_sell_when_3d_below_minus_half_percent():
    out = evaluate_sell_signal(pd.DataFrame([_row(pred_ret_3d=-0.006)]))
    assert out.loc[0, "sell_signal"] == "sell"


def test_strong_sell_when_3d_below_minus_1_5_percent():
    out = evaluate_sell_signal(pd.DataFrame([_row(pred_ret_3d=-0.02)]))
    assert out.loc[0, "sell_signal"] == "strong_sell"
    assert "3日预期" in out.loc[0, "sell_reason"]


def test_strong_sell_when_3d_deep_negative():
    out = evaluate_sell_signal(pd.DataFrame([_row(pred_ret_3d=-0.025)]))
    assert out.loc[0, "sell_signal"] == "strong_sell"
...
def test_missing_columns_safe():
    ...
    out = evaluate_sell_signal(
        pd.DataFrame([{"symbol": "600519", "pred_ret_3d": 0.01, "prob_up": 0.60}])
    )
...
def test_red_wins_over_orange_and_yellow():
    # 同票同时命中 红(pain 0.65) / 橙(pred_ret_3d -0.5%) / 黄(prob 0.48) → 红
    out = evaluate_sell_signal(
        pd.DataFrame([_row(pred_ret_3d=-0.005, prob_up=0.48, pain_prob=0.65)])
    )
    assert out.loc[0, "sell_signal"] == "strong_sell"
...
def test_multiple_symbols_vectorized():
    rows = [
        _row(symbol="600000", pred_ret_3d=-0.02),
        _row(symbol="600001", prob_up=0.43),
        _row(symbol="600002", pred_ret_3d=-0.003),
        _row(symbol="600003"),
    ]
