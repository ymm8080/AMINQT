...
def _make_tiny_onnx(path: str) -> np.ndarray:
    ...
    inp = onnx.helper.make_tensor_value_info("input", onnx.TensorProto.FLOAT, ["N", 20, 2])
    outp = onnx.helper.make_tensor_value_info("output", onnx.TensorProto.FLOAT, ["N", 1])
    ...
    graph = onnx.helper.make_graph([reshape, matmul], "tiny", [inp], [outp], [shape_c, w_t])
    model = onnx.helper.make_graph if False else onnx.helper.make_model(
...
class TestONNXPredictor:
    ...
    def test_verify_against_snapshot(self, model_dir):
        ...
        assert not p.verify_against_snapshot(X, snapshot + 1.0,
                                             universe=Universe.MAIN_BOARD)
