# -*- coding: utf-8 -*-
"""3年看跌信号池个股级回填 v3 — getDataList 真分页版 (perpage=100 真生效).

沿革: v2 及以前 get-robot-data 锁死10行/页 → 看跌日均2739只要~275请求/天 ≈ 天文数字;
v3 同看涨破局: robot-data(body风格) → footer_info.url → getDataList form POST
perpage=100 真生效 → ~28请求/天 × 490日 ≈ 1.4万请求 ≈ 半天级.
保留: found_n 审计 (bear_audit.csv), 全量扫描, 单实例锁(与看涨共用), 12轮自愈, fail-fast.
"""
import json
import os
import re
...
OUT_DIR = Path(r"D:/AMINQT/AMINQT CODES/tmp_t/ths_bear_daily_0910")
OUT_DIR.mkdir(exist_ok=True)
STATE = OUT_DIR / "_state.json"
AUDIT = OUT_DIR / "bear_audit.csv"
MAX_PAGES = 60  # 5096最大池/100 = 51页
MAX_ROUNDS = 12
...
# 单实例锁 (与看涨共享同一把: bull/bear 不允许同时打同一出口)
LOCK = Path(r"D:/AMINQT/AMINQT CODES/tmp_t/ths_bull_daily_0910/_sweep.lock")
...
QUESTION = "{dstr}看跌信号的股票"
t0 = time.time()
for rnd in range(1, MAX_ROUNDS + 1):
    pending = [d for d in dates if not _audit_valid(d.strftime("%Y%m%d"))]
    print(f"[round {rnd}/{MAX_ROUNDS}] 待处理 {len(pending)}/{len(dates)} 天")
    if not pending:
        break
    consec = 0
    done_cnt = 0
    ...
    for i, d in enumerate(pending):
        ...
        try:
            ...
            df.to_parquet(OUT_DIR / f"bear_{dstr}.parquet")
            ...
            consec = 0
            done_cnt += 1
            if done_cnt % 5 == 0 or not complete or captured == 0:
                el = time.time() - t0
                print(f"[r{rnd} {done_cnt}/{len(pending)}] {dstr}: 找到N={found_n} "
                      f"抓到{captured} ({end_reason}) | {el/3600:.1f}h")
            time.sleep(2.0)
        except Exception as e:
            consec += 1
            state["consec_fail"] = consec
            state["fail_date"] = dstr
            STATE.write_text(json.dumps(state), encoding="utf-8")
            print(f"[FAIL r{rnd}] {dstr}: {type(e).__name__} {str(e)[:80]} consec={consec}")
            if "WAF" in str(e) or "403" in str(e):
                time.sleep(180)
            else:
                time.sleep(15)
            if consec >= 8:
                print(f"[round-abort] r{rnd} 连续8失败, 冷却45min后开新轮")
                aborted = True
                break
    left = [d for d in dates if not _audit_valid(d.strftime("%Y%m%d"))]
    print(f"[round {rnd} done] 本轮处理{done_cnt}天, 剩{len(left)}天")
    if not left:
        break
    if not aborted:
        time.sleep(60)
    ...
    left = [d.strftime("%Y%m%d") for d in dates if not _audit_valid(d.strftime("%Y%m%d"))]
    print(f"[EXHAUSTED] {MAX_ROUNDS}轮耗尽, 仍缺 {len(left)} 天: {left[:20]}")
    print("SCRIPT-END")
    sys.exit(2)

n_complete = int(audit["complete"].sum()) if len(audit) else 0
print(f"[done] 全部交易日完成, audit complete {n_complete}/{len(dates)}, 最后 {state['last_done']}")
print("SCRIPT-END")
