#!/usr/bin/env python3
"""用已训练的 ab_test_after 模型跑预测，输出完整结果."""
...
def main():
    ...
    bundles = find_bundles(model_dir="models/pipeline1", tag="ab_test_after")

    output = {"boards": {}}

    for board, df in [("main", main_df), ("dual", dual_df)]:
        if len(df) == 0 or board not in bundles:
            logger.info(f"{board} 跳过 (df={len(df)}, bundle={'有' if board in bundles else '无'})")
            continue
        ...
        for kind in ('1d_reg', '3d_reg', '5d_reg', '1d_cls'):
            if kind in bundle.get("models", {}):
                ...
                board_out["model_info"][kind] = {
                    ...
                    "top5_importance_share": round(top5_share, 4),
                }
                logger.info(f"  {kind}: best_iter={bi}/{ne} features={model.n_features_in_} "
                            f"nonzero={nonzero} top5_share={top5_share:.2%}")
    ...
    with open("ab_after.json", 'w') as f:
        json.dump(output, f, indent=2, default=str)
    logger.info(f"\n结果已保存到 ab_after.json")
