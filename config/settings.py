...
# 排名 = 综合分降序. 主视界 = T+3 (2026-08-05 用户定案: 短持 3 天, 3d 权重最高 0.40, 短区间 2d+3d 合计 0.65).
...
# 2026-08-05 用户定案: 保留判定**只看主视界** (primary_horizon, 当前 T+3 短持), 不许用
# T+5/T+10 兜底把没优势的组合硬留下来 — "如果没有优势就不要入选" → 全不过线则空仓观望.
REGIME_GATE = {
    "enable": True,
    "top_quantile": 0.20,       # 高分 top-quantile = "系统选出的股"
    "margin": 0.01,             # 至少跑赢池基线 1pp (净收益) 才算"有优势"
    "primary_horizon": "3d",    # 保留判定只看主视界 (须与 SHORTLIST_SCORE 最高权重视界一致)
    "horizons": ("2d", "3d", "5d", "10d"),  # 仅 SUMMARY 展示用 (判定只认 primary_horizon)
