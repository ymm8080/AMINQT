...
# 按板覆写。ma5gt10_up5/ma5_gt_10 为已测旋钮, 勿再重扫。
PARALLEL_TREND_GATE = {
    "enable": True,
    # 按板分档: {"board": "ma10_up"|"ma5gt10_up5"|"ma5_gt_10"|"off"};
    # 字符串 = 全局同档 (整体回退口)
...
GENIOUS = {
    ...
    # ★ 闸**只作用于 Sheet1 冠军四段**: 冠军四段被砍到 1.3票/日, 若同时砍 Sheet2, 观察池
    #   Top20 会被掏空 (最近20日冠军四段过闸合计18只, 中位0)。故 Sheet2 保持按观察分取前 N
    #   **不过闸**, 只加「大涨闸」标注列; Sheet3 全量 + 同标注。
    #   注: B 在观察池上**反而是加分** (末250 ≥10% 1.51x→1.93x, ≥20% 1.20x→1.45x), 与冠军段
    #   相反 —— 但两处都不足以当筛子, Sheet2/Sheet3 的「大涨闸」列一律用与 Sheet1 相同的
    #   ACD 口径, 保证列义一致; 观察池自带 1.51x(≥10%) / 1.20x(≥20%), 够当自查标注。
    # 回退: dir_gate=False (Sheet1 全体过闸)。
    "dir_gate": True,
    "dir_gate_mom_max": 0.0,  # A 低动量: r10 上限
