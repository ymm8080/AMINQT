...
def zhuli_lasheng(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["吸筹峰"] = ((VAR5 > 0) & (VAR5 >= REF(VAR5, 1))
                   & (VAR5 >= REF(VAR5, -1)) & (REF(VAR5, -1) < REF(VAR5, -2)))

    # ---- 口诀信号：0轴上方死叉 + 出货柱 → 卖 ----
    df["轨迹死叉"] = (df["主力轨迹"] < df["MAZL"]) & (REF(df["主力轨迹"], 1) >= REF(df["MAZL"], 1))
...
if __name__ == "__main__":
    ...
    print("最近3日指标值：")
    print(df[["time", "close", "主力轨迹", "MAZL", "VAR5", "VAR51"]]
          .tail(3).round(3).to_string(index=False))

    print("\n近两年信号分布：")
    for sig in ["吸筹", "洗盘", "拉高", "出货", "吸筹峰", "上方死叉出货"]:
        dates = df.loc[df[sig] == True, "time"].astype(str).tolist()
        # 按月归并，便于肉眼校验
    ...
    for anchor in ["202409", "202410"]:
        ...
        print(f"{anchor}: 吸筹{int(seg['吸筹'].sum())}天 拉高{int(seg['拉高'].sum())}天 "
