...
def yimeng_dingdi(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["见顶"] = ((REF(M, 1) > 85) & (REF(S, 1) > 85) & (REF(L, 1) > 65)
                 & CROSS(L, S))

    top_zone = ((M < REF(M, 1)) & (REF(M, 1) > 80)
                & ((REF(S, 1) > 95) | (REF(S, 2) > 95))
                & (L > 60) & (S < 83.5) & (S < M) & (S < L + 4))
    df["顶部"] = FILTER(top_zone, 4)

    df["底部区域"] = ((L < 12) & (M < 8) & ((S < 7.2) | (REF(S, 1) < 5))
                     & ((M > REF(M, 1)) | (S > REF(S, 1)))) \
                    | ((L < 8) & (M < 7) & (S < 15) & (S > REF(S, 1))) \
                    | ((L < 10) & (M < 7) & (S < 1))

    df["低位金叉"] = ((L < 15) & (REF(L, 1) < 15) & (M < 18)
                     & (S > REF(S, 1)) & CROSS(S, L) & (S > M)
                     & ((REF(S, 1) < 5) | (REF(S, 2) < 5))
                     & ((M >= L) | (REF(S, 1) < 1)))
...
if __name__ == "__main__":
    ...
    for sig in ["见顶", "顶部", "底部区域", "低位金叉"]:
        dates = df.loc[df[sig] == True, "time"].tolist()
