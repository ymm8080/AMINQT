...
class Action(str, Enum):
    BUY = "BUY"; SELL_ALL = "SELL_ALL"; SELL_HALF = "SELL_HALF"
    HOLD = "HOLD"; BLOCKED = "BLOCKED"; WARN = "WARN"
...
class RuleEngine:
    ...
    # ---------------- L0 合规 ----------------
    def can_buy(self, c: Candidate, tick: Optional[Tick], prev_close: float) -> Optional[str]:
    ...
    # ---------------- 盘后任务 (STEP1~STEP4) ----------------
    def after_close(self, day: int, candidates: list[Candidate],
                    pf: PortfolioState) -> dict:
        ...
        for c in candidates:
            ...
            if self.feed.control_ratio(c.code) > cfg.control_min:
                case1 = (self.feed.had_accumulation_peak(c.code, cfg.peak_lookback)
                ...
                case4 = (self.feed.bottom_breakout_volume(c.code)
                         and self.feed.recent_shadow_lines(c.code))
                if case1 or case2 or case3 or case4:
                    c.flag_watch = True
                    watch.append(c)
            # STEP3 日线买点: 池内 + 红蓝距离最小 + 换手<50%
            if (self.feed.red_blue_distance_min(c.code)
        ...
        for code, pos in pf.positions.items():
            ...
            if hist:
                if len(hist.daily_closes) >= cfg.daily_close_lookback + 1:
                    if hist.daily_closes[-1] < hist.daily_closes[-1 - cfg.daily_close_lookback]:
                        reasons.append(f"收盘低于{cfg.daily_close_lookback}日前")
                if self.feed.profit_chip_ratio(code) < cfg.profit_chip_min:
                    reasons.append(f"获利盘{self.feed.profit_chip_ratio(code):.0f}%<40%")
            ...
            if self.feed.latest_prob_up(code) < cfg.prob_exit:
                reasons.append(f"概率衰减{self.feed.latest_prob_up(code):.2f}<{cfg.prob_exit}")
            if reasons:
                sell_marks.append(Order(day, "AFTER_CLOSE", code, Action.SELL_ALL,
                                        priority="P9-日线", reason=";".join(reasons)))
        return {"pool": pool, "watch": watch, "daily_buy": daily_buy,
                "daily_sell_marks": sell_marks, "rejected": rejected}

    # ---------------- 集合竞价 (9:15-9:25) ----------------
    def on_auction(self, day: int, pf: PortfolioState,
                   auction_prices: dict, prev_close: dict,
                   daily_sell_marks: list[Order]) -> list[Order]:
        ...
        for code in marked:
            ...
            if gap >= self.cfg.auction_gap_sell:
                orders.append(Order(day, "09:25", code, Action.SELL_ALL,
                                    priority="P2", reason=f"竞价高开{gap:.1f}%兑现"))
                pf.sold_today.add(code)
        return orders

    # ---------------- 盘中 tick (9:30-15:00, 每2分钟) ----------------
    def on_tick(self, day: int, tick_map: dict[str, Tick],
                candidates: list[Candidate], pf: PortfolioState,
                prev_close: dict, breadth: float,
                daily_sell_marks: list[Order]) -> list[Order]:
        orders = []
        marked = {o.code for o in daily_sell_marks}
        ...
        for code, tick in tick_map.items():
            ...
            if pos and code not in pf.sold_today:
                ...
                if blocked:
                    if blocked == "T+1不可卖":
                        orders.append(Order(day, tick.time, code, Action.HOLD,
                                            priority="L0", reason="T+1，风控挂单待明日"))
                else:
            ...
            # ===== 买入: 日内买点判定 + 入场形态 =====
            c = cand_map.get(code)
            if (c and c.flag_daily_buy and not c.flag_intraday_buy
                    and code not in pf.positions and code not in pf.sold_today
    ...
    def _sell_state_machine(self, day, code, pos, tick, chg, tr, marked):
        ...
        if chg <= cfg.hard_stop_intraday:
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P1",
        ...
        if pos.high_since_buy > pos.cost and tick.price > 0:
            ...
            if dd_high <= -cfg.trailing_drawdown:
                return Order(day, tick.time, code, Action.SELL_ALL, priority="P11",
        ...
        if prob < cfg.prob_exit:
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P12",
                         reason=f"概率衰减{prob:.2f}<{cfg.prob_exit}")

        # P3 涨15%全卖 (板块可达才生效: 20%/30%板块)
        if chg >= min(cfg.climax_full_exit, limit - 1.0):
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P3",
                         reason=f"涨{chg:.1f}%达板块高潮线，全卖")

        # P4 涨7%+无量+高峰回落 (量价背离)
        if (chg >= cfg.climax_move and tr.confirmed_peaks
                and tick.price < tr.confirmed_peaks[-1] * (1 - cfg.peak_confirm_drop)
                and tick.volume < 1.0):
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P4",
                         reason="涨7%+无量背离，高峰回落")

        # P5 涨7%+主力筹码<40%+下午 → 全卖
        if (chg >= cfg.climax_move and chip < cfg.chip_profit_min
                and tick.time >= cfg.afternoon):
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P5",
                         reason=f"午后涨{chg:.1f}%但获利盘{chip:.0f}%<40%，全卖")

        # P6 换手>40% 且涨8% → 全卖
        if tick.turnover > cfg.climax_turnover and chg >= cfg.half_sell_move:
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P6",
                         reason=f"换手{tick.turnover:.0f}%+涨{chg:.1f}%，全卖")

        # P7 涨7%+换手>40% → 卖一半 (只执行一次)
        if (chg >= cfg.climax_move and tick.turnover > cfg.climax_turnover
                and not pos.half_sold):
            pos.half_sold = True
            return Order(day, tick.time, code, Action.SELL_HALF, priority="P7",
                         reason="涨7%+高换手，减半")

        # P8 三峰连续下降 → 第三峰确认后卖
        if tr.three_peaks_descending():
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P8",
                         reason="三峰连续下降")

        # P9 日线卖出标记 (盘后生成, 盘中执行)
        if code in marked:
            return Order(day, tick.time, code, Action.SELL_ALL, priority="P9",
                         reason="日线卖出标记执行")

        # P10 浮盈20%+ WARNING (不自动卖, 人工复核)
        gain = (tick.price / pos.cost - 1) * 100
        if gain >= cfg.warn_gain:
            return Order(day, tick.time, code, Action.WARN, priority="P10",
                         reason=f"浮盈{gain:.0f}%≥{cfg.warn_gain:.0f}%，人工复核")
        return None

    # ---- 日内买点: 路径A(跳空) / 路径B(10:40前多维确认) ----
    def _check_intraday_buy_point(self, c: Candidate, tick: Tick,
                                  chg: float, breadth: float) -> None:
        cfg = self.cfg
        # 路径A: 跳空>4% + 放量
        ...
        if (tick.time <= cfg.buy_cutoff and breadth >= cfg.breadth_min
    ...
    def _entry_shape_machine(self, day, c, tick, tr) -> Optional[Order]:
        ...
        if state == BuyShape.WAIT_OPEN:
            ...
            if len(tr.bars) >= 5:
                ...
                self.buy_shapes[c.code] = (BuyShape.DOWN_WAIT_TROUGH if now < first
                                           else BuyShape.UP_WAIT_PEAK2)
            return None
        ...
        if state == BuyShape.DOWN_WAIT_TROUGH:
            ...
            if tr.confirmed_troughs and tick.price > tr.confirmed_troughs[-1]:
                ...
                return Order(day, tick.time, c.code, Action.BUY, priority="L4-形态",
