...
LGB_PARAMS_REG = dict(objective="huber", n_estimators=1000, learning_rate=0.05,
                      random_state=42, verbosity=-1)
LGB_PARAMS_CLS = dict(objective="binary", n_estimators=1000, learning_rate=0.05,
                      random_state=42, verbosity=-1)
...
class DualTrackTrainer:
    ...
    # ---------------- 单模型训练 ----------------
    def _train_one(self, kind: str, segs: dict[str, pd.DataFrame],
                   feature_cols: list[str]):
        import lightgbm as lgb

        label = {"1d_reg": "label_1d", "1d_cls": "label_cls",
                 "3d_reg": "label_3d", "5d_reg": "label_5d"}[kind]
        ...
        model.fit(X, y, sample_weight=w, eval_set=[(X_es, y_es)],
                  callbacks=[lgb.early_stopping(ES_PATIENCE, verbose=False)])
        return model, label

    # ---------------- 窗口训练 (单板块 4 模型) ----------------
    def train_window(self, df: pd.DataFrame, board: str,
        ...
        for kind in MODEL_KINDS:
            ...
            logger.info("[%s] %s 训练完成, 样本 %d", board, kind,
    ...
    def save(self, trained: dict, tag: str) -> str:
        ...
        with open(path, "wb") as fh:
            pickle.dump({"board": trained["board"],
                         "feature_cols": trained["feature_cols"],
                         "models": trained["models"],
                         "calibrator": trained["calibrator"]}, fh)
        return path
    ...
    def feature_similarity_check(trained: dict, threshold: float = 0.8) -> bool:
        ...
        corrs = [spearmanr(imps[i], imps[j]).statistic
    ...
    def monthly_retrain(self, panels: dict[str, pd.DataFrame],
        ...
        for board, df in panels.items():
            ...
            if not oos["pass"]:
                logger.warning("[%s] 新模型 OOS IC=%.4f < %.2f, 保留旧模型",
                               board, oos["ics"].get("1d_reg", 0.0), OOS_IC_MIN)
        return results
