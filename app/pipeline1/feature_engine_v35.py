...
class FeatureEngineV35:
    ...
    def build(self, df: pd.DataFrame,
              float_shares_map: dict | None = None) -> pd.DataFrame:
    ...
    def dim01_price_volume(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def per_stock(g: pd.DataFrame) -> pd.DataFrame:
            ...
            # KDJ(9,3,3)
            hhv9 = g["high_hfq"].rolling(9, min_periods=9).max() if "high_hfq" in g \
                else g["high"].rolling(9, min_periods=9).max()
            llv9 = g["low_hfq"].rolling(9, min_periods=9).min() if "low_hfq" in g \
            ...
            g["pv_divergence"] = np.where((pc > 0) & (vc < 0), 1,
                                          np.where((pc < 0) & (vc > 0), -1, 0))
            return g
    ...
    def dim02_volatility(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def per_stock(g: pd.DataFrame) -> pd.DataFrame:
            h = g.get("high_hfq", g["high"]); l = g.get("low_hfq", g["low"]); c = g["close_hfq"]
    ...
    # ---------------- ③基本面 ----------------
    def dim03_fundamentals(self, df: pd.DataFrame,
        ...
        if fundamentals is not None and len(fundamentals):
            ...
            df = pd.merge_asof(df, f, left_on="date", right_on="announce_date",
    ...
    def dim07_limit_gene(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "touched_limit_up" in df.columns:
            ...
            touched = gb["touched_limit_up"].rolling(20).sum().reset_index(level=0, drop=True)
            closed = df["limit_up_days_20"]
            df["break_board_rate"] = ((touched - closed) / touched.replace(0, np.nan))
        # 连板高度: 连续涨停计数, 截断到 4 (>=5 合并到 4)
        cons = (g.apply(lambda x: x * (x.groupby((x == 0).cumsum()).cumcount() + 1))
    ...
    def dim04_sector_effect(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["sector_return_5d"] = df.groupby("industry")["sector_return"] \
    ...
    def dim09_custom_formulas(self, df: pd.DataFrame,
                              float_shares_map: dict | None = None) -> pd.DataFrame:
    ...
    def dim10_money_flow(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        shift_cols = [c for c in ("chip_concentration", "profit_ratio",
                                  "main_money_flow", "super_large_order_net")
                      if c in df.columns]
    ...
    def dim12_ma_system(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def per_stock(g: pd.DataFrame) -> pd.DataFrame:
            ...
            g["ma_bull_align"] = ((g["MA5_dist"] > g["MA10_dist"])
                                  & (g["MA10_dist"] > g["MA20_dist"])).astype(int)
            g["ma_bear_align"] = ((g["MA5_dist"] < g["MA10_dist"])
                                  & (g["MA10_dist"] < g["MA20_dist"])).astype(int)
            return g
        return _apply_per_stock(df, per_stock)
    ...
    def dim14_market_sentiment(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        daily = df.groupby("date").agg(
            ...
            market_limit_up=("is_limit_up", "sum"))
        daily["market_turnover_ratio_5d"] = (daily["market_turnover"]
                                             / daily["market_turnover"].rolling(5).mean())
        daily["market_turnover_ratio_20d"] = (daily["market_turnover"]
                                              / daily["market_turnover"].rolling(20).mean())
    ...
    def industry_neutralize(df: pd.DataFrame, cols: list | None = None) -> pd.DataFrame:
        ...
        for col in (cols or NEUTRALIZE_COLS):
            if col in df.columns:
                df[f"{col}_industry_rank"] = df.groupby(["date", "industry"])[col].rank(pct=True)
        return df

    # ---------------- 缺失值策略 ----------------
    @staticmethod
    def add_missingness_flags(df: pd.DataFrame, cols: list | None = None) -> pd.DataFrame:
    ...
    def feature_columns(df: pd.DataFrame) -> list[str]:
        ...
        id_cols = {"symbol", "date", "board", "industry", "open", "high", "low",
                   "close", "pre_close", "volume", "amount", "turnover_rate",
                   "free_float_turnover_rate", "is_suspended", "is_st", "list_days",
                   "open_hfq", "high_hfq", "low_hfq", "close_hfq", "limit_pct",
                   "announce_date", "PE_TTM", "touched_limit_up", "score_rank",
                   "rank_amount", "rank_ff_turnover", "liquidity_score", "churn_suspect",
                   # dim09 中间量与前瞻信号: 吸筹峰含 REF(X,-1) 前瞻, 严禁入特征 (安全网 #4)
                   "吸筹峰", "VAR5", "VAR51", "time", "红在蓝上"}
