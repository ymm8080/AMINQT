...
# 申万一级行业指数代码 (28个) — 面板 industry 用老申万名; 801730 在面板记作 电气设备
# (SW2021 更名 电力设备). Tushare index_daily 取数需加 ".SI" 后缀.
SW_INDEX_CODES = {
    "801010": "农林牧渔",
    "801020": "采掘",
    "801030": "化工",
...
class DataSupplyChain:
    ...
    def fetch_chip_distribution(
        ...
        try:
            ...
            raise DataSupplyError(f"cyq_perf {ts_code} 拉取失败: {exc}") from exc
        if raw is None or len(raw) == 0:
            return pd.DataFrame()
        out = pd.DataFrame(
            {
            ...
            try:
                ...
                if raw is not None and len(raw) > 0:
                    ...
                    tushare_df = pd.DataFrame(
                        {
                            "date": pd.to_datetime(raw["trade_date"], format="%Y%m%d"),
                            "north_net_buy_sh": pd.to_numeric(
                                raw["hgt"], errors="coerce"
                            ),
                            "north_net_buy_sz": pd.to_numeric(
                                raw["sgt"], errors="coerce"
                            ),
                    ...
                    if len(tushare_df) > 0:
                        ...
                        logger.info(
                            "Tushare moneyflow_hsgt: %d 行北向净买入数据 (最近日期 %s)",
                            len(tushare_df),
        ...
        try:
            ...
            for market, prefix in [("沪股通", "sh"), ("深股通", "sz")]:
                try:
                    ...
                    if raw is not None and len(raw) > 0:
                        ...
                        if len(out) < before:
                            logger.info(
                                "AKShare 北向资金(%s): 过滤 %d 行无明细数据 (最后有效日期≈2024-08-16)",
                                market,
                                before - len(out),
                            )
            ...
            if sh_frames:
                ...
                if trade_date:
                    ...
                    if end_date:
                        merged = merged[
                            merged["date"] <= pd.to_datetime(end_date, format="%Y%m%d")
                        ]
                if len(merged) > 0:
        ...
        return pd.DataFrame(
            {
                ...
                "margin_balance": pd.to_numeric(
                    raw.get("rzye", raw.get("margin_balance", 0)), errors="coerce"
                ),
                "short_balance": pd.to_numeric(
                    raw.get("rqye", raw.get("short_balance", 0)), errors="coerce"
                ),
                "margin_buy_amt": pd.to_numeric(
                    raw.get("rzmre", raw.get("margin_buy", 0)), errors="coerce"
                ),
                "short_sell_vol": pd.to_numeric(
                    raw.get("rqmcl", raw.get("short_sell", 0)), errors="coerce"
                ),
            }
        )
        ...
        if pro is not None and trade_date:
            try:
                ...
                if top is not None and len(top) > 0:
                    top["symbol"] = (
                        top["ts_code"].str.replace(".SZ", "").str.replace(".SH", "")
                    )
                    top["date"] = pd.to_datetime(top["trade_date"], format="%Y%m%d")
                    # Tushare top_list 列名版本兼容 (net_buy/net_amount, l_buy/buy_amount 等)
                    net_col = next(
                        (c for c in ["net_buy", "net_amount"] if c in top.columns), None
                    )
                    top["lhb_net_buy"] = (
                        pd.to_numeric(top[net_col], errors="coerce") if net_col else 0
                    )
                    buy_col = next(
                        (c for c in ["buy_amount", "l_buy"] if c in top.columns), None
                    )
                    top["lhb_buy_amt"] = (
                        pd.to_numeric(top[buy_col], errors="coerce") if buy_col else 0
                    )
                    sell_col = next(
                        (c for c in ["sell_amount", "l_sell"] if c in top.columns), None
                    )
                    top["lhb_sell_amt"] = (
                        pd.to_numeric(top[sell_col], errors="coerce") if sell_col else 0
                    )
                    frames.append(top)
                ...
                try:
                    ...
                    if inst is not None and len(inst) > 0:
                        ...
                        inst["date"] = pd.to_datetime(
                            inst["trade_date"], format="%Y%m%d"
                        )
                        # 按 symbol+date 聚合机构席位
                        inst_agg = (
                            inst.groupby(["symbol", "date"])
                            .agg(
                        ...
                        inst_agg["lhb_institutional_net_buy"] = inst_agg[
                            "lhb_institutional_buy"
                        ].fillna(0) - inst_agg["lhb_institutional_sell"].fillna(0)
                        # merge 回 top_list 主表
                        if frames:
                            base = frames[0]
                            base = base.merge(
                                inst_agg, on=["symbol", "date"], how="left"
                            )
                            frames[0] = base
                except Exception as exc:
        ...
        if not frames:
            try:
                ...
                for fn, kwargs in [
                    ...
                    (
                        getattr(ak, "stock_lhb_jgmmtj_em", None),
                        {"start_date": dt, "end_date": de},
                    ),
                ]:
                    if fn is None:
                        continue
                ...
                if raw is not None and len(raw) > 0:
                    ...
                    symbol_col = next(
                        (
                            c
                            for c in ["代码", "stock_code", "ts_code", "symbol"]
                            if c in raw.columns
                        ),
                        None,
                    )
                    ...
                    date_col = next(
                        (
                            c
                            for c in ["日期", "上榜日期", "trade_date", "date"]
                            if c in raw.columns
                        ),
                        None,
                    )
                    ...
                    for col in raw.columns:
                        if "龙虎榜" in col and "净买" in col:
                            raw["lhb_net_buy"] = pd.to_numeric(
                                raw[col], errors="coerce"
                            )
                        elif "龙虎榜买入" in col:
                            raw["lhb_buy_amt"] = pd.to_numeric(
                                raw[col], errors="coerce"
                            )
                        elif "龙虎榜卖出" in col:
                            raw["lhb_sell_amt"] = pd.to_numeric(
                                raw[col], errors="coerce"
                            )
                    ...
                    out = pd.DataFrame(
                        {
                            "symbol": symbol,
                            "date": pd.to_datetime(
                                raw.get("股东户数统计截止日", raw.get("截止日期", "")),
                                errors="coerce",
                            ),
                            "holder_count": pd.to_numeric(
                                raw.get("股东户数", raw.get("股东总户数", 0)),
                                errors="coerce",
                            ),
                            "avg_shares_per_holder": pd.to_numeric(
                                raw.get("户均持股数", raw.get("户均持有流通股数", 0)),
                                errors="coerce",
                            ),
                        }
                    )
            ...
            try:
                ...
                if all_pages:
                    ...
                    change_vol = pd.to_numeric(
                        raw.get("change_vol", 0), errors="coerce"
                    ).fillna(0)
                    avg_price = pd.to_numeric(
                        raw.get("avg_price", 0), errors="coerce"
                    ).fillna(0)
                    ...
                    out = pd.DataFrame(
                        {
                            ...
                            .str.replace(".SH", ""),
                            "date": pd.to_datetime(
                                raw.get("ann_date", raw.get("trade_date", "")),
                                format="%Y%m%d",
                                errors="coerce",
                            ),
            ...
            try:
                ...
                if raw is not None and len(raw) > 0 and "增减持" in str(raw.columns):
                    ...
                    out["date"] = pd.to_datetime(
                        raw.get(
                            "公告日期",
                            raw.get("变动日期", datetime.now().strftime("%Y%m%d")),
                        ),
                        errors="coerce",
                    )
                    ...
                    out = pd.DataFrame(
                        {
                            ...
                            "open": pd.to_numeric(
                                raw.get("开盘", raw.get("开盘价", np.nan)),
                                errors="coerce",
                            ),
                            "high": pd.to_numeric(
                                raw.get("最高", raw.get("最高价", np.nan)),
                                errors="coerce",
                            ),
                            "low": pd.to_numeric(
                                raw.get("最低", raw.get("最低价", np.nan)),
                                errors="coerce",
                            ),
                            "close": pd.to_numeric(
                                raw.get("收盘", raw.get("收盘价", np.nan)),
                                errors="coerce",
                            ),
                            "volume": pd.to_numeric(
                                raw.get("成交量", np.nan), errors="coerce"
                            ),
                            "amount": pd.to_numeric(
                                raw.get("成交额", np.nan), errors="coerce"
                            ),
                        }
                    )
                    return out
            ...
            out = pd.DataFrame(
                {
                    ...
                    "volume_ratio": pd.to_numeric(
                        raw.get("volume_ratio", np.nan), errors="coerce"
                    ),
                    ...
                    "dv_ratio": pd.to_numeric(
                        raw.get("dv_ratio", np.nan), errors="coerce"
                    ),
                    "dv_ttm": pd.to_numeric(raw.get("dv_ttm", np.nan), errors="coerce"),
                    "total_mv": pd.to_numeric(
                        raw.get("total_mv", np.nan), errors="coerce"
                    ),
                    "circ_mv": pd.to_numeric(
                        raw.get("circ_mv", np.nan), errors="coerce"
                    ),
                    "total_share": pd.to_numeric(
                        raw.get("total_share", np.nan), errors="coerce"
                    ),
                    "float_share": pd.to_numeric(
                        raw.get("float_share", np.nan), errors="coerce"
                    ),
                    "free_share": pd.to_numeric(
                        raw.get("free_share", np.nan), errors="coerce"
                    ),
                }
            )
            out.to_parquet(path, index=False)
            ...
            out = pd.DataFrame(
                {
                    ...
                    "up_limit_raw": pd.to_numeric(
                        raw.get("up_limit", np.nan), errors="coerce"
                    ),
                    "down_limit_raw": pd.to_numeric(
                        raw.get("down_limit", np.nan), errors="coerce"
                    ),
                }
            )
            out.to_parquet(path, index=False)
    ...
    def append_today_to_panel(
        ...
        for src in alt_sources:
            ...
            if len(alt) == 0:
                continue
            if "date" in alt.columns and not pd.api.types.is_datetime64_any_dtype(
                alt["date"]
            ):
            ...
            if "symbol" in alt.columns:
                ...
                today_panel = today_panel.drop(
                    columns=[c for c in alt_cols if c in today_panel.columns],
                    errors="ignore",
                )
