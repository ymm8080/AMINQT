...
REQUIRED_COLUMNS = [
    "symbol", "date", "board", "open", "high", "low", "close",
...
class DataSupplyChain:
    ...
    def __init__(self, cache_dir: str = "data/supply_cache",
                 fetcher=None, fetcher_hist=None):
    ...
    def _akshare_fetch_hist(symbol: str, start: str, end: str) -> pd.DataFrame:
        ...
        fmt = lambda d: str(d).replace("-", "")
        raw = ak.stock_zh_a_hist(symbol=code6, period="daily",
                                 start_date=fmt(start), end_date=fmt(end), adjust="")
        hfq = ak.stock_zh_a_hist(symbol=code6, period="daily",
                                 start_date=fmt(start), end_date=fmt(end), adjust="hfq")
    ...
    def fetch_fundamentals(self) -> pd.DataFrame:
        ...
        try:
            ...
            daily = daily.rename(columns={"ts_code": "symbol", "trade_date": "announce_date",
                                          "pe_ttm": "PE_TTM", "pb": "PB_LF"})
            logger.warning("tushare daily_basic 以 trade_date 近似 announce_date; "
                           "严格 PIT 需 fina_indicator 的 ann_date (积分接口)")
            return daily
        except Exception as exc:
            raise NotImplementedError(
                "生产财务数据: 配置 TUSHARE_TOKEN (PIT 对齐需 announce_date)") from exc

    def fetch_money_flow(self, trade_date: str) -> pd.DataFrame:
        """资金流/筹码 — 锁死单一数据源 (东财), 换源即换模型."""
        import akshare as ak
        df = ak.stock_individual_fund_flow_rank(indicator="今日")
        df = df.rename(columns={"代码": "symbol", "今日主力净流入-净额": "main_money_flow",
    ...
    def fetch_market_sentiment(self, trade_date: str) -> dict:
        ...
        limit_up = int((daily["close"] >= daily["pre_close"] * 1.098).sum()) if len(daily) else 0
        limit_dn = int((daily["close"] <= daily["pre_close"] * 0.902).sum()) if len(daily) else 0
        return {"market_turnover": float(daily["amount"].sum()),
                "count_limit_up": limit_up, "count_limit_down": limit_dn,
                "hs300_chg": hs300_chg}
