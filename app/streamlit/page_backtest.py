...
def _demo_lists(panel: pd.DataFrame) -> dict:
    ...
    return {d: pd.DataFrame({"symbol": g["symbol"].values,
...
def render() -> None:
    ...
    st.caption("成交价 T+1 open + 滑点0.05% | 佣金万2.5双边 + 印花税0.05% | "
    ...
    if st.button("▶ 执行回测", type="primary"):
        proto = BacktestProtocol(top_n=top_n, max_hold_days=max_hold,
    ...
    tunable = st.multiselect("调参目标 ([TUNABLE])", sorted(TUNABLE_BOUNDS),
    ...
    if st.button("🔍 网格搜索 + OOS 复验"):
        if len(tunable) > 4:
            ...
            st.json({"best_params": report["best_params"],
