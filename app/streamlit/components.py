...
def kline_chart(df: pd.DataFrame, ma_list: tuple = (5, 20, 60),
                title: str = "") -> go.Figure:
    """日K线 + 均线 + 成交量副图."""
    ...
    fig.add_trace(go.Candlestick(
        ...
        increasing_line_color="#e54545", decreasing_line_color="#26a69a"), row=1, col=1)
    for w in ma_list:
        if len(df) >= w:
...
def intraday_chart(df: pd.DataFrame, prev_close: float | None = None,
                   title: str = "分时") -> go.Figure:
    """分时走势线 + 均价线 + 昨收价."""
    avg = (df["price"] * df["volume"]).cumsum() / df["volume"].cumsum()
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df["time"], y=df["price"], name="价格",
                             line=dict(color="#1f77b4", width=1.5)))
    fig.add_trace(go.Scatter(x=df["time"], y=avg, name="均价",
...
def equity_curve(nav_df: pd.DataFrame, title: str = "净值曲线") -> go.Figure:
    """回测净值曲线."""
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=nav_df["date"], y=nav_df["nav"], name="策略净值",
...
def drawdown_chart(nav_df: pd.DataFrame, title: str = "回撤") -> go.Figure:
    ...
    fig.add_trace(go.Scatter(x=nav_df["date"], y=dd, fill="tozeroy",
                             name="回撤", line=dict(color="#26a69a")))
...
def factor_radar(factors: dict, top_n: int = 10, title: str = "因子雷达") -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=list(values) + [values[0] if values else 0],
                                  theta=list(labels) + [labels[0] if labels else ""],
                                  fill="toself", name="因子值"))
