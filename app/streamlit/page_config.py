...
CONFIG_PATHS = {"selection": "config/selection_config.yaml",
...
def render() -> None:
    ...
    with tab_rules:
        ...
        st.caption("初始值 = 用户规则预设; 边界 = TUNABLE_BOUNDS; "
                   "调优流程见 回测中心 → 参数调优")
        cfg = Config()
        cols = st.columns(3)
        for i, (name, (lo, hi, step)) in enumerate(sorted(TUNABLE_BOUNDS.items())):
            with cols[i % 3]:
                st.number_input(f"{name} [{lo}~{hi}]", value=float(getattr(cfg, name)),
                                key=f"cfg_{name}", disabled=True,
                                help="P2 定稿后开放在线编辑; 当前经 回测中心→调参 写回")
        st.info("在线写回将在 Pipeline-2 定稿后开放; 当前流程: 回测中心调参 → tuning_report.json → apply_to_config")
    ...
    with tab_yaml:
        for label, path in CONFIG_PATHS.items():
            with st.expander(f"{label}: {path}"):
                ...
                text = st.text_area("YAML", value=_to_yaml(data), height=240,
                                    key=f"yaml_{label}")
    ...
    with tab_factors:
        ...
        st.dataframe({"维度": [d for d, _ in FACTOR_DIMS],
                      "组成": [c for _, c in FACTOR_DIMS]},
                     use_container_width=True)
        st.caption("因子集不固定: 每滚动重训窗口由 ICScreener 重算 (三标签并集 + 分类AUC), "
