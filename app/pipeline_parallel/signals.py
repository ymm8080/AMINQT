...
def no_buy_flags(df: pd.DataFrame, spec: dict | None = None) -> pd.DataFrame:
    ...
    df["vol_spike_up"] = (
        (_per_symbol_ret(df) > spec["vol_spike_up_max"])
...
def sell_signals(df: pd.DataFrame, spec: dict | None = None,
    ...
    prev_max = (prev.groupby(df["symbol"])
                .rolling(spec["turnover_spike_win"],
                         min_periods=spec["turnover_spike_win"])
                .max().reset_index(level=0, drop=True))
    df["turnover_spike"] = turn > prev_max
    if "adx_falling" not in df.columns:      # 独立调用 (未跑 no_buy_flags) → 就地算
        df["adx_falling"] = df["adx_rise5"] < 0
    if cost is not None:
        df["tp_80_div"] = ((df["close_cont"] / cost - 1) > spec["tp_gain"]) \
            & df["adx_falling"]
    else:
        win = spec["tp_high_window"]
        near_high = (
            df["close_cont"] >= df["close_cont"].groupby(df["symbol"])
            .rolling(win, min_periods=win).max().reset_index(level=0, drop=True)
            * (1 - spec["tp_high_near"])
        )
...
def trailing_stop_price(cost: float, profit_pct: float, ma20: float,
...
def daily_slowbull_pool(work: pd.DataFrame, date, board: str, spec,
    ...
    cols = ("symbol", "date", "close_cont", "ma5", "ma10", "ma20", "ma60",
            "adx", "pdi", "mdi", "adx_rise5", "turnover_rate", "vol_ratio",
